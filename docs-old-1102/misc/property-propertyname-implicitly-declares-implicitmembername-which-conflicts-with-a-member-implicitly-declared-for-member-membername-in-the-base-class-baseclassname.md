...
Member '\<membername1>' implicitly declares '\<implicitmembername>', which conflicts with a member implicitly declared for member '\<membername2>' in the base class '\<baseclassname>'. So the member should not be declared 'Overloads'.  
 ...
 A property in a derived class generates an implicit member with the same name as an implicit member of the base class and specifies the [Overloads](../Topic/Overloads%20\(Visual%20Basic\).md) keyword.  
...
-   If you intend to hide, or shadow, the implicit base class member, replace the [Overloads](../Topic/Overloads%20\(Visual%20Basic\).md) keyword with the [Shadows](../Topic/Shadows%20\(Visual%20Basic\).md) keyword in the declaration of the property.  

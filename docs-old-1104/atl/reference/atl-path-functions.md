...
---
# ATL Path functions
These functions support manipulation of file paths.
...
|[ATLPath::QutoeSpaces](#atlpath__quotespaces)|This function is an overloaded wrapper for [PathQuoteSpaces](http://msdn.microsoft.com/library/windows/desktop/bb773739).|
...
```    
inline char* Combine(char* pszDest, const char* pszDir, const char* pszFile);  
inline wchar_t* Combine(wchar_t* pszDest, const wchar_t* pszDir, const wchar_t* pszFile);  
```  
...
```    
inline int CommonPrefix(const char* pszFile1, const char* pszFile2, char* pszDest);  
inline int CommonPrefix(const wchar_t* pszFile1, const wchar_t* pszFile2, wchar_t* pszDest);  
```  
...
##  <a name="atlpath__conmpactpathex"></a>   ATLPath::CompactPathEx
...
##  <a name="atlpath__qutoespaces"></a>   ATLPath::QuoteSpaces

...
## The open Function for Output Streams  
 To use an output file stream ([ofstream](../Topic/ofstream.md)), you must associate that stream with a specific disk file in the constructor or the **open** function. If you use the **open** function, you can reuse the same stream object with a series of files. In either case, the arguments describing the file are the same.  
  
 When you open the file associated with an output stream, you generally specify an **open_mode** flag. You can combine these flags, which are defined as enumerators in the `ios` class, with the bitwise OR ( &#124; ) operator. See [ios_base::openmode](../Topic/ios_base::openmode.md) for a list of the enumerators.  
...
|--------------|------------------|  
|[bad](../Topic/basic_ios::bad.md)|Returns **true** if there is an unrecoverable error.|  
|[fail](../Topic/basic_ios::fail.md)|Returns **true** if there is an unrecoverable error or an "expected" condition, such as a conversion error, or if the file is not found. Processing can often resume after a call to **clear** with a zero argument.|  
|[good](../Topic/basic_ios::good.md)|Returns **true** if there is no error condition (unrecoverable or otherwise) and the end-of-file flag is not set.|  
|[eof](../Topic/basic_ios::eof.md)|Returns **true** on the end-of-file condition.|  
|[clear](../Topic/basic_ios::clear.md)|Sets the internal error state. If called with the default arguments, it clears all error bits.|  
|[rdstate](../Topic/basic_ios::rdstate.md)|Returns the current error state.|  

...
## Remarks  
 ...
 Key framework classes derived from `CCmdTarget` include [CView](../VS_visualcpp/CView-Class.md), [CWinApp](../VS_visualcpp/CWinApp-Class.md), [CDocument](../VS_visualcpp/CDocument-Class.md), [CWnd](../VS_visualcpp/CWnd-Class.md), and [CFrameWnd](../VS_visualcpp/CFrameWnd-Class.md). If you intend for a new class to handle messages, derive the class from one of these `CCmdTarget`-derived classes. You will rarely derive a class from `CCmdTarget` directly.  
  
 For an overview of command targets and `OnCmdMsg` routing, see [Command Targets](../VS_visualcpp/Command-Targets.md), [Command Routing](../VS_visualcpp/Command-Routing.md), and [Mapping Messages](../VS_visualcpp/Mapping-Messages.md).  
...
### Remarks  
 This function is typically called from the constructor of your object and should only be called if a dispatch map has been declared for the class. For more information on automation see the articles [Automation Clients](../VS_visualcpp/Automation-Clients.md) and [Automation Servers](../VS_visualcpp/Automation-Servers.md).  
...
### Remarks  
 Derived classes should override this member function (if not overridden, `GetDispatchIID` returns FALSE). See [COleControl](../VS_visualcpp/COleControl-Class.md).  
...
### Remarks  
 ...
 Derived classes should override this function to return the number of type information interfaces provided (either 0 or 1). If not overridden, **GetTypeInfoCount** returns 0. To override, use the [IMPLEMENT_OLETYPELIB](../Topic/IMPLEMENT_OLETYPELIB.md) macro, which also implements `GetTypeLib` and `GetTypeLibCache`.  
...
### Remarks  
 Derived classes should override this member function (if not overridden, `GetTypeLib` returns TYPE_E_CANTLOADLIBRARY). Use the [IMPLEMENT_OLETYPELIB](../Topic/IMPLEMENT_OLETYPELIB.md) macro, which also implements `GetTypeInfoCount` and `GetTypeLibCache`.  
...
### Remarks  
 Derived classes should override this member function (if not overridden, **GetTypeLibCache** returns NULL). Use the [IMPLEMENT_OLETYPELIB](../Topic/IMPLEMENT_OLETYPELIB.md) macro, which also implements `GetTypeInfoCount` and `GetTypeLib`.  
...
### Remarks  
 ...
 Derived classes can override this function to return appropriate values (if not overridden, `IsInvokeAllowed` returns TRUE). See in particular [COleControl::IsInvokeAllowed](../VS_visualcpp/COleControl-Class.md#colecontrol__isinvokeallowed).  
...
### Remarks  
 ...
 At run time, `OnCmdMsg` dispatches a command to other objects or handles the command itself by calling the root class `CCmdTarget::OnCmdMsg`, which does the actual message-map lookup. For a complete description of the default command routing, see [Message Handling and Mapping Topics](../VS_visualcpp/Message-Handling-and-Mapping.md).  
  
 On rare occasions, you may want to override this member function to extend the framework's standard command routing. Refer to [Technical Note 21](../VS_visualcpp/TN021--Command-and-Message-Routing.md) for advanced details of the command-routing architecture.  
...
|-------------------|--------------------|  
|CN_COMMAND|[CCmdUI](../VS_visualcpp/CCmdUI-Class.md)*|  
|CN_EVENT|AFX_EVENT*|  
|CN_UPDATE_COMMAND_UI|CCmdUI*|  
|CN_OLECOMMAND|[COleCmdUI](../VS_visualcpp/COleCmdUI-Class.md)*|  
...
## See Also  
 [MFC Sample ACDUAL](../VS_visualcpp/Visual-C---Samples.md)   

...
---
# gets_s, _getws_s
Gets a line from the `stdin` stream. These versions of [gets, _getws](../VS_visualcpp/gets--_getws.md) have security enhancements, as described in [Security Features in the CRT](../VS_visualcpp/Security-Features-in-the-CRT.md).  
...
## Return Value  
 Returns `buffer` if successful. A `NULL` pointer indicates an error or end-of-file condition. Use [ferror](../VS_visualcpp/ferror.md) or [feof](../VS_visualcpp/feof.md) to determine which one has occurred.  
...
## Remarks  
 ...
 If `buffer` is `NULL` or `sizeInCharacters` is less than or equal to zero, or if the buffer is too small to contain the input line and null terminator, these functions invoke an invalid parameter handler, as described in [Parameter Validation](../VS_visualcpp/Parameter-Validation.md). If execution is allowed to continue, these functions return `NULL` and set errno to `ERANGE`.  

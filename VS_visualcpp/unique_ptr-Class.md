...
public:  
    ...
    unique_ptr( const unique_ptr& Right  
            ) = delete;  
    unique_ptr& operator=(const unique_ptr& Right  
            ) = delete;  
};  
...
template <class T, class D> class unique_ptr<T[], D> {   public:          typedef pointer;  
     ...
     constexpr unique_ptr(nullptr_t) noexcept : unique_ptr() { }     template <class U, class E>       unique_ptr(unique_ptr<U, E>&& u) noexcept;  
       ~unique_ptr();  
     ...
     void swap(unique_ptr& u) noexcept;  
      // disable copy from lvalue     unique_ptr(const unique_ptr&) = delete;  
...
## Remarks  
 ...
 Use the [make_unique](../VS_visualcpp/-memory--functions.md#make_unique) helper function to efficiently create new instances of `unique_ptr`.  
 ...
 The resource is freed by calling a stored `deleter` object of type `Del` that knows how resources are allocated for a particular `unique_ptr`. The default `deleter``default_delete``<T>` assumes that the resource pointed to by `_Ptr` is allocated with `new`, and that it can be freed by calling `delete _``Ptr`. (A partial specialization `unique_ptr<T[]>`manages array objects allocated with `new[]`, and has the default `deleter``default_delete<T[]>`, specialized to call delete[] `_Ptr`.)  

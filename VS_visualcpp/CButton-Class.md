...
## Remarks  
 ...
 Typical buttons are the check box, radio button, and pushbutton. A `CButton` object can become any of these, according to the [button style](../VS_visualcpp/Button-Styles.md) specified at its initialization by the [Create](#cbutton__create) member function.  
  
 In addition, the [CBitmapButton](../VS_visualcpp/CBitmapButton-Class.md) class derived from `CButton` supports creation of button controls labeled with bitmap images instead of text. A `CBitmapButton` can have separate bitmaps for a button's up, down, focused, and disabled states.  
 ...
 If you want to handle Windows notification messages sent by a button control to its parent (usually a class derived from [CDialog](../VS_visualcpp/CDialog-Class.md)), add a message-map entry and message-handler member function to the parent class for each message.  
...
### Example  
 [!CODE [NVC_MFC_CButton#1](../CodeSnippet/VS_Snippets_Cpp/NVC_MFC_CButton#1)]  
...
### Parameters  
 ...
 `dwStyle`  
 Specifies the button control's style. Apply any combination of [button styles](../VS_visualcpp/Button-Styles.md) to the button.  
...
### Remarks  
 ...
 Apply the following [window styles](../VS_visualcpp/Window-Styles.md) to a button control:  
...
### Parameters  
 `lpDrawItemStruct`  
 A long pointer to a [DRAWITEMSTRUCT](../VS_visualcpp/DRAWITEMSTRUCT-Structure.md) structure. The structure contains information about the item to be drawn and the type of drawing required.  
...
### Remarks  
 ...
 Also see the [BS_](../VS_visualcpp/Button-Styles.md) style values.  
...
### Return Value  
 Returns the button styles for this `CButton` object. This function returns only the [BS_](../VS_visualcpp/Button-Styles.md) style values, not any of the other window styles.  
...
### Return Value  
 In the first overload, a [CString](../VS_visualcpp/Using-CString.md) object that contains the note text associated with the current command link control.  
...
##  <a name="cbutton__getsplitimagelist"></a>  CButton::GetSplitImageList  
 Retrieves the [image list](../VS_visualcpp/CImageList-Class.md) for the current split button control.  
...
### Return Value  
 A pointer to a [CImageList](../VS_visualcpp/CImageList-Class.md) object.  
...
-   **BS_VCENTER**  
 ...
 Unlike [CBitmapButton](../VS_visualcpp/CBitmapButton-Class.md), which uses four bitmaps per button, `SetBitmap` uses only one bitmap per the button. When the button is pressed, the bitmap appears to shift down and to the right.  
...
### Parameters  
 `nStyle`  
 Specifies the [button style](../VS_visualcpp/Button-Styles.md).  
...
-   **BS_VCENTER**  
 ...
 Unlike [CBitmapButton](../VS_visualcpp/CBitmapButton-Class.md), which uses four bitmaps per button, `SetCursor` uses only one cursor per the button. When the button is pressed, the cursor appears to shift down and to the right.  
...
-   **BS_VCENTER**  
 ...
 Unlike [CBitmapButton](../VS_visualcpp/CBitmapButton-Class.md), which uses four bitmaps per button, `SetIcon` uses only one icon per the button. When the button is pressed, the icon appears to shift down and to the right.  
...
##  <a name="cbutton__setsplitimagelist"></a>  CButton::SetSplitImageList  
 Associates an [image list](../VS_visualcpp/CImageList-Class.md) with the current split button control.  
...
|Parameter|Description|  
|---------------|-----------------|  
|[in] `pSplitImageList`|Pointer to a [CImageList](../VS_visualcpp/CImageList-Class.md) object to assign to the current split button control.|  

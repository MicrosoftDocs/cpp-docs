...
## Remarks  
 You can rely on the framework to handle many aspects of the printing process for your application. For more information about using the framework to handle printing tasks, see the article [Printing](../VS_visualcpp/Printing.md).  
 ...
 For more information on using `CPrintDialogEx`, see [Common Dialog Classes](../VS_visualcpp/Common-Dialog-Classes.md).  
...
### Remarks  
 ...
 `GetDefaults` does not display the Print property sheet. Instead, it sets the **hDevNames** and **hDevMode** members of [m_pdex](#cprintdialogex__m_pdex) to handles to the                         [DEVMODE](http://msdn.microsoft.com/library/windows/desktop/dd183565) and [DEVNAMES](../VS_visualcpp/DEVNAMES-Structure.md) structures that are initialized for the system default printer. Both **hDevNames** and **hDevMode** must be NULL, or `GetDefaults` fails.  
...
### Remarks  
 Use a pointer to the `CString` object returned by `GetDeviceName` as the value of `lpszDeviceName` in a call to [CDC::CreateDC](../VS_visualcpp/CDC-Class.md#cdc__createdc).  
...
### Remarks  
 Use a pointer to the `CString` object returned by `GetDriverName` as the value of `lpszDriverName` in a call to [CDC::CreateDC](../VS_visualcpp/CDC-Class.md#cdc__createdc).  
...
### Remarks  
 The specified page ranges can be determined from [m_pdex](#cprintdialogex__m_pdex) (see **nPageRanges**, **nMaxPageRanges**, and **lpPageRanges** in the                         [PRINTDLGEX](http://msdn.microsoft.com/library/windows/desktop/ms646844) structure in the Windows SDK).  

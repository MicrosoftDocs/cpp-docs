...
### Parameters  
 `pSid`  
 Pointer to a [CSid](../VS_visualcpp/CSid-Class.md) (security identifier) that receives a copy of the group stored in the CDacl.  
...
CSecurityDesc &  operator = (  
    const SECURITY_DESCRIPTOR & rhs ) throw(...);  
  
    CSecurityDesc &  operator = (  
    const CSecurityDesc & rhs ) throw(...);  
```  
...
### Parameters  
 `Sid`  
 The [CSid](../VS_visualcpp/CSid-Class.md) object for the security descriptor's new primary owner. This parameter must not be NULL.  

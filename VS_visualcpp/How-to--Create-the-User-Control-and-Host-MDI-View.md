...
The following steps show how to create a .NET Framework user control, author the user control in a control class library (specifically, a Windows Control Library project), and then compile the project into an assembly. The control can then be consumed from an MFC application that uses classes derived from [CView Class](../VS_visualcpp/CView-Class.md) and [CWinFormsView Class](../VS_visualcpp/CWinFormsView-Class.md).  
...
>  In some cases, Windows Forms controls, such as a third-party Grid control, might not behave reliably when hosted in an MFC application. A recommended workaround is to place a Windows Forms User Control in the MFC application and place the third-party Grid control inside the User control.  
 ...
 This procedure assumes that you created a Windows Forms Controls Library project named WindowsFormsControlLibrary1, as per the procedure in [How to: Create the User Control and Host in a Dialog Box](../VS_visualcpp/How-to--Create-the-User-Control-and-Host-in-a-Dialog-Box.md).  
...
6.  Modify the view class so that it inherits from [CWinFormsView](../VS_visualcpp/CWinFormsView-Class.md).  
  
     In MFC02View.h, replace [CView](../VS_visualcpp/CView-Class.md) with [CWinFormsView](../VS_visualcpp/CWinFormsView-Class.md) so that the code appears as follows:  

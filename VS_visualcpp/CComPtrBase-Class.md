...
## Remarks  
 This class provides the basis for other smart pointers which use COM memory management routines, such as [CComQIPtr](../VS_visualcpp/CComQIPtr-Class.md) and [CComPtr](../VS_visualcpp/CComPtr-Class.md). The derived classes add their own constructors and operators, but rely on the methods provided by `CComPtrBase`.  

...
>  This topic applies to objects derived from `CRecordset` in which bulk row fetching has not been implemented. If you are using bulk row fetching, see [Recordset: Fetching Records in Bulk (ODBC)](../VS_visualcpp/Recordset--Fetching-Records-in-Bulk--ODBC-.md).  
 ...
 As a supplement, you might want to read [Record Field Exchange: How RFX Works](../VS_visualcpp/Record-Field-Exchange--How-RFX-Works.md), which describes the corresponding role of RFX in update operations.  
...
-   If **::SQLSetPos** cannot be used, MFC does the following:  
    ...
    3.  **Update** commits the changes — executes the **UPDATE** statement — and the record is changed on the data source, but not committed if a transaction is in progress (see [Transaction: Performing a Transaction in a Recordset (ODBC)](../VS_visualcpp/Transaction--Performing-a-Transaction-in-a-Recordset--ODBC-.md) for information about how the transaction affects the update). ODBC keeps a copy of the record, which also changes.  
...
-   If **::SQLSetPos** cannot be used, MFC does the following:  
 ...
 For information about the SQL statements used in update operations, see [SQL](../VS_visualcpp/SQL.md).  

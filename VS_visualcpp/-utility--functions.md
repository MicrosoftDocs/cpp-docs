...
### Remarks  
 `make_pair` converts object of type [reference_wrapper Class](../VS_visualcpp/reference_wrapper-Class.md) to reference types and converts decaying arrays and functions to pointers.  
...
-   Otherwise, the returned type `Type1` is `decay<Type1>::type`. If [decay Class](../VS_visualcpp/decay-Class.md) is not supported, the returned type `Type1` is the same as the input type `Type1`.  
...
### Example  
  For an example about how to use the helper function `make_pair` to declare and initialize a pair, see [pair Structure](../VS_visualcpp/pair-Structure.md).  
...
##  <a name="swap"></a>  swap  
 Exchanges the elements of two [pair Structure](../VS_visualcpp/pair-Structure.md) objects.  
  
```  
template<class Type1, class Type2> void swap(pair<Type1, Type2>&  _Left, pair<Type1, Type2>&  _Right);  
```  

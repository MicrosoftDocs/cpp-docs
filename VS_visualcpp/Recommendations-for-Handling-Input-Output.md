...
-   No, the primary data doesn't reside in a disk file:  
    ...
    -   Yes, the data resides in an ODBC data source:  
         ...
         Use MFC's database support. The standard MFC implementation for this case includes a **CDocument** object that stores a `CDatabase` object, as discussed in the article [What Is the MFC Database Programming Model?](../VS_visualcpp/What-Is-the-MFC-Database-Programming-Model-.md). The application might also read and write an auxiliary file — the purpose of the application wizard "both a database view and file support" option. In this case, you'd use serialization for the auxiliary file.  
 ...
 For information about serialization, see [Serialization](../VS_visualcpp/Serialization-in-MFC.md).  

...
## Example  
 This example uses the [Asynchronous Agents Library](../VS_visualcpp/Asynchronous-Agents-Library.md) to download files from HTTP servers. The `http_reader` class derives from [concurrency::agent](../VS_visualcpp/agent-Class.md) and uses message passing to asynchronously read which URL names to download.  
  
 The `http_reader` class uses the [concurrency::task_group](../Topic/task_group%20Class.md) class to concurrently read each file. Each task calls the [concurrency::Context::Oversubscribe](../Topic/Context::Oversubscribe%20Method.md) method with the `_BeginOversubscription` parameter set to `true` to enable oversubscription in the current context. Each task then uses the Microsoft Foundation Classes (MFC) [CInternetSession](../VS_visualcpp/CInternetSession-Class.md) and [CHttpFile](../VS_visualcpp/CHttpFile-Class.md) classes to download the file. Finally, each task calls `Context::Oversubscribe` with the `_BeginOversubscription` parameter set to `false` to disable oversubscription.  

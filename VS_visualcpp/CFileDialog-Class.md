...
## Remarks  
 ...
 You can use [CFileDialog](../VS_visualcpp/CFileDialog-Class.md) as is with the constructor provided, or you can derive your own dialog box class from `CFileDialog` and write a constructor to suit your needs. In either case, these dialog boxes will behave like standard MFC dialog boxes because they are derived from the [CCommonDialog Class](../VS_visualcpp/CCommonDialog-Class.md). `CFileDialog` relies on the COMMDLG.DLL file that is included in Windows.  
...
-   [CDialog::OnInitDialog](../VS_visualcpp/CDialog-Class.md#cdialog__oninitdialog)  
  
-   [CDialog::OnSetFont](../VS_visualcpp/CDialog-Class.md#cdialog__onsetfont)  
  
 The windows messages for the `CFileDialog` class vary based on what operating system you are using. For example, Windows XP does not support [CDialog::OnCancel](../VS_visualcpp/CDialog-Class.md#cdialog__oncancel) and [CDialog::OnOK](../VS_visualcpp/CDialog-Class.md#cdialog__onok) for the `CFileDialog` class. However, Windows Vista does support them. For more information about the different messages that are generated and the order in which they are received, see [CFileDialog Sample: Logging Event Order](../VS_visualcpp/Visual-C---Samples.md).  
  
 To use a `CFileDialog` object, first create the object by using the `CFileDialog` constructor. After the dialog box has been constructed, you can set or modify any values in the [CFileDialog::m_ofn](#cfiledialog__m_ofn) structure to initialize the values or states of the dialog box controls. The `m_ofn` structure is of type `OPENFILENAME`. For more information, see the                 [OPENFILENAME](http://msdn.microsoft.com/library/windows/desktop/ms646839) structure in the Windows SDK.  
...
> [!NOTE]
>  Under Windows Vista, multiple calls to                     [IFileDialog::SetFileTypes](http://msdn.microsoft.com/library/windows/desktop/bb775980) causes an error. The second call to `SetFileTypes` for any instance of a `CFileDialog` will return `E_UNEXPECTED` in Windows Vista. Some `CFileDialog` method functions call `SetFileTypes`. For example, two calls to `CFileDialog::DoModal` for the same instance of a `CFileDialog` generates [ASSERT](../Topic/ASSERT%20\(MFC\).md).  
 ...
 The destruction of `CFileDialog` objects is handled automatically. You do not have to call [CDialog::EndDialog](../VS_visualcpp/CDialog-Class.md#cdialog__enddialog).  
 ...
 For more information about how to use `CFileDialog`, see [Common Dialog Classes](../VS_visualcpp/Common-Dialog-Classes.md).  
...
##  <a name="cfiledialog__applyofntoshelldialog"></a>  CFileDialog::ApplyOFNToShellDialog  
 Updates the current state of the [CFileDialog](../VS_visualcpp/CFileDialog-Class.md) based on the values stored in the `m_ofn` data structure.  
...
### Remarks  
 ...
 To enable the user to select multiple files, set the `OFN_ALLOWMULTISELECT` flag before you call [DoModal](#cfiledialog__domodal). You must supply your own file name buffer to store the returned list of multiple file names. Do this by replacing `m_ofn.lpstrFile` with a pointer to a buffer you have allocated, after you construct the [CFileDialog](../VS_visualcpp/CFileDialog-Class.md), but before you call `DoModal`. Additionally, you must set `m_ofn.nMaxFile` with the number of characters in the buffer pointed to by `m_ofn.lpstrFile`. If you set the maximum number of files to be selected to `n`, the necessary buffer size is `n`*(_MAX_PATH + 1) + 1. For example:  
 ...
 The `lpszFilter` parameter is used to determine the type of file name a file must have to be displayed in the file list. The first string in the string pair describes the filter; the second string indicates the file name extension to use. Multiple extensions may be specified by using a semicolon (the ';' character) as the delimiter. The string ends with two '&#124;' characters, followed by a `NULL` character. You can also use a [CString](../VS_visualcpp/Using-CString.md) object for this parameter.  
...
### Remarks  
 ...
 For example, if you want to allow the user to select multiple files, set the `OFN_ALLOWMULTISELECT` flag before calling `DoModal`, as shown in the code example in [CFileDialog Class](../VS_visualcpp/CFileDialog-Class.md).  
...
### Return Value  
 A [CString](../VS_visualcpp/CStringT-Class.md) object containing the currently open folder or directory.  
...
##  <a name="cfiledialog__getifiledialogcustomize"></a>  CFileDialog::GetIFileDialogCustomize  
 Retrieves a pointer to the internal COM object for a given [CFileDialog](../VS_visualcpp/CFileDialog-Class.md).  
...
### Remarks  
 You can hide the Read Only check box by setting the `OFN_HIDEREADONLY` style in the [CFileDialog](../VS_visualcpp/CFileDialog-Class.md) constructor.  
  
> [!NOTE]
>  Windows Vista style `CFileDialog` objects do not support this function. Attempting to use this function on a Windows Vista style `CFileDialog` will throw [CNotSupportedException](../VS_visualcpp/CNotSupportedException-Class.md). See [CFileDialog Class](../VS_visualcpp/CFileDialog-Class.md) for more information.  
...
-   OFN_SHOWHELP  
 ...
 For more information about this structure, see the                         [OPENFILENAME](http://msdn.microsoft.com/library/windows/desktop/ms646839) structure in the Windows SDK. For more information about the different behavior of the `CFileDialog` under Windows Vista, see [CFileDialog Class](../VS_visualcpp/CFileDialog-Class.md).  
...
> [!NOTE]
>  Windows Vista style file dialogs do not support this function. Attempting to use this function on a Windows Vista style file dialog will throw [CNotSupportedException](../VS_visualcpp/CNotSupportedException-Class.md). See [CFileDialog Class](../VS_visualcpp/CFileDialog-Class.md) for more information.  
...
##  <a name="cfiledialog__settemplate"></a>  CFileDialog::SetTemplate  
 Sets the dialog box template for the [CFileDialog](../VS_visualcpp/CFileDialog-Class.md) object.  
...
> [!NOTE]
>  Windows Vista style file dialog boxes do not support this function. Attempting to use this function on a Windows Vista style file dialog box will throw [CNotSupportedException](../VS_visualcpp/CNotSupportedException-Class.md). See [CFileDialog Class](../VS_visualcpp/CFileDialog-Class.md) for more information. An alternative is to use a customized dialog. For more information about using a custom `CFileDialog`, see                             [IFileDialogCustomize](http://msdn.microsoft.com/library/windows/desktop/bb775912).  
...
##  <a name="cfiledialog__updateofnfromshelldialog"></a>  CFileDialog::UpdateOFNFromShellDialog  
 Updates the `m_ofn` data structure of the [CFileDialog](../VS_visualcpp/CFileDialog-Class.md) based on the current state of the internal object.  

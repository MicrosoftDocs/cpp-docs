...
A [CString](../VS_visualcpp/Using-CString.md) object contains character string data. `CString` inherits the set of the [methods and operators](../VS_visualcpp/CStringT-Class.md) that are defined in the class template [CStringT](../VS_visualcpp/CStringT-Class.md) to work with string data. (`CString` is a `typedef` that specializes `CStringT` to work with the kind of character data that `CString` supports.)  

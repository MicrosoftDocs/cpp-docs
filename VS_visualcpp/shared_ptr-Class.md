...
-   if it was constructed from a [weak_ptr Class](../VS_visualcpp/weak_ptr-Class.md) object that points to that resource, or  
...
### Remarks  
 The constructors each construct an object that owns the resource named by the operand sequence. The constructor `shared_ptr(const weak_ptr<Other>& wp)` throws an exception object of type [bad_weak_ptr Class](../VS_visualcpp/bad_weak_ptr-Class.md) if `wp.expired()`.  

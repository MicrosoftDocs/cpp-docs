...
---
# fopen_s, _wfopen_s
Opens a file. These versions of [fopen, _wfopen](../VS_visualcpp/fopen--_wfopen.md) have security enhancements, as described in [Security Features in the CRT](../VS_visualcpp/Security-Features-in-the-CRT.md).  
...
## Return Value  
 Zero if successful; an error code on failure. See [errno, _doserrno, _sys_errlist, and _sys_nerr](../VS_visualcpp/errno--_doserrno--_sys_errlist--and-_sys_nerr.md) for more information about these error codes.  
...
## Remarks  
 Files that are opened by `fopen_s` and `_wfopen_s` are not sharable. If you require that a file be sharable, use [_fsopen, _wfsopen](../VS_visualcpp/_fsopen--_wfsopen.md) with the appropriate sharing mode constant—for example, `_SH_DENYNO` for read/write sharing.  
 ...
 These functions validate their parameters. If `pFile`, `filename`, or `mode` is a null pointer, these functions generate an invalid parameter exception, as described in [Parameter Validation](../VS_visualcpp/Parameter-Validation.md).  
  
 Always check the return value to see if the function succeeded before you perform any further operations on the file. If an error occurs, the error code is returned and the global variable `errno` is set. For more information, see [errno, _doserrno, _sys_errlist, and _sys_nerr](../VS_visualcpp/errno--_doserrno--_sys_errlist--and-_sys_nerr.md).  
...
|`_tfopen_s`|`fopen_s`|`fopen_s`|`_wfopen_s`|  
 ...
 For more information about using text and binary modes in Unicode and multibyte stream-I/O, see [Text and Binary Mode File I/O](../VS_visualcpp/Text-and-Binary-Mode-File-I-O.md) and [Unicode Stream I/O in Text and Binary Modes](../VS_visualcpp/Unicode-Stream-I-O-in-Text-and-Binary-Modes.md).  
  
 `c`  
 Enable the commit flag for the associated `filename` so that the contents of the file buffer are written directly to disk if either `fflush` or `_flushall` is called.  
  
 `n`  
 Reset the commit flag for the associated `filename` to "no-commit." This is the default. It also overrides the global commit flag if you link your program with COMMODE.OBJ. The global commit flag default is "no-commit" unless you explicitly link your program with COMMODE.OBJ (see [Link Options](../VS_visualcpp/Link-Options.md)).  
 ...
 Valid characters for the `mode` string used in `fopen_s` and `_fdopen` correspond to `oflag` arguments used in [_open](../VS_visualcpp/_open--_wopen.md) and [_sopen](../VS_visualcpp/_sopen--_wsopen.md), as follows.  

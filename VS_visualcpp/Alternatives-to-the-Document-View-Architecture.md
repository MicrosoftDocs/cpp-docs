...
ms.tgt_pltfrm: na
ms.topic: article
H1: Alternatives to the Document/View Architecture
...
-   If your original code already mixes data management with data viewing, moving the code to the document/view model is not worth the effort because you must separate the two. You might prefer to leave the code as is.  
 ...
 To create an application that does not use the document/view architecture, clear the **Document/View architecture support** check box in step 1 of the MFC Application Wizard. See [MFC Application Wizard](../VS_visualcpp/MFC-Application-Wizard.md) for details.  
...
-   Treat the document as an unused appendage and implement your data management code in the view class, as suggested above. Overhead for the document is relatively low. A single [CDocument](../VS_visualcpp/CDocument-Class.md) object incurs a small amount of overhead by itself, plus the small overhead of **CDocument**'s base classes, [CCmdTarget](../VS_visualcpp/CCmdTarget-Class.md) and [CObject](../VS_visualcpp/CObject-Class.md). Both of the latter classes are small.  

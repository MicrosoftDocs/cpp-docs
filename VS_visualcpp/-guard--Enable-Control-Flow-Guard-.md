...
## Remarks  
 ...
 A common attack on software takes advantage of bugs in handling extreme or unexpected inputs. Carefully crafted input to the application may overwrite a location that contains a pointer to executable code. This can be used to redirect control flow to code controlled by the attacker. The CFG runtime checks do not fix the data corruption bugs in your executable. They instead make it more difficult for an attacker to use them to execute arbitrary code. CFG is a mitigation tool that prevents calls to locations other than function entry points in your code. It's similar to how Data Execution Prevention (DEP),  [/GS](../VS_visualcpp/-GS--Buffer-Security-Check-.md) stack checks, and [/DYNAMICBASE](../VS_visualcpp/-DYNAMICBASE--Use-address-space-layout-randomization-.md) and [/HIGHENTROPYVA](../VS_visualcpp/-HIGHENTROPYVA--Support-64-Bit-ASLR-.md) address space layout randomization (ASLR) lower the chances that your code becomes an exploit vector.  
 ...
 The **/guard:cf** option is incompatible with [/ZI](../VS_visualcpp/-Z7---Zi---ZI--Debug-Information-Format-.md) (Edit and Continue debug information) or [/clr](../VS_visualcpp/-clr--Common-Language-Runtime-Compilation-.md) (Common Language Runtime Compilation).  

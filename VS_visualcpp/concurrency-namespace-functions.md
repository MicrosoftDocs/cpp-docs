...
### Remarks  
 For more information about which scenarios in your application could benefit from using the Caching Suballocator, see                         [Task Scheduler](../VS_visualcpp/Task-Scheduler--Concurrency-Runtime-.md).  
...
### Remarks  
 For more information, see                         [Message Passing Functions](../VS_visualcpp/Message-Passing-Functions.md).  
...
### Remarks  
 ...
 [unsupported_os](../VS_visualcpp/unsupported_os-Class.md) is thrown if the operating system is not supported by the Concurrency Runtime.  
...
### Remarks  
 For more information, see                         [Cancellation](../VS_visualcpp/Cancellation-in-the-PPL.md).  
...
### Parameters  
 ...
 `_Step`  
 The value by which to step when iterating from                                 `_First` to                                 `_Last`. The step must be positive.                                 [invalid_argument](../VS_visualcpp/invalid_argument-Class.md) is thrown if the step is less than 1.  
 ...
 `_Part`  
 A reference to the partitioner object. The argument can be one of                                 `const`[auto_partitioner](../VS_visualcpp/auto_partitioner-Class.md)`&`,                                 `const`[static_partitioner](../VS_visualcpp/static_partitioner-Class.md)`&`,                                 `const`[simple_partitioner](../VS_visualcpp/simple_partitioner-Class.md)`&` or                                 [affinity_partitioner](../VS_visualcpp/affinity_partitioner-Class.md)`&` If an                                 [affinity_partitioner](../VS_visualcpp/affinity_partitioner-Class.md) object is used, the reference must be a non-const l-value reference, so that the algorithm can store state for future loops to re-use.  
  
### Remarks  
 For more information, see                         [Parallel Algorithms](../VS_visualcpp/Parallel-Algorithms.md).  
...
### Remarks  
 [auto_partitioner](../VS_visualcpp/auto_partitioner-Class.md) will be used for the overload without an explicit partitioner.  
  
 For iterators that do not support random access, only                         [auto_partitioner](../VS_visualcpp/auto_partitioner-Class.md) is supported.  
...
### Remarks  
 [auto_partitioner](../VS_visualcpp/auto_partitioner-Class.md) will be used for the overloads without an explicit partitioner argument.  
...
### Remarks  
 The method will throw an                         [invalid_operation](../VS_visualcpp/invalid_operation-Class.md) exception if a Resource Manager is present at the time it is invoked, and an                         [invalid_argument](../VS_visualcpp/invalid_argument-Class.md) exception if the affinity specified results in an empty set of resources.  
  
 The version of the method that takes an array of group affinities as a parameter should only be used on operating systems with version Windows 7 or higher. Otherwise, an                         [invalid_operation](../VS_visualcpp/invalid_operation-Class.md) exception is thrown.  
...
### Remarks  
 `when_all` is a non-blocking function that produces a                         `task` as its result. Unlike                         [task::wait](../VS_visualcpp/task-Class--Concurrency-Runtime-.md#task__wait_method), it is safe to call this function in a                         Windows 8.x Store app on the ASTA (Application STA) thread.  
  
 If one of the tasks is canceled or throws an exception, the returned task will complete early, in the canceled state, and the exception, if one is encoutered, will be thrown if you call                         [task::get](../VS_visualcpp/task-Class--Concurrency-Runtime-.md#task__get_method) or                         `task::wait` on that task.  
  
 For more information, see                         [Task Parallelism](../VS_visualcpp/Task-Parallelism--Concurrency-Runtime-.md).  
...
### Remarks  
 `when_any` is a non-blocking function that produces a                         `task` as its result. Unlike                         [task::wait](../VS_visualcpp/task-Class--Concurrency-Runtime-.md#task__wait_method), it is safe to call this function in a                         Windows 8.x Store app on the ASTA (Application STA) thread.  

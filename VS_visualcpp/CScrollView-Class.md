...
---
# CScrollView Class
A [CView](../VS_visualcpp/CView-Class.md) with scrolling capabilities.  
...
## Remarks  
 You can handle standard scrolling yourself in any class derived from `CView` by overriding the message-mapped [OnHScroll](../VS_visualcpp/CWnd-Class.md#cwnd__onhscroll) and [OnVScroll](../VS_visualcpp/CWnd-Class.md#cwnd__onvscroll) member functions. But `CScrollView` adds the following features to its `CView` capabilities:  
...
-   It scrolls automatically in response to messages from the keyboard, a non-scrolling mouse, or the IntelliMouse wheel.  
 ...
 You can handle mouse wheel scrolling yourself by overriding the message-mapped [OnMouseWheel](../VS_visualcpp/CWnd-Class.md#cwnd__onmousewheel) and [OnRegisteredMouseWheel](../VS_visualcpp/CWnd-Class.md#cwnd__onregisteredmousewheel) member functions. As they are for `CScrollView`, these member functions support the recommended behaviour for                 [WM_MOUSEWHEEL](http://msdn.microsoft.com/library/windows/desktop/ms645617), the wheel rotation message.  
  
 To take advantage of automatic scrolling, derive your view class from `CScrollView` instead of from `CView`. When the view is first created, if you want to calculate the size of the scrollable view based on the size of the document, call the `SetScrollSizes` member function from your override of either [CView::OnInitialUpdate](../VS_visualcpp/CView-Class.md#cview__oninitialupdate) or [CView::OnUpdate](../VS_visualcpp/CView-Class.md#cview__onupdate). (You must write your own code to query the size of the document. For an example, see the [Scribble sample](../VS_visualcpp/Visual-C---Samples.md).)  
...
-   The framework also forwards scroll messages from a `CSplitterWnd` splitter control to the currently active splitter pane (a view). When placed in a [CSplitterWnd](../VS_visualcpp/CSplitterWnd-Class.md) with shared scroll bars, a `CScrollView` object will use the shared ones rather than creating its own.  
  
 For more information on using `CScrollView`, see [Document/View Architecture](../VS_visualcpp/Document-View-Architecture.md) and [Derived View Classes Available in MFC](../VS_visualcpp/Derived-View-Classes-Available-in-MFC.md).  
...
## See Also  
 [MFC Sample DIBLOOK](../VS_visualcpp/Visual-C---Samples.md)   

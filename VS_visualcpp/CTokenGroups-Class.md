...
void Add(  
    const CSid & rSid,  
    DWORD dwAttributes ) throw(...);  
  
    void Add(  
    const TOKEN_GROUPS & rTokenGroups ) throw(...);  
```  
  
### Parameters  
 `rSid`  
 A [CSid](../VS_visualcpp/CSid-Class.md) object.  
...
### Parameters  
 `rSid`  
 The [CSid](../VS_visualcpp/CSid-Class.md) object for which the security identifier (SID) and attributes should be removed.  
...
### Return Value  
 Returns the number of [CSid](../VS_visualcpp/CSid-Class.md) objects and their associated attributes contained in the `CTokenGroups` object.  
...
### Parameters  
 `pSids`  
 Pointer to an array of [CSid](../VS_visualcpp/CSid-Class.md) objects.  
...
### Parameters  
 `rSid`  
 The [CSid](../VS_visualcpp/CSid-Class.md) object.  
...
CTokenGroups &  operator = (  
    const TOKEN_GROUPS & rhs ) throw(...);  
  
    CTokenGroups &  operator = (  
    const CTokenGroups & rhs ) throw(...);  
```  

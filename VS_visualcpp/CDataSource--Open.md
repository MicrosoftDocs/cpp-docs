 ...
 *pPassword*  
 ...
 `enumerator`  
 [in] A [CEnumerator](../VS_visualcpp/CEnumerator-Class.md) object used to obtain a moniker for opening the connection when the caller does not specify a **CLSID**.  
...
## Remarks  
 ...
 The method overloads that do not use the `hWnd` parameter open a data source object without using the service components in oledb32.dll. A [CDataSource](../VS_visualcpp/CDataSource-Class.md) object opened with these function overloads will be unable to utilize any of the functionality of Service Components.  

...
```  
 ...
 ![Structural elements of a lambda expression](../VS_visualcpp/media/LambdaExpSyntax.png "LambdaExpSyntax")  
...
```  
 ...
 A `capture` followed by an ellipsis is a pack expansion, as shown in this [variadic template](../VS_visualcpp/Ellipses-and-Variadic-Templates.md) example:  
...
```  
 ...
 A lambda expression can take another lambda expression as its argument. For more information, see "Higher-Order Lambda Expressions" in the topic [Examples of Lambda Expressions](../VS_visualcpp/Examples-of-Lambda-Expressions.md).  
...
### Return Type  
 The return type of a lambda expression is automatically deduced. You don't have to use the [auto](../VS_visualcpp/auto--C---.md) keyword unless you specify a *trailing-return-type*. The *trailing-return-type* resembles the return-type part of an ordinary method or function. However, the return type must follow the parameter list, and you must include the trailing-return-type keyword `->` before the return type.  
...
```  
 ...
 A lambda expression can produce another lambda expression as its return value. For more information, see "Higher-Order Lambda Expressions" in [Examples of Lambda Expressions](../VS_visualcpp/Examples-of-Lambda-Expressions.md).  
...
**vector v after 2nd call to fillVector(): 10 11 12 13 14 15 16 17 18** For more information, see [generate_n](../Topic/generate_n.md).  
...
## Microsoft-Specific  
 ...
 If you are using a Microsoft-specific modifier such as [__declspec](../VS_visualcpp/__declspec.md), you can insert it into a lambda expression immediately after the `parameter-declaration-clause`—for example:  
...
```  
 ...
 To determine whether a modifier is supported by lambdas, see the article about it in the [Microsoft-Specific Modifiers](../VS_visualcpp/Microsoft-Specific-Modifiers.md) section of the documentation.  
  
 Visual Studio supports C++11 Standard lambda expression syntax and functionality, with these exceptions:  
  
-   Like all other classes, lambdas don't get automatically generated move constructors and move assignment operators. For more information about support for rvalue reference behaviors, see the "Rvalue References" section in [Support For C++11/14/17 Features (Modern C++)](../VS_visualcpp/Support-For-C--11-14-17-Features--Modern-C---.md).  

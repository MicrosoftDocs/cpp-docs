...
1.  Replace the `Apply` function in PolyProp.h with the following code:  
 ...
 A property page can have more than one client attached to it at a time, so the `Apply` function loops around and calls `put_Sides` on each client with the value retrieved from the edit box. You are using the [CComQIPtr](../VS_visualcpp/CComQIPtr-Class.md) class, which performs the `QueryInterface` on each object to obtain the `IPolyCtl` interface from the **IUnknown** interface (stored in the `m_ppUnk` array).  
...
## Building and Testing the Control  
 ...
 [Back to Step 5](../VS_visualcpp/Adding-an-Event--ATL-Tutorial--Part-5-.md) &#124; [On to Step 7](../VS_visualcpp/Putting-the-Control-on-a-Web-Page--ATL-Tutorial--Part-7-.md)  
  
## See Also  
 [Tutorial](../VS_visualcpp/Active-Template-Library--ATL--Tutorial.md)
...
---
# CSharedFile Class
The [CMemFile](../VS_visualcpp/CMemFile-Class.md)-derived class that supports shared memory files.  
 ...
 **GlobalAlloc** returns an `HGLOBAL` handle rather than a pointer to memory, such as the pointer returned by [malloc](../VS_visualcpp/malloc.md). The `HGLOBAL` handle is needed in certain applications. For example, to put data on the Clipboard you need an `HGLOBAL` handle.  
 ...
 For more information, see the article [Files in MFC](../VS_visualcpp/Files-in-MFC.md) and [File Handling](../VS_visualcpp/File-Handling.md) in the                 *Run-Time Library Reference*.  

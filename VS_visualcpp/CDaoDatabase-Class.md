...
|[CDaoDatabase::m_pWorkspace](#cdaodatabase__m_pworkspace)|A pointer to the [CDaoWorkspace](../VS_visualcpp/CDaoWorkspace-Class.md) object that contains the database and defines its transaction space.|  
  
## Remarks  
 For information about the database formats supported, see the [GetName](../VS_visualcpp/CDaoWorkspace-Class.md#cdaoworkspace__getname) member function. You can have one or more `CDaoDatabase` objects active at a time in a given "workspace," represented by a [CDaoWorkspace](../VS_visualcpp/CDaoWorkspace-Class.md) object. The workspace maintains a collection of open database objects, called the Databases collection.  
  
> [!NOTE]
>  The MFC DAO database classes are distinct from the MFC database classes based on ODBC. All DAO database class names have the "CDao" prefix. Class `CDaoDatabase` supplies an interface similar to that of the ODBC class [CDatabase](../VS_visualcpp/CDatabase-Class.md). The main difference is that `CDatabase` accesses the DBMS through Open Database Connectivity (ODBC) and an ODBC driver for that DBMS. `CDaoDatabase` accesses data through a Data Access Object (DAO) based on the Microsoft Jet database engine. In general, the MFC classes based on DAO are more capable than the MFC classes based on ODBC; the DAO-based classes can access data, including through ODBC drivers, via their own database engine. The DAO-based classes also support Data Definition Language (DDL) operations, such as adding tables via the classes, without having to call DAO directly.  
...
-   Construct a `CDaoDatabase` object, passing a pointer to an open [CDaoWorkspace](../VS_visualcpp/CDaoWorkspace-Class.md) object.  
...
-   Or construct a `CDaoDatabase` object without specifying the workspace (MFC creates a temporary workspace object).  
 ...
 Any of these techniques appends the DAO database object to the workspace's Databases collection and opens a connection to the data. When you then construct [CDaoRecordset](../VS_visualcpp/CDaoRecordset-Class.md), [CDaoTableDef](../VS_visualcpp/CDaoTableDef-Class.md), or [CDaoQueryDef](../VS_visualcpp/CDaoQueryDef-Class.md) objects for operating on the connected database, pass the constructors for these objects a pointer to your `CDaoDatabase` object. When you finish using the connection, call the [Close](#cdaodatabase__close) member function and destroy the `CDaoDatabase` object. **Close** closes any recordsets you have not closed previously.  
  
## Transactions  
 Database transaction processing is supplied at the workspace level — see the [BeginTrans](../VS_visualcpp/CDaoWorkspace-Class.md#cdaoworkspace__begintrans), [CommitTrans](../VS_visualcpp/CDaoWorkspace-Class.md#cdaoworkspace__committrans), and [Rollback](../VS_visualcpp/CDaoWorkspace-Class.md#cdaoworkspace__rollback) member functions of class `CDaoWorkspace`.  
...
> [!NOTE]
>  A `CDaoDatabase` object is also created implicitly if you open a [CDaoRecordset](../VS_visualcpp/CDaoRecordset-Class.md) object without passing a pointer to an existing `CDaoDatabase` object. This database object is closed when you close the recordset object.  
...
### Remarks  
 It is good practice to close these objects yourself before you call this member function. Closing a `CDaoDatabase` object removes it from the Databases collection in the associated [workspace](../VS_visualcpp/CDaoWorkspace-Class.md). Because **Close** does not destroy the `CDaoDatabase` object, you can reuse the object by opening the same database or a different database.  
  
> [!CAUTION]
>  Call the [Update](../VS_visualcpp/CDaoRecordset-Class.md#cdaorecordset__update) member function (if there are pending edits) and the **Close** member function on all open recordset objects before you close a database. If you exit a function that declares [CDaoRecordset](../VS_visualcpp/CDaoRecordset-Class.md) or `CDaoDatabase` objects on the stack, the database is closed, any unsaved changes are lost, all pending transactions are rolled back, and any pending edits to your data are lost.  
...
### Parameters  
 ...
 `lpszTable`  
 The name of the primary table in the relation. If the table does not exist, MFC throws an exception of type [CDaoException](../VS_visualcpp/CDaoException-Class.md).  
 ...
 *relinfo*  
 A reference to a [CDaoRelationInfo](../VS_visualcpp/CDaoRelationInfo-Structure.md) object that contains information about the relation you want to create.  
...
### Remarks  
 ...
 For information about creating querydef objects, see class [CDaoQueryDef](../VS_visualcpp/CDaoQueryDef-Class.md). A querydef object becomes associated with a particular `CDaoDatabase` object when you construct the `CDaoQueryDef` object, passing it a pointer to the database object.  
...
>  Be very careful not to delete system tables.  
 ...
 For information about creating tabledef objects, see class [CDaoTableDef](../VS_visualcpp/CDaoTableDef-Class.md). A tabledef object becomes associated with a particular `CDaoDatabase` object when you construct the `CDaoTableDef` object, passing it a pointer to the database object.  
...
> [!TIP]
>  Given a syntactically correct SQL statement and proper permissions, the **Execute** member function will not fail even if not a single row can be modified or deleted. Therefore, always use the **dbFailOnError** option when using the **Execute** member function to run an update or delete query. This option causes MFC to throw an exception of type [CDaoException](../VS_visualcpp/CDaoException-Class.md) and rolls back all successful changes if any of the records affected are locked and cannot be updated or deleted. Note that you can always call `GetRecordsAffected` to see how many records were affected.  
...
### Return Value  
 The full path and file name of the database if successful; otherwise, an empty [CString](../VS_visualcpp/CStringT-Class.md).  
...
### Remarks  
 ...
 You might, for example, want to display this name in a heading. If an error occurs while the name is being retrieved, MFC throws an exception of type [CDaoException](../VS_visualcpp/CDaoException-Class.md).  
 ...
 *querydefinfo*  
 A reference to a [CDaoQueryDefInfo](../VS_visualcpp/CDaoQueryDefInfo-Structure.md) object that returns the information requested.  
...
### Remarks  
 An operation might time out due to network access problems, excessive query processing time, and so on. While the setting is in effect, it affects all open, add new, update, and delete operations on any recordsets associated with this `CDaoDatabase` object. You can change the current timeout setting by calling [SetQueryTimeout](#cdaodatabase__setquerytimeout). Changing the query timeout value for a recordset after opening does not change the value for the recordset. For example, subsequent [Move](../VS_visualcpp/CDaoRecordset-Class.md#cdaorecordset__move) operations do not use the new value. The default value is initially set when the database engine is initialized.  
  
 The default value for query timeouts is taken from the Windows registry. If there is no registry setting, the default is 60 seconds. Not all databases support the ability to set a query timeout value. If you set a query timeout value of 0, no timeout occurs; and communication with the database may stop responding. This behavior may be useful during development. If the call fails, MFC throws an exception of type [CDaoException](../VS_visualcpp/CDaoException-Class.md).  
 ...
 *relinfo*  
 A reference to a [CDaoRelationInfo](../VS_visualcpp/CDaoRelationInfo-Structure.md) object that returns the information requested.  
...
### Parameters  
 ...
 `tabledefinfo`  
 A reference to a [CDaoTableDefInfo](../VS_visualcpp/CDaoTableDefInfo-Structure.md) object that returns the information requested.  
...
### Return Value  
 A [CString](../VS_visualcpp/CStringT-Class.md) that indicates the version of the database file associated with the object.  
...
### Remarks  
 ...
 For information about calling DAO directly, see [Technical Note 54](../VS_visualcpp/TN054--Calling-DAO-Directly-While-Using-MFC-DAO-Classes.md).  
  
##  <a name="cdaodatabase__m_pworkspace"></a>  CDaoDatabase::m_pWorkspace  
 Contains a pointer to the [CDaoWorkspace](../VS_visualcpp/CDaoWorkspace-Class.md) object that contains the database object.  
...
-   Refers to a database that is already open for exclusive access by another user, MFC throws an exception of type [CDaoException](../VS_visualcpp/CDaoException-Class.md). Trap that exception to let your user know that the database is unavailable.  
...
>  For better performance when accessing external databases, including ISAM databases and ODBC data sources, it is recommended that you attach external database tables to a Microsoft Jet engine database (.MDB) rather than connecting directly to the data source.  
 ...
 It is possible for a connection attempt to time out if, for example, the DBMS host is unavailable. If the connection attempt fails, **Open** throws an exception of type [CDaoException](../VS_visualcpp/CDaoException-Class.md).  
...
### Remarks  
 An operation might time out because of network access problems, excessive query processing time, and so on. Call `SetQueryTimeout` before opening your recordset or before calling the recordset's [AddNew](../VS_visualcpp/CDaoRecordset-Class.md#cdaorecordset__addnew), [Update](../VS_visualcpp/CDaoRecordset-Class.md#cdaorecordset__update), or [Delete](../VS_visualcpp/CDaoRecordset-Class.md#cdaorecordset__delete) member functions if you want to change the query timeout value. The setting affects all subsequent [Open](../VS_visualcpp/CDaoRecordset-Class.md#cdaorecordset__open), `AddNew`, **Update**, and **Delete** calls to any recordsets associated with this `CDaoDatabase` object. Changing the query timeout value for a recordset after opening does not change the value for the recordset. For example, subsequent [Move](../VS_visualcpp/CDaoRecordset-Class.md#cdaorecordset__move) operations do not use the new value.  

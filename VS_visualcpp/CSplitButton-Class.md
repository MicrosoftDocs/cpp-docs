...
## Remarks  
 ...
 ![Dialog with a splitbutton and pager control.](../VS_visualcpp/media/SplitButton_Pager.png "SplitButton_Pager")  
 ...
 **Header:** afxcmn.h  
 ...
 Additional requirements for this class are described in [Build Requirements for Windows Vista Common Controls](../VS_visualcpp/Build-Requirements-for-Windows-Vista-Common-Controls.md).  
...
|---------------|-----------------|  
|[in] `dwStyle`|A bitwise combination (OR) of styles to be applied to the control. For more information, see [Button Styles](../VS_visualcpp/Button-Styles.md).|  
...
|[in] `pParentWnd`|A non-null pointer to a [CWnd](../VS_visualcpp/CWnd-Class.md) object that is the parent window of the control.|  
...
|[in] `pMenu`|A pointer to a [CMenu](../VS_visualcpp/CMenu-Class.md) object that specifies a submenu. The `CSplitButton` object deletes the `CMenu` object and its associated `HMENU` when the `CSplitButton` object goes out of scope.|  
...
### Remarks  
 ...
 To implement a custom action that the containing control supports, use a [CButton](../VS_visualcpp/CButton-Class.md) object with a style of `BS_SPLITBUTTON` instead of a `CSplitButton` object. Then implement a handler for the `BCN_DROPDOWN` notification in the `CButton` object. For more information, see [Button Styles](../VS_visualcpp/Button-Styles.md).  
  
 To implement a custom action that the split button control itself supports, use [message reflection](../VS_visualcpp/TN062--Message-Reflection-for-Windows-Controls.md). Derive your own class from the `CSplitButton` class and name it, for example, CMySplitButton. Then add the following message map to your application to handle the `BCN_DROPDOWN` notification:  
...
|[in] `nMenuId`|The resource ID of the menu bar.|  
|[in] `nSubMenuId`|The resource ID of a submenu.|  
|[in] `pMenu`|Pointer to a [CMenu](../VS_visualcpp/CMenu-Class.md) object that specifies a submenu. The `CSplitButton` object deletes the `CMenu` object and its associated `HMENU` when the `CSplitButton` object goes out of scope.|  

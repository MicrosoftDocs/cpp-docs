...
```  
 ...
 In the preceding example, the static objects `sd1` and `sd2` are created and initialized before entry to `main`. After this program terminates using the `return` statement, first `sd2` is destroyed and then `sd1`. The destructor for the `ShowData` class closes the files associated with these static objects. (For more information about initialization, constructors, and destructors, see [Special Member Functions](../Topic/Special%20Member%20Functions%20\(C++\).md).)  

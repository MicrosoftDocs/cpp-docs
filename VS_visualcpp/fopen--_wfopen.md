...
---
# fopen, _wfopen
Opens a file. More-secure versions of these functions that perform additional parameter validation and return error codes are available; see [fopen_s, _wfopen_s](../VS_visualcpp/fopen_s--_wfopen_s.md).  
...
## Return Value  
 Each of these functions returns a pointer to the open file. A null pointer value indicates an error. If `filename` or `mode` is `NULL` or an empty string, these functions trigger the invalid parameter handler, which is described in [Parameter Validation](../VS_visualcpp/Parameter-Validation.md). If execution is allowed to continue, these functions return `NULL` and set `errno` to `EINVAL`.  
...
## Remarks  
 ...
 Always check the return value to see whether the pointer is NULL before you perform any additional operations on the file. If an error occurs, the global variable `errno` is set and may be used to obtain specific error information. For more information, see [errno, _doserrno, _sys_errlist, and _sys_nerr](../VS_visualcpp/errno--_doserrno--_sys_errlist--and-_sys_nerr.md).  
...
|`_tfopen`|`fopen`|`fopen`|`_wfopen`|  
 ...
 For more information about how to use text and binary modes in Unicode and multibyte stream-I/O, see [Text and Binary Mode File I/O](../VS_visualcpp/Text-and-Binary-Mode-File-I-O.md) and [Unicode Stream I/O in Text and Binary Modes](../VS_visualcpp/Unicode-Stream-I-O-in-Text-and-Binary-Modes.md).  
 ...
 Valid characters for the `mode` string that is used in `fopen` and `_fdopen` correspond to `oflag` arguments that are used in [_open](../VS_visualcpp/_open--_wopen.md) and [_sopen](../VS_visualcpp/_sopen--_wsopen.md), as follows.  
...
|`_wfopen`|<stdio.h> or <wchar.h>|  
 ...
 `_wfopen` is a Microsoft extension. For more information about compatibility, see [Compatibility](../VS_visualcpp/Compatibility.md).  

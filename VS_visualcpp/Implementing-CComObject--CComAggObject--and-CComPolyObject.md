...
The template classes [CComObject](../VS_visualcpp/CComObject-Class.md), [CComAggObject](../VS_visualcpp/CComAggObject-Class.md), and [CComPolyObject](../VS_visualcpp/CComPolyObject-Class.md) are always the most derived classes in the inheritance chain. It is their responsibility to handle all of the methods in **IUnknown**: `QueryInterface`, `AddRef`, and **Release**. In addition, `CComAggObject` and `CComPolyObject` (when used for aggregated objects) provide the special reference counting and `QueryInterface` semantics required for the inner unknown.  

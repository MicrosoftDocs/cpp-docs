...
This note describes the MFC Support for custom and self-drawing controls. It also describes dynamic subclassing, and describes the relationship between [CWnd](../VS_visualcpp/CWnd-Class.md) objects and `HWND`s.  
  
 The MFC sample application CTRLTEST illustrates how to use many custom controls. See the source code for the MFC General sample [CTRLTEST](../VS_visualcpp/Visual-C---Samples.md) and online help.  
...
## Self-Draw Controls and Menus  
 MFC provides a default implementation (in the `CWnd` and [CMenu](../VS_visualcpp/CMenu-Class.md) classes) for the standard owner-draw messages. This default implementation will decode the owner-draw parameters and delegate the owner-draw messages to the controls or menu. This is called self-draw because the drawing code is in the class of the control or menu, not in the owner window.  
...
-   For self-draw combo boxes:  
 ...
 For details on the owner-draw structures ([DRAWITEMSTRUCT](../VS_visualcpp/DRAWITEMSTRUCT-Structure.md), [MEASUREITEMSTRUCT](../VS_visualcpp/MEASUREITEMSTRUCT-Structure.md), [COMPAREITEMSTRUCT](../VS_visualcpp/COMPAREITEMSTRUCT-Structure.md), and [DELETEITEMSTRUCT](../VS_visualcpp/DELETEITEMSTRUCT-Structure.md)) see the MFC documentation for `CWnd::OnDrawItem`, `CWnd::OnMeasureItem`, `CWnd::OnCompareItem`, and `CWnd::OnDeleteItem` respectively.  
  
## Using self-draw controls and menus  
 For self-draw menus, you must override both the `OnMeasureItem` and `OnDrawItem` methods.  
  
 For self-draw list boxes and combo boxes, you must override `OnMeasureItem` and `OnDrawItem`. You must specify the `LBS_OWNERDRAWVARIABLE` style for list boxes or `CBS_OWNERDRAWVARIABLE` style for combo boxes in the dialog template. The `OWNERDRAWFIXED` style will not work with self-draw items because the fixed item height is determined before self-draw controls are attached to the list box. (You can use the methods [CListBox::SetItemHeight](../Topic/CListBox::SetItemHeight.md) and [CComboBox::SetItemHeight](../Topic/CComboBox::SetItemHeight.md) to overcome this limitation.)  
...
## Examples of Self-Drawing Controls and Menus  
 The MFC General sample [CTRLTEST](../VS_visualcpp/Visual-C---Samples.md) provides samples of a self-draw menu and a self-draw list box.  
  
 The most typical example of a self-drawing button is a bitmap button. A bitmap button is a button that shows one, two, or three bitmap images for the different states. An example of this is provided in the MFC class [CBitmapButton](../VS_visualcpp/CBitmapButton-Class.md).  
...
## Dynamic Subclassing  
 ...
 Subclassing is the Windows term for replacing the [WndProc](assetId:///94ba8ffa-3c36-46d4-ac74-9bd10b1ffd26) of a window with a customized `WndProc` and calling the old `WndProc` for default functionality.  
...
-   `CWnd` creates the `HWND`. You can modify the behavior in a derived class by creating a class derived from `CWnd`. The `HWND` is created when your application calls [CWnd::Create](../Topic/CWnd::Create.md).  
  
-   The application attaches a `CWnd` to an existing `HWND`. The behavior of the existing window is not modified. This is a case of delegation and is made possible by calling [CWnd::Attach](../Topic/CWnd::Attach.md) to alias an existing `HWND` to a `CWnd` object.  
...
-   `CWnd` is attached to an existing `HWND` and you can modify the behavior in a derived class. This is called dynamic subclassing because we are changing the behavior, and therefore the class, of a Windows object at run time.  
 ...
 You can achieve dynamic subclassing by using the methods [CWnd::SubclassWindow](../Topic/CWnd::SubclassWindow.md) and[CWnd::SubclassDlgItem](../Topic/CWnd::SubclassDlgItem.md).  
 ...
 See the [CTRLTEST](../VS_visualcpp/Visual-C---Samples.md) example for several examples of when to use `SubclassWindow` and `SubclassDlgItem`.  

...
## Remarks  
 ...
 initializes the controlled sequence with the sequence `[``right``.`[list::begin (STL/CLR)](../VS_visualcpp/list--begin--STL-CLR-.md)`(),` `right``.`[list::end (STL/CLR)](../VS_visualcpp/list--end--STL-CLR-.md)`())`. You use it to specify an initial controlled sequence that is a copy of the sequence controlled by the list object `right`.  
  
 The constructor:  
  
 `list(list<Value>^ right);`  
  
 initializes the controlled sequence with the sequence `[``right``->`[list::begin (STL/CLR)](../VS_visualcpp/list--begin--STL-CLR-.md)`(),` `right``->`[list::end (STL/CLR)](../VS_visualcpp/list--end--STL-CLR-.md)`())`. You use it to specify an initial controlled sequence that is a copy of the sequence controlled by the list object whose handle is `right`.  

...
> [!NOTE]
>  Microsoft C++ Component Extensions provides support for the `new` keyword to add vtable slot entries. For more information, see [new (new slot in vtable)](../VS_visualcpp/new--new-slot-in-vtable----C---Component-Extensions-.md)  
...
## Remarks  
 If unsuccessful, **new** returns zero or throws an exception; see [The new and delete Operators](../VS_visualcpp/new-and-delete-Operators.md) for more information. You can change this default behavior by writing a custom exception-handling routine and calling the [_set_new_handler](../VS_visualcpp/_set_new_handler.md) run-time library function with your function name as its argument.  
  
 For information on how to create an object on the managed heap, see [gcnew](../VS_visualcpp/ref-new--gcnew---C---Component-Extensions-.md).  
  
 When **new** is used to allocate memory for a C++ class object, the object's constructor is called after the memory is allocated.  
  
 Use the [delete](../VS_visualcpp/delete-Operator--C---.md) operator to deallocate the memory allocated with the **new** operator.  
...
```  
 ...
 If you use the operator **new** without any extra arguments, and compile with the [/GX](../VS_visualcpp/-GX--Enable-Exception-Handling-.md), [/EHa](../VS_visualcpp/-EH--Exception-Handling-Model-.md), or [/EHs](../VS_visualcpp/-EH--Exception-Handling-Model-.md) option, the compiler will generate code to call operator **delete** if the constructor throws an exception.  
...
-   The class has a default constructor (a constructor that can be called with no arguments).  
 ...
 No explicit per-element initialization can be done when allocating arrays using the **new** operator; only the default constructor, if present, is called. See [Default Arguments](../VS_visualcpp/Default-Arguments.md) for more information.  
...
>  The argument to `operator new` is of type **size_t**. This type is defined in DIRECT.H, MALLOC.H, MEMORY.H, SEARCH.H, STDDEF.H, STDIO.H, STDLIB.H, STRING.H, and TIME.H.  
 ...
 An option in the grammar allows specification of *placement* (see the Grammar for [new Operator](../VS_visualcpp/new-Operator--C---.md)). The *placement* parameter can be used only for user-defined implementations of `operator new`; it allows extra information to be passed to `operator new`. An expression with a *placement* field such as `T *TObject = new ( 0x0040 ) T;` is translated to `T *TObject = T::operator new( sizeof( T ), 0x0040 );` if class T has member operator new, otherwise to `T *TObject = ::operator new( sizeof( T ), 0x0040 );`.  
...
## See Also  
 ...
 [operator new Function](../Topic/operator%20new%20Function.md)
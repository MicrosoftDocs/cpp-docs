...
## Sorting arrays by using custom criteria  
 ...
 In the following example, a structure named `Element` is derived from <xref:System.IComparable?qualifyHint=False>, and written to provide a <xref:System.IComparable.CompareTo?qualifyHint=False> method that uses the average of two integers as the sort criterion.  

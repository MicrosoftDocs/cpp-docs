...
```  
 ...
 A range-based `for` loop terminates when one of these in `statement` is executed: a [break](../VS_visualcpp/break-Statement--C---.md), [return](../VS_visualcpp/return-Statement--C---.md), or [goto](../VS_visualcpp/goto-Statement--C---.md) to a labeled statement outside the range-based **for** loop. A [continue](../VS_visualcpp/continue-Statement--C---.md) statement in a range-based `for` loop terminates only the current iteration.  

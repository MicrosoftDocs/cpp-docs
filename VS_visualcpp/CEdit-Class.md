...
## Remarks  
 ...
 `CEdit` inherits significant functionality from `CWnd`. To set and retrieve text from a `CEdit` object, use the `CWnd` member functions [SetWindowText](../VS_visualcpp/CWnd-Class.md#cwnd__setwindowtext) and [GetWindowText](../VS_visualcpp/CWnd-Class.md#cwnd__getwindowtext), which set or get the entire contents of an edit control, even if it is a multiline control. Text lines in a multiline control are separated by '\r\n' character sequences. Also, if an edit control is multiline, get and set part of the control's text by calling the `CEdit` member functions [GetLine](#cedit__getline), [SetSel](#cedit__setsel), [GetSel](#cedit__getsel), and [ReplaceSel](#cedit__replacesel).  
...
-   **ON_EN_VSCROLL** The user clicks an edit control's vertical scroll bar. The parent window is notified before the screen is updated.  
 ...
 To modify certain styles in an edit control (such as **ES_READONLY**) you must send specific messages to the control instead of using [ModifyStyle](../VS_visualcpp/CWnd-Class.md#cwnd__modifystyle). See                 [Edit Control Styles](http://msdn.microsoft.com/library/windows/desktop/bb775464) in the Windows SDK.  
...
### Parameters  
 `dwStyle`  
 Specifies the edit control's style. Apply any combination of [edit styles](../VS_visualcpp/Edit-Styles.md) to the control.  
...
### Remarks  
 ...
 These messages are handled by default by the [OnNcCreate](../VS_visualcpp/CWnd-Class.md#cwnd__onnccreate), [OnNcCalcSize](../VS_visualcpp/CWnd-Class.md#cwnd__onnccalcsize), [OnCreate](../VS_visualcpp/CWnd-Class.md#cwnd__oncreate), and [OnGetMinMaxInfo](../VS_visualcpp/CWnd-Class.md#cwnd__ongetminmaxinfo) member functions in the `CWnd` base class. To extend the default message handling, derive a class from `CEdit`, add a message map to the new class, and override the above message-handler member functions. Override `OnCreate`, for example, to perform needed initialization for the new class.  
  
 Apply the following [window styles](../VS_visualcpp/Window-Styles.md) to an edit control.  
...
### Remarks  
 ...
 The undo flag is automatically cleared whenever the [SetWindowText](../VS_visualcpp/CWnd-Class.md#cwnd__setwindowtext) or [SetHandle](#cedit__sethandle)`CWnd` member functions are called.  
...
### Return Value  
 ...
 For the second overload, a [CString](../VS_visualcpp/Using-CString.md) that contains the cue text if the method is successful; otherwise, the empty string ("").  
...
### Example  
  See the example for [CEditView::GetEditCtrl](../VS_visualcpp/CEditView-Class.md#ceditview__geteditctrl).  
...
### Remarks  
 Replaces only a portion of the text in an edit control. If you want to replace all of the text, use the [CWnd::SetWindowText](../VS_visualcpp/CWnd-Class.md#cwnd__setwindowtext) member function.  
...
### Remarks  
 ...
 If the edit control is created with the [ES_PASSWORD](../VS_visualcpp/Edit-Styles.md) style, the default password character is set to an asterisk ( **\***). This style is removed if `SetPasswordChar` is called with                         *ch* set to 0.  
...
### Remarks  
 The current setting can be found by testing the [ES_READONLY](../VS_visualcpp/Edit-Styles.md) flag in the return value of [CWnd::GetStyle](../VS_visualcpp/CWnd-Class.md#cwnd__getstyle).  
...
### Remarks  
 ...
 `SetTabStops` does not automatically redraw the edit window. If you change the tab stops for text already in the edit control, call [CWnd::InvalidateRect](../VS_visualcpp/CWnd-Class.md#cwnd__invalidaterect) to redraw the edit window.  
...
### Example  
  See the example for [CEditView::SetTabStops](../VS_visualcpp/CEditView-Class.md#ceditview__settabstops).  

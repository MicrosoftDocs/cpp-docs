...
-   The **Help** menu contains menu items for both the Active document container and server.  
 ...
 For more information on menu merging and Active document activation, see Overview of [Active Document Containment](../VS_visualcpp/Active-Document-Containment.md).  
...
### Parameters  
 `pCaller`  
 A pointer to a [CView](../VS_visualcpp/CView-Class.md) object that is sending the print command.  
  
 `pInfo`  
 A pointer to a [CPrintInfo](../VS_visualcpp/CPrintInfo-Structure.md) object that describes the job to be printed.  
...
### Parameters  
 `dwCloseOption`  
 Flag specifying under what circumstances the OLE item is saved when it returns to the loaded state. For a list of possible values, see [COleClientItem::Close](../VS_visualcpp/COleClientItem-Class.md#coleclientitem__close).  

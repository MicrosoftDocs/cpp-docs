...
##  <a name="_core_using_database_classes_in_activex_controls"></a> Using Database Classes in ActiveX Controls  
 ...
 For a general overview of the MFC database classes, see [MFC Database Classes (DAO and ODBC)](../VS_visualcpp/MFC-Database-Classes--ODBC-and-DAO-.md). The article introduces both the MFC ODBC classes and the MFC DAO classes and directs you to more details on either.  
  
> [!NOTE]
>  As of Visual C++ .NET, the Visual C++ environment and wizards no longer support DAO (although the DAO classes are included and you can still use them). Microsoft recommends that you use [OLE DB Templates](../VS_visualcpp/OLE-DB-Programming.md) or [ODBC and MFC](../VS_visualcpp/ODBC-and-MFC.md) for new projects. You should only use DAO in maintaining existing applications.  

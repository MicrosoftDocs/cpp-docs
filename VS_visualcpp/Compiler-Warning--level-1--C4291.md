...
'declaration' : no matching operator delete found; memory will not be freed if initialization throws an exception  
 ...
 A placement [new](../VS_visualcpp/new-Operator--C---.md) is used for which there is no placement [delete](../VS_visualcpp/delete-Operator--C---.md).  
 ...
 If you use the operator **new** without any extra arguments and compile with [/GX](../VS_visualcpp/-GX--Enable-Exception-Handling-.md), [/EHs](../VS_visualcpp/-EH--Exception-Handling-Model-.md), or /EHa options to enable exception handling, the compiler will generate code to call operator **delete** if the constructor throws an exception.  

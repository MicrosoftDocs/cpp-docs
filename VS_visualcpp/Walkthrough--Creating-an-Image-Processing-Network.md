...
-   [Walkthrough: Creating a Dataflow Agent](../VS_visualcpp/Walkthrough--Creating-a-Dataflow-Agent.md)  
 ...
 We also recommend that you understand the basics of [!INCLUDE[ndptecgdiplus](../Token/ndptecgdiplus_md.md)] before you start this walkthrough. For more information about [!INCLUDE[ndptecgdiplus](../Token/ndptecgdiplus_md.md)], see [GDI+](_gdiplus_GDI_start_cpp).  
...
##  <a name="functionality"></a> Defining Image Processing Functionality  
 ...
 The following function, `GetColorDominance`, also calls the `ProcessImage` function. However, instead of changing the value of each color, this function uses [concurrency::combinable](../VS_visualcpp/combinable-Class.md) objects to compute whether the red, green, or blue color component dominates the image.  
...
##  <a name="network"></a> Creating the Image Processing Network  
 This section describes how to create a network of asynchronous message blocks that perform image processing on every [!INCLUDE[TLA#tla_jpeg](../Token/TLA%23tla_jpeg_md.md)] (.jpg) image in a given directory. The network performs the following image-processing operations:  
...
3.  For any other image, apply sepia toning.  
 ...
 The following steps show how to create a function that implements this image processing network and applies that network to every [!INCLUDE[TLA#tla_jpeg](../Token/TLA%23tla_jpeg_md.md)] image in a given directory.  
...
3.  Create a [std::map](../VS_visualcpp/map-Class.md) object that associates a `Bitmap` object with its original file name.  
...
6.  Add the following code to send to the head of the network the full path of each [!INCLUDE[TLA#tla_jpeg](../Token/TLA%23tla_jpeg_md.md)] file in the directory.  
...
|Member|Description|  
|------------|-----------------|  
|`load_bitmap`|A [concurrency::transformer](../VS_visualcpp/transformer-Class.md) object that loads a `Bitmap` object from disk and adds an entry to the `map` object to associate the image with its original file name.|  
...
|`decrement`|A [concurrency::call](../VS_visualcpp/call-Class.md) object that acts as the terminal node in the network. It decrements the `countdown_event` object to signal to the main application that an image has been processed.|  
 ...
 ![Image processing network](../VS_visualcpp/media/ConcRT_ImageProc.png "ConcRT_ImageProc")  
  
 The `countdown_event` object in this example enables the image processing network to inform the main application when all images have been processed. The `countdown_event` class uses a [concurrency::event](../VS_visualcpp/event-Class.md) object to signal when a counter value reaches zero. The main application increments the counter every time that it sends a file name to the network. The terminal node of the network decrements the counter after each image has been processed. After the main application traverses the specified directory, it waits for the `countdown_event` object to signal that its counter has reached zero.  
...
##  <a name="complete"></a> The Complete Example  
 The following code shows the complete example. The `wmain` function manages the [!INCLUDE[ndptecgdiplus](../Token/ndptecgdiplus_md.md)] library and calls the `ProcessImages` function to process the [!INCLUDE[TLA#tla_jpeg](../Token/TLA%23tla_jpeg_md.md)] files in the `Sample Pictures` directory.  
 ...
 ![Sample output for the example](../VS_visualcpp/media/ConcRT_ImageOut.png "ConcRT_ImageOut")  

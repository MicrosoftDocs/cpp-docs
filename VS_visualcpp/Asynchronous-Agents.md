...
## Agent Life Cycle  
 ...
 ![Agent State Diagram](../VS_visualcpp/media/AgentState.png "AgentState")  
...
## Methods and Features  
 The following table shows some of the important methods that belong to the `agent` class. For more information about all of the `agent` class methods, see [agent Class](../VS_visualcpp/agent-Class.md).  
...
|[wait_for_one](../Topic/agent::wait_for_one%20Method.md)|Waits for at least one of the provided `agent` objects to enter the `agent_done` or `agent_canceled` state.|  
 ...
 The runtime does not manage exceptions that are thrown by asynchronous agents. For more information about exception handling and agents, see [Exception Handling](../VS_visualcpp/Exception-Handling-in-the-Concurrency-Runtime.md).  
  
## Example  
 For an example that shows how to create a basic agent-based application, see [Walkthrough: Creating an Agent-Based Application](../VS_visualcpp/Walkthrough--Creating-an-Agent-Based-Application.md).  

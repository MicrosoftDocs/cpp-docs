...
## Remarks  
 This class is a wrapper for the **SAFEARRAYBOUND** structure used by [CComSafeArray](../VS_visualcpp/CComSafeArray-Class.md). It provides methods for querying and setting the upper and lower bounds of a single dimension of a `CComSafeArray` object and the number of elements it contains. A multidimensional `CComSafeArray` object uses an array of `CComSafeArrayBound` objects, one for each dimension. Therefore, when using methods such as [GetCount](../Topic/CComSafeArrayBound::GetCount.md), be aware that this method will not return the total number of elements in a multidimensional array.  
...
CComSafeArrayBound &  operator = (  
    const CComSafeArrayBound & bound ) throw();  

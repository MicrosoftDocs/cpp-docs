...
## Remarks  
 The member function returns a pair of iterators `cliext::pair<iterator, iterator>(` [map::lower_bound (STL/CLR)](../VS_visualcpp/map--lower_bound--STL-CLR-.md)`(``key``),` [map::upper_bound (STL/CLR)](../VS_visualcpp/map--upper_bound--STL-CLR-.md)`(``key``))`. You use it to determine the range of elements currently in the controlled sequence that match a specified key.  

...
Used with `BEGIN_COLUMN_MAP` and `END_COLUMN_MAP` to bind a binary large object ([BLOB](https://msdn.microsoft.com/en-us/library/ms711511.aspx)). Similar to [BLOB_NAME](../VS_visualcpp/BLOB_NAME.md), except that this macro also gets the length and status of the BLOB data column.  

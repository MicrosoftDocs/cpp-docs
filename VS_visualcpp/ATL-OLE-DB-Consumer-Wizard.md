...
-   **All** tab  
    ...
    3.  **Select the database on the server:** Click the drop-down list menu to display all registered databases on the data server, and select one.  
         ...
         **Use data source name:** This drop-down list displays data sources registered in your machine. You can set up data sources ahead of time using the [ODBC Data Source Administrator](!Alink("dasdkODBCDataSourceAdmin")).-or-**Use connection string:** Either enter a connection string you have already obtained, or click the **Build** button; the **Select Data Source** dialog box appears. Select a file or machine data source and click **OK**.  
...
-   If you select a consumer **Type** of Command, the wizard uses the `db_source` and **db_command** attributes, and uses **db_column** to create the column map, for example:  
 ...
 See [Basic Mechanics of Attributes](../VS_visualcpp/Basic-Mechanics-of-Attributes.md) for more information.  
 ...
 **Support**  
 Select the check boxes to specify the kinds of updates to be supported in the consumer (the default is none). Each of the following will set [DBPROP_IRowsetChange](https://msdn.microsoft.com/en-us/library/ms715892.aspx) and the appropriate entries for [DBPROP_UPDATABILITY](https://msdn.microsoft.com/en-us/library/ms722676.aspx) in the property set map.  
...
## See Also  
 [ATL OLE DB Consumer](../VS_visualcpp/Adding-an-ATL-OLE-DB-Consumer.md)   

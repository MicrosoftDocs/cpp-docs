...
## Remarks  
 ...
 If you use **/EHs** or **/EHsc**, then your `catch(...)` clause does not catch asynchronous structured exceptions. Access violations and managed <xref:System.Exception?qualifyHint=True> exceptions are not caught, and objects in scope when an asynchronous exception is generated are not destroyed even if the asynchronous exception is handled.  
...
```  
 ...
 For information about exception handling restrictions under **/clr**, see [_set_se_translator](../VS_visualcpp/_set_se_translator.md).  
...
-   See <xref:Microsoft.VisualStudio.VCProjectEngine.VCCLCompilerTool.ExceptionHandling?qualifyHint=False>.  

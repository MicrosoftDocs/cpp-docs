...
```  
 ...
 The left-align flag is set by using the [setiosflags](../Topic/setiosflags.md) manipulator with the [left](../Topic/left.md) enumerator. This enumerator is defined in the [ios](../Topic/ios.md) class, so its reference must include the **ios::** prefix. The [resetiosflags](../Topic/resetiosflags.md) manipulator turns off the left-align flag. Unlike **width** and `setw`, the effect of `setiosflags` and `resetiosflags` is permanent.  
  
##  <a name="vclrfprecisionanchor5"></a> Precision  
 The default value for floating-point precision is six. For example, the number 3466.9768 prints as 3466.98. To change the way this value prints, use the [setprecision](../Topic/setprecision.md) manipulator. The manipulator has two flags: [fixed](../Topic/fixed.md) and [scientific](../Topic/scientific.md). If [fixed](../Topic/fixed.md) is set, the number prints as 3466.976800. If **scientific** is set, it prints as 3.4669773+003.  
...
```  
 ...
 The delimiters are present only in the stream object; they are not present in the extracted string but they are present in the string returned by [basic_stringstream::str](../Topic/basic_stringstream::str.md)().  
 ...
 For more information and full code examples, see [quoted](../Topic/quoted.md).  

...
You can create Visual C++ programs that target the Windows Runtime and build them on the command line. Visual C++ supports Visual C++ component extensions (C++/CX), which has additional types and operators to target the Windows Runtime programming model. You can use C++/CX to build apps for Windows Phone 8.1, Windows Store, and Windows desktop. For more information, see [A Tour of C++/CX](http://msdn.microsoft.com/magazine/dn166929.aspx) and [Component Extensions for Runtime Platforms](../VS_visualcpp/Component-Extensions-for-Runtime-Platforms.md).  
...
## Compiling a C++/CX Program  
 To enable compilation for C++/CX, you must use the [/ZW](../VS_visualcpp/-ZW--Windows-Runtime-Compilation-.md) compiler option. The Visual C++ compiler generates an .exe file that targets the Windows Runtime, and links to the required libraries.  
...
5.  At the command prompt, enter **cl /EHsc /ZW basiccx.cpp /link /SUBSYSTEM:CONSOLE**. The cl.exe compiler compiles the source code into an .obj file, and then runs the linker to generate an executable program named basiccx.exe. (The [/EHsc](../VS_visualcpp/-EH--Exception-Handling-Model-.md) compiler option specifies the C++ exception-handling model, and the [/link](../VS_visualcpp/-link--Pass-Options-to-Linker-.md) flag specifies a console application.)  

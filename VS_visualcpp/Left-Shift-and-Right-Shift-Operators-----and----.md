...
## Left Shifts  
 ...
 The following example shows left-shift operations using unsigned numbers. The example shows what is happening to the bits by representing the value as a bitset. For more information, see [bitset Class](../VS_visualcpp/bitset-Class.md).  
...
## Shifts and Promotions  
 The expressions on both sides of a shift operator must be integral types. Integral promotions are performed according to the rules described in [Integral Promotions](../Topic/Integral%20Promotions.md). The type of the result is the same as the type of the promoted `shift-expression`.  

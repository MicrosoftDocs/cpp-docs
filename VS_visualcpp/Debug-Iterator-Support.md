...
-   Increasing the size of a [vector](../VS_visualcpp/-vector-.md) (push or insert) causes iterators into the `vector` to become invalid.  
...
## Example  
 You can use the symbol [_HAS_ITERATOR_DEBUGGING](../VS_visualcpp/_HAS_ITERATOR_DEBUGGING.md) to turn off the iterator debugging feature in a debug build. The following program does not assert, but still triggers undefined behavior.  

...
> [!NOTE]
>  MFC does not use the [Run-Time Type Information](../VS_visualcpp/Run-Time-Type-Information.md) (RTTI) support introduced in Visual C++ 4.0.  
  
 If you have derived your class from [CObject](../VS_visualcpp/CObject-Class.md) and used the **DECLARE**_**DYNAMIC** and `IMPLEMENT_DYNAMIC`, the `DECLARE_DYNCREATE` and `IMPLEMENT_DYNCREATE`, or the `DECLARE_SERIAL` and `IMPLEMENT_SERIAL` macros explained in the article [Deriving a Class from CObject](../VS_visualcpp/Deriving-a-Class-from-CObject.md), the `CObject` class has the ability to determine the exact class of an object at run time.  
...
2.  Call the `IsKindOf` member function for objects of that class, using the `RUNTIME_CLASS` macro to generate the `CRuntimeClass` argument, as shown here:  
 ...
 One use of run-time class information is in the dynamic creation of objects. This process is discussed in the article [Dynamic Object Creation](../VS_visualcpp/Dynamic-Object-Creation.md).  
  
 For more detailed information on serialization and run-time class information, see the articles [Files in MFC](../VS_visualcpp/Files-in-MFC.md) and [Serialization](../VS_visualcpp/Serialization-in-MFC.md).  

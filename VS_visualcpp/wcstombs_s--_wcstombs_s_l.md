...
Converts a sequence of wide characters to a corresponding sequence of multibyte characters. A version of [wcstombs, _wcstombs_l](../VS_visualcpp/wcstombs--_wcstombs_l.md) with security enhancements as described in [Security Features in the CRT](../VS_visualcpp/Security-Features-in-the-CRT.md).  
...
#### Parameters  
 ...
 [in] `count`  
 The maximum number of bytes to store in the `mbstr` buffer, not including the terminating null character, or [_TRUNCATE](../VS_visualcpp/_TRUNCATE.md).  
...
-   The number of bytes stored in the `mbstr` buffer equals `count`.  
 ...
 If `count` is the special value [_TRUNCATE](../VS_visualcpp/_TRUNCATE.md), then `wcstombs_s` converts as much of the string as will fit into the destination buffer, while still leaving room for a null terminator.  
...
>  Ensure that `wcstr` and `mbstr` do not overlap, and that `count` correctly reflects the number of wide characters to convert.  
 ...
 `wcstombs_s` uses the current locale for any locale-dependent behavior; `_wcstombs_s_l` is identical to `wcstombs` except that it uses the locale passed in instead. For more information, see [Locale](../VS_visualcpp/Locale.md).  

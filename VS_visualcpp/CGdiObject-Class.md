...
## Remarks  
 ...
 For more information on `CGdiObject`, see [Graphic Objects](../VS_visualcpp/Graphic-Objects.md).  
...
|Object|Buffer type|  
|------------|-----------------|  
|`CPen`|[LOGPEN](../VS_visualcpp/LOGPEN-Structure.md)|  
|`CBrush`|[LOGBRUSH](../VS_visualcpp/LOGBRUSH-Structure.md)|  
|`CFont`|[LOGFONT](http://msdn.microsoft.com/library/windows/desktop/dd145037)|  
|`CBitmap`|[BITMAP](../VS_visualcpp/BITMAP-Structure.md)|  
|`CPalette`|**WORD**|  
|`CRgn`|Not supported|  
  
 If the object is a `CBitmap` object, `GetObject` returns only the width, height, and color format information of the bitmap. The actual bits can be retrieved by using [CBitmap::GetBitmapBits](../VS_visualcpp/CBitmap-Class.md#cbitmap__getbitmapbits).  
  
 If the object is a `CPalette` object, `GetObject` retrieves a **WORD** that specifies the number of entries in the palette. The function does not retrieve the                         [LOGPALETTE](http://msdn.microsoft.com/library/windows/desktop/dd145040) structure that defines the palette. An application can get information on palette entries by calling [CPalette::GetPaletteEntries](../VS_visualcpp/CPalette-Class.md#cpalette__getpaletteentries).  
...
### Example  
  See the example for [CWnd::IsWindowEnabled](../VS_visualcpp/CWnd-Class.md#cwnd__iswindowenabled).  

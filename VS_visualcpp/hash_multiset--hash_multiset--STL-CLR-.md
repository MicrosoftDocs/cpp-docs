...
## Remarks  
 ...
 initializes the controlled sequence with the sequence `[``right``.`[hash_multiset::begin (STL/CLR)](../VS_visualcpp/hash_multiset--begin--STL-CLR-.md)`(),` `right``.`[hash_multiset::end (STL/CLR)](../VS_visualcpp/hash_multiset--end--STL-CLR-.md)`())`, with the default ordering predicate, and with the default hash function. You use it to specify an initial controlled sequence that is a copy of the sequence controlled by the hash_multiset object `right`, with the default ordering predicate and hash function.  

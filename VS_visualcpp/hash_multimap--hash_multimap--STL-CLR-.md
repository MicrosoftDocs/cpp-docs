...
ms.tgt_pltfrm: na
ms.topic: reference
H1: hash_multimap::hash_multimap (STL/CLR)
...
## Remarks  
 ...
 initializes the controlled sequence with the sequence `[``right``.`[hash_multimap::begin (STL/CLR)](../VS_visualcpp/hash_multimap--begin--STL-CLR-.md)`(),` `right``.`[hash_multimap::end (STL/CLR)](../VS_visualcpp/hash_multimap--end--STL-CLR-.md)`())`, with the default ordering predicate, and with the default hash function. You use it to specify an initial controlled sequence that is a copy of the sequence controlled by the hash_multimap object `right`, with the default ordering predicate and hash function.  

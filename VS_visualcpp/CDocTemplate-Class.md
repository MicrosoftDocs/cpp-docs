...
-   A frame window class, which contains the view. For a single document interface (SDI) application, you derive this class from `CFrameWnd`. For a multiple document interface (MDI) application, you derive this class from `CMDIChildWnd`. If you don't need to customize the behavior of the frame window, you can use `CFrameWnd` or `CMDIChildWnd` directly without deriving your own class.  
 ...
 For more information on `CDocTemplate`, see [Document Templates and the Document/View Creation Process](../VS_visualcpp/Document-Templates-and-the-Document-View-Creation-Process.md).  
...
### Remarks  
 The derived classes [CMultiDocTemplate](../VS_visualcpp/CMultiDocTemplate-Class.md) and [CSingleDocTemplate](../VS_visualcpp/CSingleDocTemplate-Class.md) override this function. If you derive your own document-template class from `CDocTemplate`, your derived class must override this function.  
...
### Remarks  
 Use this member function to construct a `CDocTemplate` object. Dynamically allocate a `CDocTemplate` object and pass it to [CWinApp::AddDocTemplate](../VS_visualcpp/CWinApp-Class.md#cwinapp__adddoctemplate) from the `InitInstance` member function of your application class.  
...
### Remarks  
 ...
 [CSingleDocTemplate](../VS_visualcpp/CSingleDocTemplate-Class.md) and [CMultiDocTemplate](../VS_visualcpp/CMultiDocTemplate-Class.md) both override this pure virtual function. Any class you derive from `CDocTemplate` must also override this function.  
...
### Remarks  
 This member function is called by the framework to load the resources for a given `CDocTemplate` or derived class. Normally it is called during construction, except when the template is being constructed globally. In that case, the call to `LoadTemplate` is delayed until [CWinApp::AddDocTemplate](../VS_visualcpp/CWinApp-Class.md#cwinapp__adddoctemplate) is called.  
...
### Remarks  
 Call this function to set the resources to be used when an OLE object is in-place activated. These resources may include menus and accelerator tables. This function is usually called in the [CWinApp::InitInstance](../VS_visualcpp/CWinApp-Class.md#cwinapp__initinstance) function of your application.  
  
 The menu associated with `nIDOleInPlaceContainer` contains separators that allow the menu of the activated in-place item to merge with the menu of the container application. For more information about merging server and container menus, see the article [Menus and Resources (OLE)](../VS_visualcpp/Menus-and-Resources--OLE-.md).  
 ...
 *pOleFrameClass*  
 Pointer to a [CRuntimeClass](../VS_visualcpp/CRuntimeClass-Structure.md) structure containing class information for the frame window object created when in-place activation occurs.  
...
### Remarks  
 ...
 The menu associated with `nIDOleInPlaceServer` contains separators that allow the server menu to merge with the menu of the container. For more information about merging server and container menus, see the article [Menus and Resources (OLE)](../VS_visualcpp/Menus-and-Resources--OLE-.md).  

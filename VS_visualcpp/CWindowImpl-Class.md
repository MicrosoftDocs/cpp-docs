 ...
 *TBase*  
 The base class of your class. By default, the base class is [CWindow](../VS_visualcpp/CWindow-Class.md).  
  
 `TWinTraits`  
 A [traits class](../VS_visualcpp/Understanding-Window-Traits.md) that defines styles for your window. The default is `CControlWinTraits`.  
...
|||  
|-|-|  
|[GetWndClassInfo](../Topic/CWindowImpl::GetWndClassInfo.md)|Returns a static instance of [CWndClassInfo](../VS_visualcpp/CWndClassInfo-Class.md), which manages the window class information.|  
...
## Remarks  
 ...
 `CWindowImpl::Create` creates a window based on the window class information that's managed by [CWndClassInfo](../VS_visualcpp/CWndClassInfo-Class.md). `CWindowImpl` contains the [DECLARE_WND_CLASS](../Topic/DECLARE_WND_CLASS.md) macro, which means `CWndClassInfo` registers a new window class. If you want to superclass an existing window class, derive your class from `CWindowImpl` and include the [DECLARE_WND_SUPERCLASS](../Topic/DECLARE_WND_SUPERCLASS.md) macro. In this case, `CWndClassInfo` registers a window class that's based on an existing class but uses `CWindowImpl::WindowProc`. For example:  
...
>  For any given `CWindowImpl` object, call either **Create** or `SubclassWindow`. Don't invoke both methods on the same object.  
 ...
 In addition to `CWindowImpl`, ATL provides [CContainedWindow](../VS_visualcpp/CContainedWindowT-Class.md) to create a window that's contained in another object.  
  
 The base class destructor (~                **CWindowImplRoot**) ensures that the window is gone before the object is destroyed.  
  
 `CWindowImpl` derives from **CWindowImplBaseT**, which derives from **CWindowImplRoot**, which derives from **TBase** and [CMessageMap](../VS_visualcpp/CMessageMap-Class.md).  
  
|For more information about|See|  
|--------------------------------|---------|  
|Creating controls|[ATL Tutorial](../VS_visualcpp/Active-Template-Library--ATL--Tutorial.md)|  
|Using windows in ATL|[ATL Window Classes](../VS_visualcpp/ATL-Window-Classes.md)|  
|ATL Project Wizard|[Creating an ATL Project](../VS_visualcpp/Creating-an-ATL-Project.md)|  
  
## Inheritance Hierarchy  
 [CMessageMap](../VS_visualcpp/CMessageMap-Class.md)  
...
### Return Value  
 A static instance of [CWndClassInfo](../VS_visualcpp/CWndClassInfo-Class.md).  
  
### Remarks  
 By default, `CWindowImpl` obtains this method through the [DECLARE_WND_CLASS](../Topic/DECLARE_WND_CLASS.md) macro, which specifies a new window class.  
  
 To superclass an existing window class, derive your class from `CWindowImpl` and include the [DECLARE_WND_SUPERCLASS](../Topic/DECLARE_WND_SUPERCLASS.md) macro to override `GetWndClassInfo`. For more information, see the [CWindowImpl](../VS_visualcpp/CWindowImpl-Class.md) overview.  
...
|--------------------|----------------------|  
|A window based on a new window class, specified through the [DECLARE_WND_CLASS](../Topic/DECLARE_WND_CLASS.md) macro.|The [DefWindowProc](http://msdn.microsoft.com/library/windows/desktop/ms633572) Win32 function.|  

...
---
# -arch (ARM)
Specifies the architecture for code generation on ARM. See also [/arch (x86)](../VS_visualcpp/-arch--x86-.md) and [/arch (x64)](../VS_visualcpp/-arch--x64-.md).  
...
## Remarks  
 The `_M_ARM_FP` macro (for ARM only) indicates which, if any, **/arch** compiler option was used. For more information, see [Predefined Macros](../VS_visualcpp/Predefined-Macros.md).  
  
 When you use [/clr](../VS_visualcpp/-clr--Common-Language-Runtime-Compilation-.md) to compile, **/arch** has no effect on code generation for managed functions. **/arch** only affects code generation for native functions.  

...
## Remarks  
 ...
 [CFrameWnd::GetDockState](../VS_visualcpp/CFrameWnd-Class.md#cframewnd__getdockstate) retrieves the state information of all the frame window's `CControlBar` objects and puts it into the `CDockState` object. You can then write the contents of the `CDockState` object to storage with [Serialize](../VS_visualcpp/CObject-Class.md#cobject__serialize) or [CDockState::SaveState](#cdockstate__savestate). If you later want to restore the state of the control bars in the frame window, you can load the state with `Serialize` or [CDockState::LoadState](#cdockstate__loadstate), then use [CFrameWnd::SetDockState](../VS_visualcpp/CFrameWnd-Class.md#cframewnd__setdockstate) to apply the saved state to the frame window's control bars.  
  
 For more information on docking control bars, see the articles [Control Bars](../VS_visualcpp/Control-Bars.md), [Toolbars: Docking and Floating](../VS_visualcpp/Docking-and-Floating-Toolbars.md), and [Frame Windows](../VS_visualcpp/Frame-Windows.md).  

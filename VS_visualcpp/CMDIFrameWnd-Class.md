...
## Remarks  
 ...
 You can construct an MDI frame window by calling the [Create](../VS_visualcpp/CFrameWnd-Class.md#cframewnd__create) or [LoadFrame](../VS_visualcpp/CFrameWnd-Class.md#cframewnd__loadframe) member function of `CFrameWnd`.  
 ...
 The `CMDIFrameWnd` class inherits much of its default implementation from `CFrameWnd`. For a detailed list of these features, refer to the [CFrameWnd](../VS_visualcpp/CFrameWnd-Class.md) class description. The `CMDIFrameWnd` class has the following additional features:  
...
-   An MDI frame window also has an implementation of **ID_WINDOW_NEW**, which creates a new frame and view on the current document. An application can override these default command implementations to customize MDI window handling.  
 ...
 For more information on `CMDIFrameWnd`, see [Frame Windows](../VS_visualcpp/Frame-Windows.md).  
...
### Parameters  
 `lpCreateStruct`  
 A long pointer to a [CREATESTRUCT](../VS_visualcpp/CREATESTRUCT-Structure.md) structure.  
...
### Remarks  
 This member function sends the [WM_MDIACTIVATE](../VS_visualcpp/CWnd-Class.md#cwnd__onmdiactivate) message to both the child window being activated and the child window being deactivated.  
...
> [!NOTE]
>  An MDI child window is activated independently of the MDI frame window. When the frame becomes active, the child window that was last activated is sent a [WM_NCACTIVATE](../VS_visualcpp/CWnd-Class.md#cwnd__onncactivate) message to draw an active window frame and caption bar, but it does not receive another `WM_MDIACTIVATE` message.  
...
### Example  
 See the example for [CMDIChildWnd::MDIMaximize](../VS_visualcpp/CMDIChildWnd-Class.md#cmdichildwnd__mdimaximize).  
...
### Example  
 See the example for [CMDIChildWnd::MDIRestore](../VS_visualcpp/CMDIChildWnd-Class.md#cmdichildwnd__mdirestore).  
...
### Remarks  
 After calling `MDISetMenu`, an application must call the [DrawMenuBar](../VS_visualcpp/CWnd-Class.md#cwnd__drawmenubar) member function of `CWnd` to update the menu bar.  

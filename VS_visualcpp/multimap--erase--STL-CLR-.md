...
## Remarks  
 The first member function removes the element of the controlled sequence pointed to by `where`, and returns an iterator that designates the first element remaining beyond the element removed, or [multimap::end (STL/CLR)](../VS_visualcpp/multimap--end--STL-CLR-.md)`()` if no such element exists. You use it to remove a single element.  

...
```  
 ...
 You do not need to extract the underlying data type from the [CDBVariant Class](../VS_visualcpp/CDBVariant-Class.md) object. Assign the value with `GetBookmark` and return to that bookmark with `SetBookmark`.  
...
> [!NOTE]
>  Absolute positioning is not available on forward-only recordsets. For more information about forward-only recordsets, see [Recordset (ODBC)](../VS_visualcpp/Recordset--ODBC-.md).  
...
>  The absolute position of a record is potentially unreliable. If the user deletes records from the recordset, the ordinal position of any subsequent record changes. Bookmarks are the recommended method for moving the current record. For more information, see [Bookmarks in MFC ODBC](#_core_bookmarks_in_mfc_odbc).  
 ...
 For more information about recordset navigation, see [Recordset: Scrolling (ODBC)](../VS_visualcpp/Recordset--Scrolling--ODBC-.md).  

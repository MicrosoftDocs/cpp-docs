...
---
# wcsrtombs_s
Convert a wide character string to its multibyte character string representation. A version of [wcsrtombs](../VS_visualcpp/wcsrtombs.md) with security enhancements as described in [Security Features in the CRT](../VS_visualcpp/Security-Features-in-the-CRT.md).  
...
#### Parameters  
 ...
 [in] `count`  
 The maximum number of bytes to be stored in the `mbstr` buffer, or [_TRUNCATE](../VS_visualcpp/_TRUNCATE.md).  
...
-   The number of bytes stored in the `mbstr` buffer equals `count`.  
 ...
 If `count` is the special value [_TRUNCATE](../VS_visualcpp/_TRUNCATE.md), then `wcsrtombs_s` converts as much of the string as will fit into the destination buffer, while still leaving room for a null terminator.  
...
>  Ensure that `wcstr` and `mbstr` do not overlap, and that `count` correctly reflects the number of wide characters to convert.  
 ...
 The `wcsrtombs_s` function differs from [wcstombs_s, _wcstombs_s_l](../VS_visualcpp/wcstombs_s--_wcstombs_s_l.md) by its restartability. The conversion state is stored in `mbstate` for subsequent calls to the same or other restartable functions. Results are undefined when mixing the use of restartable and nonrestartable functions. For example, an application would use `wcsrlen` rather than `wcslen`, if a subsequent call to `wcsrtombs_s` were used instead of `wcstombs_s.`  

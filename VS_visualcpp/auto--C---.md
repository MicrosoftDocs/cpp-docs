...
-   Expression template helper types—for example, `(valarray+valarray)`.  
 ...
 The initialization expression can be an assignment (equal-sign syntax), a direct initialization (function-style syntax), an [operator new](../Topic/operator%20new%20\(%3Cnew%3E\).md) expression, or the initialization expression can be the *for-range-declaration* parameter in a [Range-based for Statement (C++)](../VS_visualcpp/Range-based-for-Statement--C---.md) statement. For more information, see [Initializers](../VS_visualcpp/Initializers.md) and the code examples later in this document.  
  
 The `auto` keyword is a placeholder for a type, but it is not itself a type. Therefore, the `auto` keyword cannot be used in casts or operators such as [sizeof](../VS_visualcpp/sizeof-Operator.md) and [typeid](../VS_visualcpp/typeid---C---Component-Extensions-.md).  
  
## Usefulness  
 The `auto` keyword is a simple way to declare a variable that has a complicated type. For example, you can use `auto` to declare a variable where the initialization expression involves templates, pointers to functions, or pointers to members.  
  
 You can also use `auto` to declare and initialize a variable to a lambda expression. You can't declare the type of the variable yourself because the type of a lambda expression is known only to the compiler. For more information, see [Examples of Lambda Expressions](../VS_visualcpp/Examples-of-Lambda-Expressions.md).  
  
## Trailing Return Types  
 You can use `auto`, together with the `decltype` type specifier, to help write template libraries. Use `auto` and `decltype` to declare a template function whose return type depends on the types of its template arguments. Or, use `auto` and `decltype` to declare a template function that wraps a call to another function, and then returns whatever is the return type of that other function. For more information, see [decltype](../VS_visualcpp/decltype---C---.md).  
...
|------------------|-----------------|  
|[C3530](../VS_visualcpp/Compiler-Error-C3530.md)|The `auto` keyword cannot be combined with any other type-specifier.|  
|[C3531](../VS_visualcpp/Compiler-Error-C3531.md)|A symbol that is declared with the `auto` keyword must have an initializer.|  
|[C3532](../VS_visualcpp/Compiler-Error-C3532.md)|You incorrectly used the `auto` keyword to declare a type. For example, you declared a method return type or an array.|  
|[C3533](../VS_visualcpp/Compiler-Error-C3533.md), [C3539](../VS_visualcpp/Compiler-Error-C3539.md)|A parameter or template argument cannot be declared with the `auto` keyword.|  
...
|[C3535](../VS_visualcpp/Compiler-Error-C3535.md)|A method or template parameter cannot be declared with the `auto` keyword.|  
|[C3536](../VS_visualcpp/Compiler-Error-C3536.md)|A symbol cannot be used before it is initialized. In practice, this means that a variable cannot be used to initialize itself.|  
|[C3537](../VS_visualcpp/Compiler-Error-C3537.md)|You cannot cast to a type that is declared with the `auto` keyword.|  
|[C3538](../VS_visualcpp/Compiler-Error-C3538.md)|All the symbols in a declarator list that is declared with the `auto` keyword must resolve to the same type. For more information, see [Declarations](../Topic/Declarations.md).|  
|[C3540](../VS_visualcpp/Compiler-Error-C3540.md), [C3541](../VS_visualcpp/Compiler-Error-C3541.md)|The [sizeof](../VS_visualcpp/sizeof-Operator.md) and [typeid](../VS_visualcpp/typeid---C---Component-Extensions-.md) operators cannot be applied to a symbol that is declared with the `auto` keyword.|  
...
## See Also  
 ...
 [decltype](../VS_visualcpp/decltype---C---.md)
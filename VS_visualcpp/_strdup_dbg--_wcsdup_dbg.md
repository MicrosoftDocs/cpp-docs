...
Versions of [_strdup and _wcsdup](../VS_visualcpp/_strdup--_wcsdup--_mbsdup.md) that use the debug version of `malloc`.  
...
## Remarks  
 The `_strdup_dbg` and `_wcsdup_dbg` functions are identical to `_strdup` and `_wcsdup` except that, when `_DEBUG` is defined, these functions use the debug version of `malloc`, `_malloc_dbg`, to allocate memory for the duplicated string. For information on the debugging features of `_malloc_dbg`, see [_malloc_dbg](../VS_visualcpp/_malloc_dbg.md).  
...
## Libraries  
 All debug versions of the [C run-time libraries](../VS_visualcpp/CRT-Library-Features.md).  

...
## Remarks  
 ...
 You can construct a `CMFCColorDialog` object on the stack and then call `DoModal`, passing the initial color as a parameter to the `CMFCColorDialog` constructor. The color selection dialog box then creates several [CMFCColorPickerCtrl Class](../VS_visualcpp/CMFCColorPickerCtrl-Class.md) objects to handle each color palette.  

...
##  <a name="critical_section"></a> critical_section  
 The [concurrency::critical_section](../VS_visualcpp/critical_section-Class.md) class represents a cooperative mutual exclusion object that yields to other tasks instead of preempting them. Critical sections are useful when multiple threads require exclusive read and write access to shared data.  
  
 The `critical_section` class is non-reentrant. The [concurrency::critical_section::lock](../Topic/critical_section::lock%20Method.md) method throws an exception of type [concurrency::improper_lock](../VS_visualcpp/improper_lock-Class.md) if it is called by the thread that already owns the lock.  
...
##  <a name="reader_writer_lock"></a> reader_writer_lock  
 ...
 Like the `critical_section` class, the `reader_writer_lock` class is non-reentrant. The [concurrency::reader_writer_lock::lock](../VS_visualcpp/reader_writer_lock--lock-Method.md) and [concurrency::reader_writer_lock::lock_read](../VS_visualcpp/reader_writer_lock--lock_read-Method.md) methods throw an exception of type `improper_lock` if they are called by a thread that already owns the lock.  
...
|Method|Description|  
|------------|-----------------|  
|[lock](../VS_visualcpp/reader_writer_lock--lock-Method.md)|Acquires read/write access to the lock.|  
|[try_lock](../VS_visualcpp/reader_writer_lock--try_lock-Method.md)|Tries to acquire read/write access to the lock, but does not block.|  
|[lock_read](../VS_visualcpp/reader_writer_lock--lock_read-Method.md)|Acquires read-only access to the lock.|  
|[try_lock_read](../VS_visualcpp/reader_writer_lock--try_lock_read-Method.md)|Tries to acquire read-only access to the lock, but does not block.|  
|[unlock](../VS_visualcpp/reader_writer_lock--unlock-Method.md)|Releases the lock.|  
...
##  <a name="scoped_lock"></a> scoped_lock and scoped_lock_read  
 ...
 The `critical_section` class contains the [concurrency::critical_section::scoped_lock](../Topic/critical_section::scoped_lock%20Class.md) class. The constructor acquires access to the provided `critical_section` object; the destructor releases access to that object. The `reader_writer_lock` class contains the [concurrency::reader_writer_lock::scoped_lock](../VS_visualcpp/reader_writer_lock--scoped_lock-Class.md) class, which resembles `critical_section::scoped_lock`, except that it manages write access to the provided `reader_writer_lock` object. The `reader_writer_lock` class also contains the [concurrency::reader_writer_lock::scoped_lock_read](../VS_visualcpp/reader_writer_lock--scoped_lock_read-Class.md) class. This class manages read access to the provided `reader_writer_lock` object.  
...
##  <a name="event"></a> event  
 The [concurrency::event](../VS_visualcpp/event-Class.md) class represents a synchronization object whose state can be signaled or non-signaled. Unlike synchronization objects, such as critical sections, whose purpose is to protect access to shared data, events synchronize flow of execution.  
...
### Example  
 For an example that shows how to use the `event` class, see [Comparing Synchronization Data Structures to the Windows API](../VS_visualcpp/Comparing-Synchronization-Data-Structures-to-the-Windows-API.md).  

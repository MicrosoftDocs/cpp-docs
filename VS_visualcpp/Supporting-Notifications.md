...
## Implementing Connection Point Interfaces on the Provider and Consumer  
 To implement notifications, a provider class must inherit from [IRowsetNotifyCP](../VS_visualcpp/IRowsetNotifyCP-Class.md) and [IConnectionPointContainer](../VS_visualcpp/IConnectionPointContainerImpl-Class.md).  
 ...
 Note that you must also implement and register `IRowsetNotify` on the consumer (also known as the sink) using [IRowsetNotifyImpl](../VS_visualcpp/IRowsetNotifyImpl-Class.md) so that the consumer can handle notifications. For information about implementing the connection point interface on the consumer, see [Receiving Notifications](../VS_visualcpp/Receiving-Notifications.md).  

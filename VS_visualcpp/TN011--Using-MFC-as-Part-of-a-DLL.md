...
## WinMain -> DllMain  
 The MFC library defines the standard Win32 `DllMain` entry point that initializes your [CWinApp](../VS_visualcpp/CWinApp-Class.md) derived object as in a typical MFC application. Place all DLL-specific initialization in the [InitInstance](../Topic/CWinApp::InitInstance.md) method as in a typical MFC application.  
  
 Note that the [CWinApp::Run](../Topic/CWinApp::Run.md) mechanism does not apply to a DLL, because the application owns the main message pump. If your DLL displays modeless dialogs or has a main frame window of its own, your application's main message pump must call a DLL-exported routine that calls [CWinApp::PreTranslateMessage](../Topic/CWinApp::PreTranslateMessage.md).  
  
 See the DLLScreenCap sample for use of this function.  
  
 The `DllMain` function that MFC provides will call the [CWinApp::ExitInstance](../Topic/CWinApp::ExitInstance.md) method of your class that is derived from `CWinApp` before the DLL is unloaded.  

...
---
# CMemFile Class
The [CFile](../VS_visualcpp/CFile-Class.md)-derived class that supports memory files.  
...
## Remarks  
 ...
 The most common use of `CMemFile` is to create a `CMemFile` object and use it by calling [CFile](../VS_visualcpp/CFile-Class.md) member functions. Note that creating a `CMemFile` automatically opens it: you do not call [CFile::Open](../VS_visualcpp/CFile-Class.md#cfile__open), which is only used for disk files. Because `CMemFile` doesn't use a disk file, the data member `CFile::m_hFile` is not used and has no meaning.  
  
 The `CFile` member functions [Duplicate](../VS_visualcpp/CFile-Class.md#cfile__duplicate), [LockRange](../VS_visualcpp/CFile-Class.md#cfile__lockrange), and [UnlockRange](../VS_visualcpp/CFile-Class.md#cfile__unlockrange) are not implemented for `CMemFile`. If you call these functions on a `CMemFile` object, you will get a [CNotSupportedException](../VS_visualcpp/CNotSupportedException-Class.md).  
  
 `CMemFile` uses the run-time library functions [malloc](../VS_visualcpp/malloc.md), [realloc](../VS_visualcpp/realloc.md), and [free](../VS_visualcpp/free.md) to allocate, reallocate, and deallocate memory; and the intrinsic [memcpy](../VS_visualcpp/memcpy--wmemcpy.md) to block copy memory when reading and writing. If you'd like to change this behavior or the behavior when `CMemFile` grows a file, derive your own class from `CMemFile` and override the appropriate functions.  
  
 For more information on `CMemFile`, see the articles [Files in MFC](../VS_visualcpp/Files-in-MFC.md) and [Memory Management (MFC)](../VS_visualcpp/Memory-Management.md) and see [File Handling](../VS_visualcpp/File-Handling.md) in the                 *Run-Time Library Reference*.  
...
### Remarks  
 ...
 The default implementation uses the run-time library function [malloc](../VS_visualcpp/malloc.md) to allocate memory.  
...
### Remarks  
 Note that the file is opened by the constructor and that you should not call [CFile::Open](../VS_visualcpp/CFile-Class.md#cfile__open).  
...
##  <a name="cmemfile__memcpy"></a>  CMemFile::Memcpy  
 This function is called by the `CMemFile` overrides of [CFile::Read](../VS_visualcpp/CFile-Class.md#cfile__read) and [CFile::Write](../VS_visualcpp/CFile-Class.md#cfile__write) to transfer data to and from the memory file.  

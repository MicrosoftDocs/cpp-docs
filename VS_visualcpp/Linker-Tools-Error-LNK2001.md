...
unresolved external symbol "symbol"  
 ...
 This error message is followed by fatal error [LNK1120](../VS_visualcpp/Linker-Tools-Error-LNK1120.md).  
...
-   When upgrading a managed library or web service project from Visual C++ 2003, the **/Zl** compiler option is added to the **Command Line** property page. This will cause LNK2001.  
     ...
     To resolve this error, either add msvcrt.lib and msvcmrt.lib to the linker's Additional Dependencies property. Or, remove **/Zl** from the **Command Line** property page. For more information, see [/Zl (Omit Default Library Name)](../VS_visualcpp/-Zl--Omit-Default-Library-Name-.md) and [How to: Open Project Property Pages](../Topic/How%20to:%20Open%20Project%20Property%20Pages.md).  
...
-   If the LNK2001 diagnostic text reports that `__check_commonlanguageruntime_version` is an unresolved external symbol, see [LNK2019](../VS_visualcpp/Linker-Tools-Error-LNK2019.md) for information on how to resolve.  
...
-   A project that uses [function inlining](../VS_visualcpp/Function-Inlining-Problems.md) yet defines the functions in a .cpp file rather than in the header file can cause LNK2001.  
  
-   Calling a C function from a C++ program without using `extern` "C" (which causes the compiler to use the C naming convention) can cause LNK2001. Compiler options [/Tp](../VS_visualcpp/-Tc---Tp---TC---TP--Specify-Source-File-Type-.md) and [/Tc](../VS_visualcpp/-Tc---Tp---TC---TP--Specify-Source-File-Type-.md) cause the compiler to compile files as C++ or C, respectively, regardless of the filename extension. These options can cause function names different from what you expect.  
...
-   A [missing function body or variable](../VS_visualcpp/Missing-Function-Body-or-Variable.md) can cause LNK2001. With just a function prototype or `extern` declaration the compiler can continue without error, but the linker cannot resolve a call to an address or reference to a variable because there is no function code or variable space reserved.  
  
-   Calling a function with parameter types that do not match those in the function declaration can cause LNK2001. [Name decoration](../VS_visualcpp/Name-Decoration.md) incorporates the parameters of a function into the final decorated function name.  
...
-   When using C++, including a function prototype in a class definition and failing to [include the implementation](../VS_visualcpp/Missing-Function-Body-or-Variable.md) of the function for that class can cause LNK2001.  
...
-   Trying to use a variable declared within a function ([a local variable](../VS_visualcpp/Automatic--Function-Scope--Variables.md)) outside the scope of that function can cause LNK2001.  
...
-   When building a Release version of an ATL project, indicates that CRT startup code is required. To fix, do one of the following,  
    ...
    -   Remove `_ATL_MIN_CRT` from the list of preprocessor defines to allow CRT startup code to be included. See [General Configuration Settings Property Page](../VS_visualcpp/General-Property-Page--Project-.md) for more information.  
...
-   The project is missing a reference to a library (.LIB) or object (.OBJ) file. See [.lib Files as Linker Input](../VS_visualcpp/.Lib-Files-as-Linker-Input.md) for more information.  
  
-   If you use [/NODEFAULTLIB](../VS_visualcpp/-NODEFAULTLIB--Ignore-Libraries-.md) or [/Zl](../VS_visualcpp/-Zl--Omit-Default-Library-Name-.md), libraries containing required code will not be linked into the project unless you have explicitly included them. (When compiling with **/clr** or **/clr:pure**, you will see a reference to .cctor; see [Initialization of Mixed Assemblies](../VS_visualcpp/Initialization-of-Mixed-Assemblies.md) for more information.)  
  
-   If you are using Unicode and MFC, you will get an unresolved external on `_WinMain@16` if you don't create an entrypoint to `wWinMainCRTStartup`; use the [/ENTRY](../VS_visualcpp/-ENTRY--Entry-Point-Symbol-.md). See [Unicode Programming Summary](../VS_visualcpp/Unicode-Programming-Summary.md).  
...
-   Linking code requiring the multithreaded libraries (any MFC code or code compiled with [/MT](../VS_visualcpp/-MD---MT---LD--Use-Run-Time-Library-.md)) causes LNK2001 on [_beginthread](../VS_visualcpp/_beginthread--_beginthreadex.md), `_beginthreadex`, [_endthread](../VS_visualcpp/_endthread--_endthreadex.md), and `_endthreadex`. See the following Knowledge Base article for more information:  
...
-   There is currently no standard for [C++ naming](../VS_visualcpp/Name-Decoration.md) between compiler vendors or even between different versions of a compiler. Therefore, linking object files compiled with other compilers may not produce the same naming scheme and thus cause error LNK2001.  
  
-   [Mixing inline and non-inline compile options](../VS_visualcpp/Function-Inlining-Problems.md) on different modules can cause LNK2001. If a C++ library is created with function inlining turned on (**/Ob1** or **/Ob2**) but the corresponding header file describing the functions has inlining turned off (no `inline` keyword), you will get this error. To prevent this problem, have the inline functions defined with `inline` in the header file you are going to include in other files.  
  
-   If you are using the `#pragma inline_depth` compiler directive, make sure you have a [value of 2 or greater set](../VS_visualcpp/Function-Inlining-Problems.md), and make sure you are using the [/Ob1](../VS_visualcpp/-Ob--Inline-Function-Expansion-.md) or [/Ob2](../VS_visualcpp/-Ob--Inline-Function-Expansion-.md) compiler option.  
  
-   Omitting the LINK option /NOENTRY when creating a resource-only DLL will cause LNK2001.  
  
-   Using incorrect /SUBSYSTEM or /ENTRY settings can cause LNK2001. For example, if you write a character-based application (a console application) and specify /SUBSYSTEM:WINDOWS, you will get an unresolved external for `WinMain`. For more information on these options and entry points, see the [/SUBSYSTEM](../VS_visualcpp/-SUBSYSTEM--Specify-Subsystem-.md) and [/ENTRY](../VS_visualcpp/-ENTRY--Entry-Point-Symbol-.md) linker options.  
  
 **Export Problems**  
  
-   When you are porting an application from 16 to 32 or 64 bits, LNK2001 can occur. The current module-definition (.def) file syntax requires that `__cdecl`, `__stdcall`, and `__fastcall` functions be listed in the EXPORTS section without underscores (undecorated). This differs from the 16-bit syntax, where they must be listed with underscores (decorated). For more information, see the description of the [EXPORTS](../VS_visualcpp/EXPORTS.md) section of module-definition files.  
 ...
 **Interpreting the Output**  
 ...
 For more information on some of the causes listed above, see [Name decoration](../VS_visualcpp/Name-Decoration.md).
...
## Remarks  
 `CComApartment` is used by [CComAutoThreadModule](../VS_visualcpp/CComAutoThreadModule-Class.md) to manage an apartment in a thread-pooled EXE module. `CComApartment` provides methods for incrementing and decrementing the lock count on a thread.  

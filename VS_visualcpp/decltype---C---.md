...
---
# decltype  (C++)
The `decltype` type specifier yields the type of a specified expression. The `decltype` type specifier, together with the [auto keyword](../VS_visualcpp/auto--C---.md), is useful primarily to developers who write template libraries. Use `auto` and `decltype` to declare a template function whose return type depends on the types of its template arguments. Or, use `auto` and `decltype` to declare a template function that wraps a call to another function, and then returns the return type of the wrapped function.  
...
|---------------|-----------------|  
|`expression`|An expression. For more information, see [Expressions](../VS_visualcpp/Expressions--C---.md).|  
...
-   If the `expression` parameter is an identifier or a [class member access](../VS_visualcpp/Member-Access-Operators--.-and---.md), `decltype(``expression``)` is the type of the entity named by `expression`. If there is no such entity or the `expression` parameter names a set of overloaded functions, the compiler yields an error message.  
...
-   If the `expression` parameter is an [rvalue](../VS_visualcpp/Lvalues-and-Rvalues--Visual-C---.md), `decltype(``expression``)` is the type of `expression`. If the `expression` parameter is an [lvalue](../VS_visualcpp/Lvalues-and-Rvalues--Visual-C---.md), `decltype(``expression``)` is an [lvalue reference](../VS_visualcpp/Lvalue-Reference-Declarator---.md) to the type of `expression`.  
...
|---------------|----------|-----------|  
|`decltype(fx());`|`const int&&`|An [rvalue reference](../VS_visualcpp/Rvalue-Reference-Declarator----.md) to a `const int`.|  

...
#### Parameters  
 *errnum*  
 A number between zero and one less than the value returned by [_RTC_NumErrors](../VS_visualcpp/_RTC_NumErrors.md).  
  
 *ErrType*  
 A value to assign to this *errnum*. For example, you might use **_CRT_ERROR**. If you are using `_CrtDbgReport` as your error handler, *ErrType* can only be one of the symbols defined in [_CrtSetReportMode](../VS_visualcpp/_CrtSetReportMode.md). If you have your own error handler ([_RTC_SetErrorFunc](../VS_visualcpp/_RTC_SetErrorFunc.md)), you can have as many *ErrType*s as there are *errnum*s.  
...
## Remarks  
 By default, all errors are set to *ErrType* = 1, which corresponds to **_CRT_ERROR**. For more information about the default error types such as **_CRT_ERROR**, see [_CrtDbgReport](../VS_visualcpp/_CrtDbgReport--_CrtDbgReportW.md).  

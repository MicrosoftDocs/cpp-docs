...
## Remarks  
 The method returns a pair of iterators `-` [multimap::lower_bound (STL/CLR)](../VS_visualcpp/multimap--lower_bound--STL-CLR-.md)`(``_Keyval``),` [multimap::upper_bound (STL/CLR)](../VS_visualcpp/multimap--upper_bound--STL-CLR-.md)`(``_Keyval``)`. You use it to determine the range of elements currently in the controlled sequence that match a specified key.  

...
### Remarks  
 [invalid_argument](../VS_visualcpp/invalid_argument-Class.md) is thrown if the parameter `nodeCount` has the value `0` was passed in, or if the parameter `pCoreCount` has the value `NULL`.  
  
 [invalid_operation](../VS_visualcpp/invalid_operation-Class.md) is thrown if this method is called while other schedulers exist in the process.  

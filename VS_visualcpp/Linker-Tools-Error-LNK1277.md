...
object record not found in pgd (filename)  
 ...
 When using [/LTCG:PGOPTIMZE](../VS_visualcpp/-LTCG--Link-time-Code-Generation-.md), the path of one of the input .lib, def, or .obj files was different from the path on which they were found during /LTCG:PGINSTRUMENT. This may be explained by a change in the LIB environment variable after /LTCG:PGINSTRUMENT. The full path to the input files is stored in the .pgd file.  

...
After you [insert a control](../VS_visualcpp/Inserting-the-Control-into-a-Visual-C---Application.md) and generate one or more [wrapper classes](../VS_visualcpp/Wrapper-Classes.md), you can invoke the control's methods and program the control's event handlers.  
  
 The control's [wrapper classes](../VS_visualcpp/Wrapper-Classes.md) specify the functions you can use to modify the run-time behavior of the control. Include the appropriate wrapper class header file and use the methods. To set a property, look for an accessor method with the property name prefixed by Set. To retrieve a property, look for an accessor method with the property name prefixed by Get. Event handlers can be written later.  
  
 Because the controls are implemented using Automation, the types passed can only be Automation-safe types such as BSTR and VARIANT. While you can use system calls to allocate and set BSTRs and VARIANTs, you might want to use the ATL wrapper classes ([CComBSTR](../VS_visualcpp/CComBSTR-Class.md), [CComVariant](../VS_visualcpp/CComVariant-Class.md)), the Visual C++ COM Compiler support wrapper classes ([_bstr_t](../VS_visualcpp/_bstr_t-Class.md), [_variant_t](../VS_visualcpp/_variant_t-Class.md)), or the MFC wrapper class ([COleVariant](../VS_visualcpp/COleVariant-Class.md)).  
 ...
 If you want to use ADO and RDO directly, you should connect to the ADO or RDO DLLs directly (Msado15.dll or Msrdo20.dll), either with the [compiler COM support classes](../VS_visualcpp/Compiler-COM-Support-Classes.md), which support the [#import directive](../VS_visualcpp/Preprocessor-Directives.md), or with the respective SDK.  

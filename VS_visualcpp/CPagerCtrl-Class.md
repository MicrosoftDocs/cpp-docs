...
## Remarks  
 ...
 The `CPagerCtrl` class is derived from the [CWnd](../VS_visualcpp/CWnd-Class.md) class. For more information and an illustration of a pager control, see                 [Pager Controls](http://msdn.microsoft.com/library/windows/desktop/bb760855).  
...
|---------------|-----------------|  
|[in] `dwStyle`|A bitwise combination (OR) of [window styles](../VS_visualcpp/Window-Styles.md) and                                         [pager control styles](http://msdn.microsoft.com/library/windows/desktop/bb760859) to be applied to the control.|  
...
|[in] `pParentWnd`|A pointer to a [CWnd](../VS_visualcpp/CWnd-Class.md) object that is the parent window of the control. This parameter cannot be `NULL`.|  
...
### Example  
 The following example uses [message reflection](../VS_visualcpp/TN062--Message-Reflection-for-Windows-Controls.md) to enable the pager control to recalculate its own size instead of requiring the control's parent dialog to perform the calculation. The example derives the `MyPagerCtrl` class from the [CPagerCtrl class](../VS_visualcpp/CPagerCtrl-Class.md), then uses a message map to associate the                                 [PGN_CALCSIZE](http://msdn.microsoft.com/library/windows/desktop/bb760864) notification with the `OnCalcsize` notification handler. In this example, the notification handler sets the width and height of the pager control to fixed values.  

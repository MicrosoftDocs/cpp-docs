...
>  When using MFC sockets in secondary threads in a statically linked MFC application, you must call `AfxSocketInit` in each thread that uses sockets to initialize the socket libraries. By default, `AfxSocketInit` is called only in the primary thread.  
 ...
 For more information, see [Windows Sockets: Using Class CAsyncSocket](../VS_visualcpp/Windows-Sockets--Using-Class-CAsyncSocket.md) and related articles., as well as                 [Windows Sockets 2 API](http://msdn.microsoft.com/library/windows/desktop/ms740673).  
...
-   **WSANOTINITIALISED** A successful [AfxSocketInit](../Topic/AfxSocketInit.md) must occur before using this API.  
  
-   **WSAENETDOWN** The Windows Sockets implementation detected that the network subsystem failed.  
  
-   **WSAEFAULT** The `lpSockAddrLen` argument is too small (less than the size of a [SOCKADDR](../VS_visualcpp/SOCKADDR-Structure.md) structure).  
...
### Remarks  
 This function is used to specify which MFC callback notification functions will be called for the socket. `AsyncSelect` automatically sets this socket to nonblocking mode. For more information, see the article [Windows Sockets: Socket Notifications](../VS_visualcpp/Windows-Sockets--Socket-Notifications.md).  
...
### Parameters  
 ...
 `lpSockAddr`  
 A pointer to a [SOCKADDR](../VS_visualcpp/SOCKADDR-Structure.md) structure that contains the address to assign to this socket.  
...
-   **WSAEFAULT** The `nSockAddrLen` argument is too small (less than the size of a [SOCKADDR](../VS_visualcpp/SOCKADDR-Structure.md) structure).  
...
### Parameters  
 ...
 `lpSockAddr`  
 A pointer to a [SOCKADDR](../VS_visualcpp/SOCKADDR-Structure.md) structure that contains the address of the connected socket.  
...
>  **Create** is **not** thread-safe.  If you are calling it in a multi-threaded environment where it could be invoked simultaneously by different threads, be sure to protect each call with a mutex or other synchronization lock.  
 ...
 For more information about stream and datagram sockets, see the articles [Windows Sockets: Background](../VS_visualcpp/Windows-Sockets--Background.md) and [Windows Sockets: Ports and Socket Addresses](../VS_visualcpp/Windows-Sockets--Ports-and-Socket-Addresses.md) and                         [Windows Sockets 2 API](http://msdn.microsoft.com/library/windows/desktop/ms740673).  
...
### Parameters  
 ...
 `lpSockAddr`  
 A pointer to the [SOCKADDR](../VS_visualcpp/SOCKADDR-Structure.md) structure that receives the name of the peer socket.  
...
### Parameters  
 ...
 `lpSockAddr`  
 A pointer to a [SOCKADDR](../VS_visualcpp/SOCKADDR-Structure.md) structure that receives the address of the socket.  
...
### Remarks  
 For more information, see [Windows Sockets: Socket Notifications](../VS_visualcpp/Windows-Sockets--Socket-Notifications.md).  
...
> [!NOTE]
>  In [CSocket](../VS_visualcpp/CSocket-Class.md), the `OnConnect` notification function is never called. For connections, you simply call **Connect**, which will return when the connection is completed (either successfully or in error). How connection notifications are handled is an MFC implementation detail.  
...
### Remarks  
 ...
 MFC supports out-of-band data, but users of class `CAsyncSocket` are discouraged from using it. The easier way is to create a second socket for passing such data. For more information about out-of-band data, see [Windows Sockets: Socket Notifications](../VS_visualcpp/Windows-Sockets--Socket-Notifications.md).  
...
##  <a name="casyncsocket__receivefrom"></a>  CAsyncSocket::ReceiveFrom  
 Call this member function to receive a datagram and store the source address in the [SOCKADDR](../VS_visualcpp/SOCKADDR-Structure.md) structure or in `rSocketAddress`.  
...
### Parameters  
 ...
 `lpSockAddr`  
 A pointer to a [SOCKADDR](../VS_visualcpp/SOCKADDR-Structure.md) structure that holds the source address upon return.  
...
##  <a name="casyncsocket__receivefromex"></a>  CAsyncSocket::ReceiveFromEx  
 Call this member function to receive a datagram and store the source address in the [SOCKADDR](../VS_visualcpp/SOCKADDR-Structure.md) structure or in `rSocketAddress` (handles IPv6 addresses).  
...
-   **MSG_OOB** Send out-of-band data ( **SOCK_STREAM** only).  
 ...
 `lpSockAddr`  
 A pointer to a [SOCKADDR](../VS_visualcpp/SOCKADDR-Structure.md) structure that contains the address of the target socket.  
...
-   **WSAEFAULT** The `lpBuf` or `lpSockAddr` parameters are not part of the user address space, or the `lpSockAddr` argument is too small (less than the size of a [SOCKADDR](../VS_visualcpp/SOCKADDR-Structure.md) structure).  

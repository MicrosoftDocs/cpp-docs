...
## Remarks  
 ...
 A common attack on software takes advantage of bugs in handling extreme or unexpected inputs. Carefully crafted input to the application may overwrite a location that contains a pointer to executable code. This can be used to redirect control flow to code controlled by the attacker. The CFG runtime checks do not fix the data corruption bugs in your executable. They instead make it more difficult for an attacker to use them to execute arbitrary code. CFG is a mitigation tool that prevents calls to locations other than function entry points in your code. It's similar to how Data Execution Prevention (DEP),  [/GS](../buildref/-gs--buffer-security-check-.md) stack checks, and [/DYNAMICBASE](../buildref/-dynamicbase--use-address-space-layout-randomization-.md) and [/HIGHENTROPYVA](../buildref/-highentropyva--support-64-bit-aslr-.md) address space layout randomization (ASLR) lower the chances that your code becomes an exploit vector.  
 ...
 The **/guard:cf** option is incompatible with [/ZI](../buildref/-z7---zi---zi--debug-information-format-.md) (Edit and Continue debug information) or [/clr](../buildref/-clr--common-language-runtime-compilation-.md) (Common Language Runtime Compilation).  

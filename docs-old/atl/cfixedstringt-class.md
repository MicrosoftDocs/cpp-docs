...
-   The memory manager for `CFixedStringT` is customized such that sharing of a [CStringData](../atl/cstringdata-class.md) object between two or more `CFixedStringT` objectsis not allowed. `CStringT` objects do not have this limitation.  
  
 For more information on the customization of `CFixedStringT` and memory management for string objects in general, see [Memory Management and CStringT](../atl/memory-management-with-cstringt.md).  
...
### Parameters  
 ...
 `pStringMgr`  
 A pointer to the memory manager of the `CFixedStringT` object. For more information on `IAtlStringMgr` and memory management for `CFixedStringT`, see [Memory Management and CStringT](../atl/memory-management-with-cstringt.md).  

...
-   [String Manipulation](../crt/string-manipulation--crt-.md) for working with C-style null-terminated strings  
  
-   Win32 API functions for managing strings  
  
-   MFC's class [CStringT Class](../atl/cstringt-class.md), which provides flexible, resizable string objects  
  
-   Class [CStringT Class](../atl/cstringt-class.md), which provides an MFC-independent string object with the same functionality as `CString`  
...
>  which the compiler treats as a Unicode string. For example, the following code:  
 ...
 [!code[NVC_ATLMFC_Utilities#187](../atl/codesnippet/CPP/string-data-management_1.cpp)]  
  
> [!NOTE]
>  is translated as a Unicode string if **_UNICODE** is defined or as an ANSI string if not. For more information, see the article [Unicode and Multibyte Character Set (MBCS) Support](../atl/unicode-and-multibyte-character-set--mbcs--support.md).  
...
> [!NOTE]
>  Use the [CSimpleStringT::GetBuffer](../Topic/CSimpleStringT::GetBuffer.md) and [CSimpleStringT::ReleaseBuffer](../Topic/CSimpleStringT::ReleaseBuffer.md) member functions when you need to directly access a `CString` as a nonconstant pointer to a character.  
  
> [!NOTE]
>  Use the [CStringT::AllocSysString](../Topic/CStringT::AllocSysString.md) and [CStringT::SetSysString](../Topic/CStringT::SetSysString.md) member functions to allocate and set `BSTR` objects used in Automation (formerly known as OLE Automation).  
...
##  <a name="_core_cstring_reference_counting"></a> CString Reference Counting  
 ...
 You can use the `CString` member functions [CSimpleStringT::LockBuffer](../Topic/CSimpleStringT::LockBuffer.md) and [CSimpleStringT::UnlockBuffer](../Topic/CSimpleStringT::UnlockBuffer.md) to disable or enable reference counting.  

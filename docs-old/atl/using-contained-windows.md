...
ATL implements contained windows with [CContainedWindowT](../atl/ccontainedwindowt-class.md). A contained window represents a window that delegates its messages to a container object instead of handling them in its own class.  
...
>  You do not need to derive a class from `CContainedWindowT` in order to use contained windows.  
 ...
 You typically use contained windows as data members of a container class. The container does not need to be a window; however, it must derive from [CMessageMap](../atl/cmessagemap-class.md).  
...
## Example  
 ...
 [!code[NVC_ATL_Windowing#67](../atl/codesnippet/CPP/using-contained-windows_1.h)]  
  
 For more information about contained windows, see the [SUBEDIT](../top/visual-c---samples.md) sample.  

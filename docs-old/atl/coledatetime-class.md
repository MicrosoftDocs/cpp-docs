...
>  Note in the table above that although day values become negative before midnight on December 30, 1899, time-of-day values do not. For example, 6:00 AM is always represented by a fractional value 0.25 regardless of whether the integer representing the day is positive (after December 30, 1899) or negative (before December 30, 1899). This means that a simple floating point comparison would erroneously sort a `COleDateTime` representing 6:00 AM on 12/29/1899 as **later** than one representing 7:00 AM on the same day.  
 ...
 The `COleDateTime` class handles dates from January 1, 100, through December 31, 9999. The `COleDateTime` class uses the Gregorian calendar; it does not support Julian dates. `COleDateTime` ignores Daylight Saving Time. (See [Date and Time: Automation Support](../atl/date-and-time--automation-support.md).)  
...
>  You can use the `%y` format to retrieve a two-digit year only for dates starting at 1900. If you use the `%y` format on a date before 1900, the code generates an ASSERT failure.  
 ...
 [!code[NVC_ATLMFC_Utilities#1](../atl/codesnippet/CPP/coledatetime-class_1.cpp)]  
  
 Basic arithmetic operations for the `COleDateTime` values use the companion class [COleDateTimeSpan](../atl/coledatetimespan-class.md). `COleDateTimeSpan` values define a time interval. The relationship between these classes is similar to the one between [CTime](../atl/ctime-class.md) and [CTimeSpan](../atl/ctimespan-class.md).  
  
 For more information about the `COleDateTime` and `COleDateTimeSpan` classes, see the article [Date and Time: Automation Support](../atl/date-and-time--automation-support.md).  
...
### Example  
 [!code[NVC_ATLMFC_Utilities#13](../atl/codesnippet/CPP/coledatetime-class_2.cpp)]  
  
### Example  
 The operators **>=**, **\<=**, **>**, and **<**, will assert if the `COleDateTime` object is set to null.  
  
 [!code[NVC_ATLMFC_Utilities#170](../atl/codesnippet/CPP/coledatetime-class_3.cpp)]  
 ...
 *systimeSrc*  
 ...
 `dbts`  
 A reference to a \<xref:System.Data.OleDb.OleDbType> structure containing the current local time.  
...
- `COleDateTime(` `wDosDate`, `wDosTime` **)** Constructs a `COleDateTime` object from the specified MS-DOS date and time values.  
 ...
 For more information on the `time_t` data type, see the [time](../crt/time--_time32--_time64.md) function in the *Run-Time Library Reference*.  
 ...
 For more information about the bounds for `COleDateTime` values, see the article [Date and Time: Automation Support](../atl/date-and-time--automation-support.md).  
  
> [!NOTE]
>  The constructor using **DBTIMESTAMP** parameter is only available when OLEDB.h is included.  
  
### Example  
 [!code[NVC_ATLMFC_Utilities#2](../atl/codesnippet/CPP/coledatetime-class_4.cpp)]  
...
### Remarks  
 ...
 `Format`( `lpszFormat`)  
 This form formats the value by using the format string which contains special formatting codes that are preceded by a percent sign (%), as in `printf`. The formatting string is passed as a parameter to the function. For more information about the formatting codes, see [strftime, wcsftime](../crt/strftime--wcsftime--_strftime_l--_wcsftime_l.md) in the Run-Time Library Reference.  
  
 `Format`( `nFormatID`)  
 This form formats the value by using the format string which contains special formatting codes that are preceded by a percent sign (%), as in `printf`. The formatting string is a resource. The ID of this string resource is passed as the parameter. For more information about the formatting codes, see [strftime, wcsftime](../crt/strftime--wcsftime--_strftime_l--_wcsftime_l.md) in the *Run-Time Library Reference*.  
  
### Example  
 [!code[NVC_ATLMFC_Utilities#3](../atl/codesnippet/CPP/coledatetime-class_5.cpp)]  
...
### Parameters  
 `dbts`  
 A reference to a \<xref:System.Data.OleDb.OleDbType> structure.  
...
### Example  
 [!code[NVC_ATLMFC_Utilities#4](../atl/codesnippet/CPP/coledatetime-class_6.cpp)]  
...
### Example  
 [!code[NVC_ATLMFC_Utilities#5](../atl/codesnippet/CPP/coledatetime-class_7.cpp)]  
...
### Example  
 [!code[NVC_ATLMFC_Utilities#6](../atl/codesnippet/CPP/coledatetime-class_8.cpp)]  
...
### Example  
 [!code[NVC_ATLMFC_Utilities#7](../atl/codesnippet/CPP/coledatetime-class_9.cpp)]  
...
### Example  
 [!code[NVC_ATLMFC_Utilities#8](../atl/codesnippet/CPP/coledatetime-class_10.cpp)]  
...
### Example  
 [!code[NVC_ATLMFC_Utilities#9](../atl/codesnippet/CPP/coledatetime-class_11.cpp)]  
...
>  The `COleDateTime` class does not support leap seconds.  
 ...
 For more information about the implementation for `COleDateTime`, see the article [Date and Time: Automation Support](../atl/date-and-time--automation-support.md).  
...
### Example  
 [!code[NVC_ATLMFC_Utilities#10](../atl/codesnippet/CPP/coledatetime-class_12.cpp)]  
...
>  Changing the value in the **DATE** object accessed by the pointer returned by this function will change the value of this `COleDateTime` object. It does not change the status of this `COleDateTime` object.  
 ...
 For more information about the implementation of the **DATE** object, see the article [Date and Time: Automation Support](../atl/date-and-time--automation-support.md).  
...
- **operator =(** `filetimeSrc` **)** The [FILETIME](http://msdn.microsoft.com/library/windows/desktop/ms724284) value is converted and copied into this `COleDateTime` object. If the conversion is successful, the status of this object is set to valid; otherwise it is set to invalid. `FILETIME` uses Universal Coordinated Time (UTC), so if you pass a UTC time in the structure, your results will be converted from UTC time to local time, and will be stored as variant time. This behavior is the same as in Visual C++ 6.0 and Visual C++.NET 2003 SP2. See [File Times](http://msdn.microsoft.com/library/windows/desktop/ms724290) in the [!INCLUDE[winSDK](../atl/includes/winsdk_md.md)] for more information.  
 ...
 For more information, see the [VARIANT](assetId:///e305240e-9e11-4006-98cc-26f4932d2118) entry in the [!INCLUDE[winSDK](../atl/includes/winsdk_md.md)].  
...
### Remarks  
 `COleDateTime` objects represent absolute times. [COleDateTimeSpan](../atl/coledatetimespan-class.md) objects represent relative times. The first two operators allow you to add and subtract a `COleDateTimeSpan` value from a `COleDateTime` value. The third operator allows you to subtract one `COleDateTime` value from another to yield a `COleDateTimeSpan` value.  
...
### Example  
 [!code[NVC_ATLMFC_Utilities#12](../atl/codesnippet/CPP/coledatetime-class_13.cpp)]  
...
```
    operatorDATE() const throw();
```  
  
### Remarks  
 This operator returns a **DATE** object whose value is copied from this `COleDateTime` object. For more information about the implementation of the **DATE** object, see the article [Date and Time: Automation Support](../atl/date-and-time--automation-support.md).  
...
>  Year values must lie between 100 and 9999, inclusively.  
 ...
 For more information about the bounds and implementation for `COleDateTime` values, see the article [Date and Time: Automation Support](../atl/date-and-time--automation-support.md).  
...
### Example  
 [!code[NVC_ATLMFC_Utilities#11](../atl/codesnippet/CPP/coledatetime-class_14.cpp)]  
...
## See Also  
 ...
 [CTime Class](../atl/ctime-class.md)   
 [CTimeSpan Class](../atl/ctimespan-class.md)   

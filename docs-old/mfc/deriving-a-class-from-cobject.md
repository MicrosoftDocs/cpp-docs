...
This article describes the minimum steps necessary to derive a class from [CObject](../mfcref/cobject-class.md). Other `CObject` class articles describe the steps needed to take advantage of specific `CObject` features, such as serialization and diagnostic debugging support.  
...
1.  Use the normal C++ syntax to derive your class from `CObject` (or from a class derived from `CObject`).  
     ...
     [!code[NVC_MFCCObjectSample#1](../mfc/codesnippet/CPP/deriving-a-class-from-cobject_1.h)]  

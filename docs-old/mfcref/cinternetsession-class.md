...
### Example  
  See the example for [CFtpFileFind](../mfcref/cftpfilefind-class.md).  
...
### Return Value  
 Nonzero if successful; otherwise 0. If the call fails, determine the cause of the failure by examining the thrown [CInternetException](../mfcref/cinternetexception-class.md) object.  
...
### Parameters  
 ...
 `pstrCookieData`  
 In the first overload, a pointer to a string containing the address of the buffer that receives the cookie data. This value can be **NULL**. In the second overload, a reference to a [CString](../atl/cstringt-class.md) object to receive the cookie data.  
...
### Return Value  
 A pointer to a [CFtpConnection](../mfcref/cftpconnection-class.md) object. If the call fails, determine the cause of the failure by examining the thrown [CInternetException](../mfcref/cinternetexception-class.md) object.  
  
### Remarks  
 `GetFtpConnection` connects to an FTP server, and creates and returns a pointer to a **CFTPConnection** object. It does not perform any specific operation on the server. If you intend to read or write to files, for example, you must perform those operations as separate steps. See the classes [CFtpConnection](../mfcref/cftpconnection-class.md) and [CFtpFileFind](../mfcref/cftpfilefind-class.md) for information about searching for files, opening files, and reading or writing to files. See the article [Internet Programming with WinInet](../mfc/win32-internet-extensions--wininet-.md) for steps in performing common FTP connection tasks.  
...
### Return Value  
 A pointer to a [CGopherConnection](../mfcref/cgopherconnection-class.md) object. If the call fails, determine the cause of the failure by examining the thrown [CInternetException](../mfcref/cinternetexception-class.md) object.  
  
### Remarks  
 `GetGopherConnection` connects to a gopher server, and creates and returns a pointer to a `CGopherConnection` object. It does not perform any specific operation on the server. If you intend to read or write data, for example, you must perform those operations as separate steps. See the classes [CGopherConnection](../mfcref/cgopherconnection-class.md), [CGopherFile](../mfcref/cgopherfile-class.md), and [CGopherFileFind](../mfcref/cgopherfilefind-class.md) for information about searching for files, opening files, and reading or writing to files. For information about browsing an FTP site, see the member function [OpenURL](#cinternetsession__openurl). See the article [Internet Programming with WinInet](../mfc/win32-internet-extensions--wininet-.md) for steps in performing common gopher connection tasks.  
 ...
 *dwflags*  
 Any combination of the **INTERNET_ FLAG_\*** flags. See the table in the **Remarks** section of [CHttpConnection::OpenRequest](../mfcref/chttpconnection-class.md#chttpconnection__openrequest) for a description of `dwFlags` values.  
  
### Return Value  
 A pointer to a [CHttpConnection](../mfcref/chttpconnection-class.md) object. If the call fails, determine the cause of the failure by examining the thrown [CInternetException](../mfcref/cinternetexception-class.md) object.  
  
### Remarks  
 `GetHttpConnection` connects to an HTTP server, and creates and returns a pointer to a `CHttpConnection` object. It does not perform any specific operation on the server. If you intend to query an HTTP header, for example, you must perform this operation as a separate step. See the classes [CHttpConnection](../mfcref/chttpconnection-class.md) and [CHttpFile](../mfcref/chttpfile-class.md) for information about operations you can perform by using a connection to an HTTP server. For information about browsing an HTTP site, see the member function [OpenURL](#cinternetsession__openurl). See the article [Internet Programming with WinInet](../mfc/win32-internet-extensions--wininet-.md) for steps in performing common HTTP connection tasks.  
...
|`INTERNET_STATUS_CONNECTING_TO_SERVER`|Connecting to the socket address ( [SOCKADDR](../mfcref/sockaddr-structure.md)) pointed to by `lpvStatusInformation`.|  
...
>  Status callbacks need thread-state protection. If you are using MFC in a shared library, add the following line to the beginning of your override:  
 ...
 [!code[NVC_MFCHtmlHttp#8](../mfcref/codesnippet/CPP/cinternetsession-class_1.cpp)]  
...
-   **INTERNET_FLAG_PASSIVE** Used for an FTP site. Uses passive FTP semantics. Used with [CInternetConnection](../mfcref/cinternetconnection-class.md) of `OpenURL`.  
...
### Remarks  
 ...
 `OpenURL`, which wraps the Win32 function **InternetOpenURL**, allows only downloading, retrieving, and reading the data from an Internet server. `OpenURL` allows no file manipulation on a remote location, so it requires no [CInternetConnection](../mfcref/cinternetconnection-class.md) object.  

...
## Remarks  
 ...
 This class is based on the assumption that you understand network communications. You are responsible for handling blocking, byte-order differences, and conversions between Unicode and multibyte character set (MBCS) strings. If you want a more convenient interface that manages these issues for you, see class [CSocket](../mfcref/csocket-class.md).  
...
-   **WSAEFAULT** The `lpSockAddrLen` argument is too small (less than the size of a [SOCKADDR](../mfcref/sockaddr-structure.md) structure).  
...
### Parameters  
 ...
 `lpSockAddr`  
 A pointer to a [SOCKADDR](../mfcref/sockaddr-structure.md) structure that contains the address to assign to this socket.  
...
-   **WSAEFAULT** The `nSockAddrLen` argument is too small (less than the size of a [SOCKADDR](../mfcref/sockaddr-structure.md) structure).  
...
### Parameters  
 ...
 `lpSockAddr`  
 A pointer to a [SOCKADDR](../mfcref/sockaddr-structure.md) structure that contains the address of the connected socket.  
...
### Parameters  
 ...
 `lpSockAddr`  
 A pointer to the [SOCKADDR](../mfcref/sockaddr-structure.md) structure that receives the name of the peer socket.  
...
### Parameters  
 ...
 `lpSockAddr`  
 A pointer to a [SOCKADDR](../mfcref/sockaddr-structure.md) structure that receives the address of the socket.  
...
> [!NOTE]
>  In [CSocket](../mfcref/csocket-class.md), the `OnConnect` notification function is never called. For connections, you simply call **Connect**, which will return when the connection is completed (either successfully or in error). How connection notifications are handled is an MFC implementation detail.  
  
 For more information, see [Windows Sockets: Socket Notifications](../mfc/windows-sockets--socket-notifications.md).  
  
### Example  
 [!code[NVC_MFCAsyncSocket#1](../mfcref/codesnippet/CPP/casyncsocket-class_1.cpp)]  
...
### Example  
 [!code[NVC_MFCAsyncSocket#2](../mfcref/codesnippet/CPP/casyncsocket-class_2.cpp)]  
...
### Example  
 [!code[NVC_MFCAsyncSocket#3](../mfcref/codesnippet/CPP/casyncsocket-class_3.cpp)]  
...
##  <a name="casyncsocket__receivefrom"></a>  CAsyncSocket::ReceiveFrom  
 Call this member function to receive a datagram and store the source address in the [SOCKADDR](../mfcref/sockaddr-structure.md) structure or in `rSocketAddress`.  
...
### Parameters  
 ...
 `lpSockAddr`  
 A pointer to a [SOCKADDR](../mfcref/sockaddr-structure.md) structure that holds the source address upon return.  
...
##  <a name="casyncsocket__receivefromex"></a>  CAsyncSocket::ReceiveFromEx  
 Call this member function to receive a datagram and store the source address in the [SOCKADDR](../mfcref/sockaddr-structure.md) structure or in `rSocketAddress` (handles IPv6 addresses).  
...
-   **MSG_OOB** Send out-of-band data ( **SOCK_STREAM** only).  
 ...
 `lpSockAddr`  
 A pointer to a [SOCKADDR](../mfcref/sockaddr-structure.md) structure that contains the address of the target socket.  
...
-   **WSAEFAULT** The `lpBuf` or `lpSockAddr` parameters are not part of the user address space, or the `lpSockAddr` argument is too small (less than the size of a [SOCKADDR](../mfcref/sockaddr-structure.md) structure).  

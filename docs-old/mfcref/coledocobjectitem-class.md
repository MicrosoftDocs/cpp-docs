...
### Parameters  
 `pCaller`  
 A pointer to a [CView](../mfcref/cview-class.md) object that is sending the print command.  
  
 `pInfo`  
 A pointer to a [CPrintInfo](../mfcref/cprintinfo-structure.md) object that describes the job to be printed.  
...
### Parameters  
 `dwCloseOption`  
 Flag specifying under what circumstances the OLE item is saved when it returns to the loaded state. For a list of possible values, see [COleClientItem::Close](../mfcref/coleclientitem-class.md#coleclientitem__close).  

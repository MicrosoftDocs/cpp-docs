...
> [!NOTE]
>  If you are working with the Data Access Objects (DAO) classes rather than the Open Database Connectivity (ODBC) classes, use class [CDaoDatabase](../mfcref/cdaodatabase-class.md) instead. For more information, see the article [Overview: Database Programming](../data/data-access-programming--mfc-atl-.md).  
 ...
 For more information about `CDatabase`, see the articles [Data Source (ODBC)](../data/data-source--odbc-.md) and [Overview: Database Programming](../data/data-access-programming--mfc-atl-.md).  
 ...
 **BeginTrans** may also lock data records on the server, depending on the requested concurrency and the capabilities of the data source. For information about locking data, see the article [Recordset: Locking Records (ODBC)](../data/recordset--locking-records--odbc-.md).  
  
 User-defined transactions are explained in the article [Transaction (ODBC)](../data/transaction--odbc-.md).  
...
>  Calling **BeginTrans** again without calling **CommitTrans** or **Rollback** is an error.  
 ...
 For more information about transactions, see the article [Transaction (ODBC)](../data/transaction--odbc-.md).  
...
### Remarks  
 For information about transactions, see the article [Transaction (ODBC)](../data/transaction--odbc-.md).  
...
### Example  
 ...
 [!code[NVC_MFCDatabase#9](../mfc/codesnippet/CPP/cdatabase-class_1.h)]  
  
 [!code[NVC_MFCDatabase#10](../mfc/codesnippet/CPP/cdatabase-class_2.cpp)]  
...
### Example  
 [!code[NVC_MFCDatabase#12](../mfc/codesnippet/CPP/cdatabase-class_3.cpp)]  
...
### Parameters  
 `lpszSQL`  
 Pointer to a null-terminated string containing a valid SQL command to execute. You can pass a [CString](../atl/cstringt-class.md).  
...
### Example  
 [!code[NVC_MFCDatabase#13](../mfc/codesnippet/CPP/cdatabase-class_4.cpp)]  
...
|`SQL_BP_OTHER_HSTMT`|Bookmarks associated with one recordset object are valid on a second recordset.|  
 ...
 For more information about this return value, see the ODBC API function **SQLGetInfo** in the [!INCLUDE[winSDK](../atl/includes/winsdk_md.md)]. For more information about bookmarks, see the article [Recordset: Bookmarks and Absolute Positions (ODBC)](../data/recordset--bookmarks-and-absolute-positions--odbc-.md).  
...
### Return Value  
 A `const`[CString](../atl/cstringt-class.md) containing the connection string if `OpenEx` or `Open` has been called; otherwise, an empty string.  
...
|`SQL_CB_PRESERVE`|Proceed normally with `CRecordset` operations.|  
 ...
 For more information about this return value, see the ODBC API function **SQLGetInfo** in the [!INCLUDE[winSDK](../atl/includes/winsdk_md.md)]. For more information about transactions, see the article [Transaction (ODBC)](../data/transaction--odbc-.md).  
...
### Return Value  
 A [CString](../atl/cstringt-class.md) containing the database name if successful; otherwise, an empty `CString`.  
...
### Example  
 [!code[NVC_MFCDatabase#15](../mfc/codesnippet/CPP/cdatabase-class_5.cpp)]  
...
### Example  
 [!code[NVC_MFCDatabase#14](../mfc/codesnippet/CPP/cdatabase-class_6.cpp)]  
...
### Example  
 [!code[NVC_MFCDatabase#11](../mfc/codesnippet/CPP/cdatabase-class_7.cpp)]  
...
### Remarks  
 ...
 After a rollback, the record that was current before the rollback remains current. For details about the state of the recordset and the data source after a rollback, see the article [Transaction (ODBC)](../data/transaction--odbc-.md).  

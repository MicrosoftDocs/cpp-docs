...
### Example  
 [!code[NVC_MFCDocView#175](../mfc/codesnippet/CPP/cgdiobject-class_1.cpp)]  
...
|Object|Buffer type|  
|------------|-----------------|  
|`CPen`|[LOGPEN](../mfcref/logpen-structure.md)|  
|`CBrush`|[LOGBRUSH](../mfcref/logbrush-structure.md)|  
|`CFont`|[LOGFONT](http://msdn.microsoft.com/library/windows/desktop/dd145037)|  
|`CBitmap`|[BITMAP](../mfcref/bitmap-structure.md)|  
|`CPalette`|**WORD**|  
|`CRgn`|Not supported|  
  
 If the object is a `CBitmap` object, `GetObject` returns only the width, height, and color format information of the bitmap. The actual bits can be retrieved by using [CBitmap::GetBitmapBits](../mfcref/cbitmap-class.md#cbitmap__getbitmapbits).  
  
 If the object is a `CPalette` object, `GetObject` retrieves a **WORD** that specifies the number of entries in the palette. The function does not retrieve the                         [LOGPALETTE](http://msdn.microsoft.com/library/windows/desktop/dd145040) structure that defines the palette. An application can get information on palette entries by calling [CPalette::GetPaletteEntries](../mfcref/cpalette-class.md#cpalette__getpaletteentries).  
...
### Example  
  See the example for [CWnd::IsWindowEnabled](../mfcref/cwnd-class.md#cwnd__iswindowenabled).  

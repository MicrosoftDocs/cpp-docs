...
## Remarks  
 A pane is usually an application-specific object derived from [CView](../mfcref/cview-class.md), but it can be any [CWnd](../mfcref/cwnd-class.md) object that has the appropriate child window ID.  
  
 A `CSplitterWnd` object is usually embedded in a parent [CFrameWnd](../mfcref/cframewnd-class.md) or [CMDIChildWnd](../mfcref/cmdichildwnd-class.md) object. Create a `CSplitterWnd` object using the following steps:  
  
1.  Embed a `CSplitterWnd` member variable in the parent frame.  
  
2.  Override the parent frame's [CFrameWnd::OnCreateClient](../mfcref/cframewnd-class.md#cframewnd__oncreateclient) member function.  
...
-   MFC sample [Scribble](../top/visual-c---samples.md)  
...
### Remarks  
 This member function is a high level command that is used by the [CView](../mfcref/cview-class.md) class to delegate to the `CSplitterWnd` implementation.  
 ...
 *nMaxCols*  
 ...
 `pContext`  
 A pointer to a [CCreateContext](../mfcref/ccreatecontext-structure.md) structure. In most cases, this can be the `pContext` passed to the parent frame window.  
...
### Remarks  
 You can embed a `CSplitterWnd` in a parent [CFrameWnd](../mfcref/cframewnd-class.md) or [CMDIChildWnd](../mfcref/cmdichildwnd-class.md) object by taking the following steps:  
...
3.  Call the **Create** member function from within the overridden `OnCreateClient`.  
 ...
 For more on dynamic splitter windows, see "Splitter Windows" in the article [Multiple Document Types, Views, and Frame Windows](../mfc/multiple-document-types--views--and-frame-windows.md), [Technical Note 29](../mfc/tn029--splitter-windows.md), and the [CSplitterWnd](../mfcref/csplitterwnd-class.md) class overview.  
  
### Example  
 [!code[NVC_MFCWindowing#125](../mfcref/codesnippet/CPP/csplitterwnd-class_1.cpp)]  
...
### Remarks  
 A `CSplitterWnd` is usually embedded in a parent `CFrameWnd` or [CMDIChildWnd](../mfcref/cmdichildwnd-class.md) object by taking the following steps:  
  
1.  Embed a `CSplitterWnd` member variable in the parent frame.  
  
2.  Override the parent frame's `OnCreateClient` member function.  
  
3.  Call the `CreateStatic` member function from within the overridden [CFrameWnd::OnCreateClient](../mfcref/cframewnd-class.md#cframewnd__oncreateclient).  
 ...
 See "Splitter Windows" in the article [Multiple Document Types, Views, and Frame Windows](../mfc/multiple-document-types--views--and-frame-windows.md), [Technical Note 29](../mfc/tn029--splitter-windows.md), and the [CSplitterWnd](../mfcref/csplitterwnd-class.md) class overview for more on static splitter windows.  
...
### Parameters  
 ...
 `pViewClass`  
 Specifies the [CRuntimeClass](../mfcref/cruntimeclass-structure.md) of the new view.  
  
 *sizeInit*  
 Specifies the initial size of the new view.  
  
 `pContext`  
 A pointer to a creation context used to create the view (usually the `pContext` passed into the parent frame's overridden [CFrameWnd::OnCreateClient](../mfcref/cframewnd-class.md#cframewnd__oncreateclient) member function in which the splitter window is being created).  
...
### Example  
 [!code[NVC_MFCWindowing#4](../mfcref/codesnippet/CPP/csplitterwnd-class_2.cpp)]  
...
### Remarks  
 If the active view is being deleted, the next view will become active. The default implementation assumes the view will auto delete in [PostNcDestroy](../mfcref/cwnd-class.md#cwnd__postncdestroy).  
...
### Return Value  
 Returns the pane at the specified row and column. The returned pane is usually a [CView](../mfcref/cview-class.md)-derived class.  
...
### Example  
 [!code[NVC_MFCWindowing#5](../mfcref/codesnippet/CPP/csplitterwnd-class_3.cpp)]  
...
### Parameters  
 `pWnd`  
 A pointer to a [CWnd](../mfcref/cwnd-class.md) object to be tested.  
...
### Parameters  
 `pDC`  
 A pointer to the device context in which to draw. If `pDC` is **NULL**, then [CWnd::RedrawWindow](../mfcref/cwnd-class.md#cwnd__redrawwindow) is called by the framework and no split window is drawn.  
...
-   **splitBorder** The split window borders.  
 ...
 `rect`  
 A reference to a [CRect](../atl/crect-class.md) object specifying the size and shape of the split windows.  
...
### Example  
 [!code[NVC_MFCWindowing#6](../mfcref/codesnippet/CPP/csplitterwnd-class_4.cpp)]  

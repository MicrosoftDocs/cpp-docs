...
-   Find and replace.  
 ...
 [!code[NVC_MFCDocView#65](../mfc/codesnippet/CPP/ceditview-class_1.cpp)]  
...
-   `CEditView` can display text in only a single font. No special character formatting is supported. See class [CRichEditView](../mfcref/cricheditview-class.md) for greater capabilities.  
...
### Remarks  
 After constructing the object, you must call the [CWnd::Create](../mfcref/cwnd-class.md#cwnd__create) function before the edit control is used. If you derive a class from `CEditView` and add it to the template using `CWinApp::AddDocTemplate`, the framework calls both this constructor and the **Create** function.  
...
### Remarks  
 This control is of type [CEdit](../mfcref/cedit-class.md), so you can manipulate the Windows edit control directly using the `CEdit` member functions.  
  
> [!CAUTION]
>  Using the `CEdit` object can change the state of the underlying Windows edit control. For example, you should not change the tab settings using the [CEdit::SetTabStops](../mfcref/cedit-class.md#cedit__settabstops) function because `CEditView` caches these settings for use both in the edit control and in printing. Instead, use [CEditView::SetTabStops](#ceditview__settabstops).  
  
### Example  
 [!code[NVC_MFCDocView#66](../mfc/codesnippet/CPP/ceditview-class_2.cpp)]  
  
##  <a name="ceditview__getprinterfont"></a>  CEditView::GetPrinterFont  
 Call `GetPrinterFont` to get a pointer to a [CFont](../mfcref/cfont-class.md) object that describes the current printer font.  
 ...
 *rectLayout*  
 Reference to a [CRect](../atl/crect-class.md) object or "RECT" structure specifying the rectangle in which the text is to be rendered.  
...
### Example  
 ...
 [!code[NVC_MFCDocView#67](../mfc/codesnippet/CPP/ceditview-class_3.cpp)]  

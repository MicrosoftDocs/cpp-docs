...
>  `CToolBar` supports bitmaps with a maximum of 16 colors. When you load an image into a toolbar editor, Visual Studio automatically converts the image to a 16-color bitmap, if necessary, and displays a warning message if the image was converted. If you use an image with more than 16 colors (using an external editor to edit the image), the application might behave unexpectedly.  
 ...
 To create a radio button, call a [CCmdUI](../mfcref/ccmdui-class.md) object's [SetRadio](../mfcref/ccmdui-class.md#ccmdui__setradio) member function from an `ON_UPDATE_COMMAND_UI` handler. Pass `SetRadio` an argument of 0 for unchecked or nonzero for checked. In order to provide a radio group's mutually exclusive behavior, you must have `ON_UPDATE_COMMAND_UI` handlers for all of the buttons in the group.  
...
### Example  
 [!code[NVC_MFCDocView#179](../mfc/codesnippet/CPP/ctoolbar-class_1.cpp)]  
...
### Parameters  
 ...
 `dwCtrlStyle`  
 Additional styles for the creation of the embedded [CToolBarCtrl](../mfcref/ctoolbarctrl-class.md) object. By default, this value is set to **TBSTYLE_FLAT**. For a complete list of toolbar styles, see `dwStyle`.  
 ...
 *rcBorders*  
 A [CRect](../atl/crect-class.md) object that defines the widths of the toolbar window borders. These borders are set to 0,0,0,0 by default, thereby resulting in a toolbar window with no borders.  
...
### Example  
 [!code[NVC_MFCDocView#180](../mfc/codesnippet/CPP/ctoolbar-class_2.cpp)]  
...
### Parameters  
 ...
 `rString`  
 A reference to a [CString](../atl/cstringt-class.md) object that will contain the text to be retrieved.  
...
### Remarks  
 Use `GetToolBarCtrl` to take advantage of the functionality of the Windows toolbar common control, and to take advantage of the support [CToolBarCtrl](../mfcref/ctoolbarctrl-class.md) provides for toolbar customization.  
...
### Example  
 [!code[NVC_MFCDocViewSDI#15](../mfc/codesnippet/CPP/ctoolbar-class_3.cpp)]  
...
> [!NOTE]
>  You can also set button states using the `nStyle` parameter; however, because button states are controlled by the [ON_UPDATE_COMMAND_UI](../Topic/ON_UPDATE_COMMAND_UI.md) handler, any state you set using `SetButtonInfo` will be lost during the next idle processing. See [How to Update User-Interface Objects](../mfc/how-to--update-user-interface-objects.md) and [TN031: Control Bars](../mfc/tn031--control-bars.md) for more information.  
  
 For information on bitmap images and buttons, see the [CToolBar](../mfcref/ctoolbar-class.md) Overview and [CToolBar::LoadBitmap](#ctoolbar__loadbitmap).  
...
> [!NOTE]
>  You can also set button states using the `nStyle` parameter; however, because button states are controlled by the [ON_UPDATE_COMMAND_UI](../Topic/ON_UPDATE_COMMAND_UI.md) handler, any state you set using `SetButtonStyle` will be lost during the next idle processing. See [How to Update User-Interface Objects](../mfc/how-to--update-user-interface-objects.md) and [TN031: Control Bars](../mfc/tn031--control-bars.md) for more information.  
...
### Example  
 [!code[NVC_MFCListView#8](../atl/codesnippet/CPP/ctoolbar-class_4.cpp)]  
  
## See Also  
 [MFC Sample CTRLBARS](../top/visual-c---samples.md)   
 [MFC Sample DLGCBR32](../top/visual-c---samples.md)   
 [MFC Sample DOCKTOOL](../top/visual-c---samples.md)   
 ...
 [CCmdUI::SetCheck](../mfcref/ccmdui-class.md#ccmdui__setcheck)   
 [CCmdUI::SetRadio](../mfcref/ccmdui-class.md#ccmdui__setradio)
...
## Example  
 ...
 [!code[NVC_MFCCollections#110](../mfc/codesnippet/CPP/ctypedptrlist-class_1.cpp)]  
  
 [!code[NVC_MFCCollections#111](../mfc/codesnippet/CPP/ctypedptrlist-class_2.cpp)]  
 ...
 *TYPE*  
 ...
 `BASE_CLASS`  
 Base class of the typed pointer list class; must be a pointer list class ( [CObList](../mfcref/coblist-class.md) or [CPtrList](../mfcref/cptrlist-class.md)).  
  
 `pNewList`  
 A pointer to another [CTypedPtrList](../mfcref/ctypedptrlist-class.md) object. The elements in `pNewList` will be added to this list.  
...
### Remarks  
 You must ensure that the list is not empty before calling `GetHead`. If the list is empty, then the Debug version of the Microsoft Foundation Class Library asserts. Use [IsEmpty](../mfcref/coblist-class.md#coblist__isempty) to verify that the list contains elements.  
...
### Remarks  
 You can use `GetNext` in a forward iteration loop if you establish the initial position with a call to `GetHeadPosition` or [CPtrList::Find](../mfcref/coblist-class.md#coblist__find).  
  
 You must ensure that your **POSITION** value represents a valid position in the list. If it is invalid, then the Debug version of the Microsoft Foundation Class Library asserts.  
  
 If the retrieved element is the last in the list, then the new value of `rPosition` is set to **NULL**.  
  
 It is possible to remove an element during an iteration. See the example for [CObList::RemoveAt](../mfcref/coblist-class.md#coblist__removeat).  
...
### Remarks  
 You must ensure that the list is not empty before calling `RemoveHead`. If the list is empty, then the Debug version of the Microsoft Foundation Class Library asserts. Use [IsEmpty](../mfcref/coblist-class.md#coblist__isempty) to verify that the list contains elements.  
...
### Remarks  
 You must ensure that the list is not empty before calling `RemoveTail`. If the list is empty, then the Debug version of the Microsoft Foundation Class Library asserts. Use [IsEmpty](../mfcref/coblist-class.md#coblist__isempty) to verify that the list contains elements.  
...
### Remarks  
 ...
 For more detailed remarks, see [CObList::SetAt](../mfcref/coblist-class.md#coblist__setat).  

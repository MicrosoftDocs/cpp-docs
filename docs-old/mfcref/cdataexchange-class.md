...
### Parameters  
 *pDlgWnd*  
 A pointer to the parent window that contains the control. Usually this is a [CDialog](../mfcref/cdialog-class.md)-derived object.  
...
### Remarks  
 Construct a `CDataExchange` object yourself to store extra information in the data exchange object to pass to your window's [CWnd::DoDataExchange](../mfcref/cwnd-class.md#cwnd__dodataexchange) member function.  
  
### Example  
  [!code[NVC_MFCControlLadenDialog#70](../mfc/codesnippet/CPP/cdataexchange-class_1.cpp)]  
...
### Remarks  
 **Fail** restores the focus and selection to the control whose validation failed (if there is a control to restore). **Fail** then throws an exception of type [CUserException](../mfcref/cuserexception-class.md) to stop the validation process. The exception causes a message box explaining the error to be displayed. After DDV validation fails, the user can reenter data in the offending control.  
...
##  <a name="cdataexchange__m_pdlgwnd"></a>  CDataExchange::m_pDlgWnd  
 Contains a pointer to the [CWnd](../mfcref/cwnd-class.md) object for which dialog data exchange (DDX) or validation (DDV) is taking place.  
...
### Remarks  
 This object is usually a [CDialog](../mfcref/cdialog-class.md) object. Implementors of custom DDX or DDV routines can use this pointer to obtain access to the dialog window that contains the controls they are operating on.  
...
## See Also  
 ...
 [CWnd::DoDataExchange](../mfcref/cwnd-class.md#cwnd__dodataexchange)   

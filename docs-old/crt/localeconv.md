...
## Return Value  
 `localeconv` returns a pointer to a filled-in object of type [struct lconv](../crt/standard-types.md). The values contained in the object can be overwritten by subsequent calls to `localeconv` and do not directly modify the object. Calls to [setlocale](../crt/setlocale--_wsetlocale.md) with `category` values of `LC_ALL`, `LC_MONETARY`, or `LC_NUMERIC` overwrite the contents of the structure.  

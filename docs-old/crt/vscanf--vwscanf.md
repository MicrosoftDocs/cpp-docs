...
---
# vscanf, vwscanf
Reads formatted data from the standard input stream. More secure versions of these function are available; see [vscanf_s, vwscanf_s](../crt/vscanf_s--vwscanf_s.md).  
...
> [!IMPORTANT]
>  When you use `vscanf` to read a string, always specify a width for the `%s` format (for example, `"%32s"` instead of `"%s"`); otherwise, incorrectly formatted input can cause a buffer overrun. As an alternative, you can use [vscanf_s, vwscanf_s](../crt/vscanf_s--vwscanf_s.md) or [fgets](../crt/fgets--fgetws.md).  
...
## See Also  
 ...
 [vscanf_s, vwscanf_s](../crt/vscanf_s--vwscanf_s.md)
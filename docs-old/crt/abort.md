 ...
 **Microsoft Specific**  
 ...
 If the Windows error reporting handler is not invoked, then `abort` calls [_exit](../crt/exit--_exit--_exit.md) to terminate the process with exit code 3 and returns control to the parent process or the operating system. `_exit` does not flush stream buffers or do `atexit`/`_onexit` processing.  

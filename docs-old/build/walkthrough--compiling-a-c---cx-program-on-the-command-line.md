...
## Compiling a C++/CX Program  
 To enable compilation for C++/CX, you must use the [/ZW](../buildref/-zw--windows-runtime-compilation-.md) compiler option. The Visual C++ compiler generates an .exe file that targets the Windows Runtime, and links to the required libraries.  
...
5.  At the command prompt, enter **cl /EHsc /ZW basiccx.cpp /link /SUBSYSTEM:CONSOLE**. The cl.exe compiler compiles the source code into an .obj file, and then runs the linker to generate an executable program named basiccx.exe. (The [/EHsc](../buildref/-eh--exception-handling-model-.md) compiler option specifies the C++ exception-handling model, and the [/link](../buildref/-link--pass-options-to-linker-.md) flag specifies a console application.)  

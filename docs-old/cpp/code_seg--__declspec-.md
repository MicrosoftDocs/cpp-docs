...
## Remarks  
 ...
 Object code that's generated when `declarator` is defined is put in the text segment specified by `segname`, which is a narrow-string literal. The name `segname` does not have to be specified in a [section](../c/section.md) pragma before it can be used in a declaration. By default, when no `code_seg` is specified, object code is put in a segment named .text. A `code_seg` attribute overrides any existing [#pragma code_seg](../c/code_seg.md) directive. A `code_seg` attribute applied to a member function overrides any `code_seg` attribute applied to the enclosing class.  

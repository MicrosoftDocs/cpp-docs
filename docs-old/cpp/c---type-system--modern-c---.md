...
```  
 ...
 The `const` qualifier is used extensively in function and variable declarations and "const correctness" is an important concept in C++; essentially it means to use `const` to guarantee, at compile time, that values are not modified unintentionally. For more information, see [const](../cpp/const--c---.md).  
  
 A `const` type is distinct from its non-const version; for example, `const int` is a distinct type from `int`. You can use the C++ `const_cast` operator on those rare occasions when you must remove *const-ness* from a variable. For more information, see [Type Conversions and Type Safety](../cpp/type-conversions-and-type-safety--modern-c---.md).  
...
```  
 ...
 For more information about smart pointers, see [Smart Pointers](../cpp/smart-pointers--modern-c---.md).  
  
 For more information about pointer conversions, see [Type Conversions and Type Safety](../cpp/type-conversions-and-type-safety--modern-c---.md).  
  
 For more information about pointers in general, see [Pointers](../cpp/pointers--c---.md).  
...
|||  
|-|-|  
|[Value Types](../cpp/value-types--modern-c---.md)|Describes *value types* along with issues relating to their use.|  
|[Type Conversions and Type Safety](../cpp/type-conversions-and-type-safety--modern-c---.md)|Describes common type conversion issues and shows how to avoid them.|  

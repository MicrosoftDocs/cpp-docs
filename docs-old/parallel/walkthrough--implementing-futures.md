...
> [!IMPORTANT]
>  This topic illustrates the concept of futures for demonstration purposes. We recommend that you use [std::future](../stdcpplib/future-class.md) or [concurrency::task](../parallel/task-class--concurrency-runtime-.md) when you require an asynchronous task that computes a value for later use.  
...
1.  Declare a template class named `async_future` that is parameterized on the type of the resulting computation. Add `public` and `private` sections to this class.  
     ...
     [!code[concrt-futures#2](../parallel/codesnippet/CPP/walkthrough--implementing-futures_1.cpp)]  
  
2.  In the `private` section of the `async_future` class, declare a `task_group` and a `single_assignment` data member.  
  
     [!code[concrt-futures#3](../parallel/codesnippet/CPP/walkthrough--implementing-futures_2.cpp)]  
...
3.  In the `public` section of the `async_future` class, implement the constructor. The constructor is a template that is parameterized on the work function that computes the result. The constructor asynchronously executes the work function in the `task_group` data member and uses the [concurrency::send](../Topic/send%20Function.md) function to write the result to the `single_assignment` data member.  
     ...
     [!code[concrt-futures#4](../parallel/codesnippet/CPP/walkthrough--implementing-futures_3.cpp)]  
  
4.  In the `public` section of the `async_future` class, implement the destructor. The destructor waits for the task to finish.  
  
     [!code[concrt-futures#5](../parallel/codesnippet/CPP/walkthrough--implementing-futures_4.cpp)]  
...
5.  In the `public` section of the `async_future` class, implement the `get` method. This method uses the [concurrency::receive](../Topic/receive%20Function.md) function to retrieve the result of the work function.  
     ...
     [!code[concrt-futures#6](../parallel/codesnippet/CPP/walkthrough--implementing-futures_5.cpp)]  
...
### Code  
 [!code[concrt-futures#1](../parallel/codesnippet/CPP/walkthrough--implementing-futures_6.cpp)]  
...
## Robust Programming  
 ...
 [!code[concrt-futures-with-eh#1](../parallel/codesnippet/CPP/walkthrough--implementing-futures_7.cpp)]  
  
 This example produces the following output:  
  
 **caught exception: error** For more information about the exception handling model in the Concurrency Runtime, see [Exception Handling](../parallel/exception-handling-in-the-concurrency-runtime.md).  

...
---
# const_seg
Specifies the segment where [const](../cpp/const--c---.md) variables are stored in the .obj file.  
...
## Remarks  
 ...
 OBJ files can be viewed with the [dumpbin](../buildref/dumpbin-command-line.md) application. The default segment in the .obj file for `const` variables is .rdata. Some `const` variables, such as scalars, are automatically inlined into the code stream. Inlined code will not appear in .rdata.  
...
**test4**   
## Comments  
 See [/SECTION](../buildref/-section--specify-section-attributes-.md) for a list of names you should not use when creating a section.  
  
 You can also specify sections for initialized data ([data_seg](../c/data_seg.md)), uninitialized data ([bss_seg](../c/bss_seg.md)), and functions ([code_seg](../c/code_seg.md)).  

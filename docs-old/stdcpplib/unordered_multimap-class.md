...
```
template<class Key, class Ty, class Hash = std::hash<Key>, class Pred = std::equal_to<Key>, class Alloc = std::allocator<Key>>
...
### Remarks  
 ...
 For a code example, see [multimap::emplace](../stdcpplib/multimap-class.md#multimap__emplace).  
...
|`ValTy`|Template parameter that specifies the argument type that the unordered_multimap can use to construct an element of [value_type](../stdcpplib/map-class.md#map__value_type), and perfect-forwards `Val` as an argument.|  
...
### Remarks  
 ...
 The initializer list member function (6) uses an [initializer_list](../stdcpplib/-initializer_list-.md) to copy elements into the unordered_multimap.  

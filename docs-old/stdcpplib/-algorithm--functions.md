...
bool binary_search(ForwardIterator first, ForwardIterator last, const Type& value);  
 ...
 template<class ForwardIterator,  class Type,  class BinaryPredicate>  
     bool binary_search(ForwardIterator first, ForwardIterator last, const Type& value, BinaryPredicate comp);  
...
OutputIterator copy(InputIterator _First , InputIteratorLast , OutputIterator _DestBeg);  
...
```  
  ...
  For another sample showing how to use copy, see [accumulate, copy, and vector::push_back](../notintoc/accumulate--copy--and-vector--push_back.md).  
...
BidirectionalIterator2 copy_backward(BidirectionalIterator1 _First, BidirectionalIterator1Last, BidirectionalIterator2 _DestEnd);  
...
```  
template<class InputIterator, class Type> typename iterator_traits<InputIterator>::difference_type   
count(InputIterator _First , InputIteratorLast , const Type& _Val);  
...
### Remarks  
 ...
 This algorithm is generalized to count elements that satisfy any predicate with the template function [count_if](../stdcpplib/-algorithm--functions.md#count_if).  
...
count_if(InputIterator _First , InputIteratorLast, Predicate  _Pred);  
...
### Remarks  
 This template function is a generalization of the algorithm [count](../stdcpplib/-algorithm--functions.md#count), replacing the predicate "equals a specific value" with any predicate.  
...
### Remarks  
 The first iterator of the pair returned by the algorithm is [lower_bound](../stdcpplib/-algorithm--functions.md#lower_bound), and the second iterator is [upper_bound](../stdcpplib/-algorithm--functions.md#upper_bound).  
...
```  
template<class ForwardIterator, class Type>  
void fill(ForwardIterator _First, ForwardIteratorLast, const Type& _Val);  
```  
...
OutputIterator fill_n(OutputIterator First, Size Count, const Type& Val);   
```  
...
### Remarks  
 ...
 For a code example using `find()`, see [find_if](../stdcpplib/-algorithm--functions.md#find_if).  
...
### Remarks  
 This template function is a generalization of the algorithm [find](../stdcpplib/-algorithm--functions.md#find), replacing the predicate "equals a specific value" with any predicate. For the logical opposite (find the first element that does not satisfy the predicate), see [find_if_not](../stdcpplib/-algorithm--functions.md#find_if_not).  
...
### Remarks  
 This template function is a generalization of the algorithm [find](../stdcpplib/-algorithm--functions.md#find), replacing the predicate "equals a specific value" with any predicate. For the logical opposite (find the first element that does satisfy the predicate), see [find_if](../stdcpplib/-algorithm--functions.md#find_if).  
  
 For a code example readily adaptable to `find_if_not()`, see [find_if](../stdcpplib/-algorithm--functions.md#find_if).  
...
Function for_each( InputIterator _First, InputIteratorLast, Function _Func );  
...
void generate(ForwardIterator _First, ForwardIteratorLast, Generator _Gen);  
...
### Remarks  
 The first template function returns [is_heap_until](../stdcpplib/-algorithm--functions.md#is_heap_until)`(``_First``,` `_Last``) ==` `_Last`.  
...
-   Elements may be added or removed in logarithmic time.  
 ...
 Heaps are an ideal way to implement priority queues and they are used in the implementation of the Standard Template Library container adaptor [priority_queue Class](../stdcpplib/priority_queue-class.md).  
...
### Remarks  
 The `min` algorithm is unusual in having objects passed as parameters. Most Standard Template Library algorithms operate on a range of elements whose position is specified by iterators passed as parameters. If you need a function that uses a range of elements, use [min_element](../stdcpplib/-algorithm--functions.md#min_element).  
...
### Return Value  
 ...
 `(` [min_element](../stdcpplib/-algorithm--functions.md#min_element)( `_First`, `_Last`), [max_element](../stdcpplib/-algorithm--functions.md#max_element)( `_First`, `_Last`)).  
...
### Remarks  
 The template function evaluates `*(``_Dest` `+ N) =`  move `(*(``_First` `+ N)))` once for each `N` in the range `[0,` `_Last` `-` `_First``)`, for strictly increasing values of `N` starting with the lowest value. It then returns `_Dest` `+ N`. If `_Dest`and `_First` designate regions of storage, `_Dest` must not be in the range `[``_First``,` `_Last``)`.  
...
```  
template<class InputIterator, class BinaryPredicate>  
bool none_of(InputIterator _First, InputIteratorLast, BinaryPredicate _Comp);  
```  
...
##  <a name="random_shuffle"></a>  random_shuffle  
 The std::random_shuffle() function is deprecated, replaced by [std::shuffle()](../stdcpplib/-algorithm--functions.md#std__shuffle). For a code example and more information, see [\<random>](../stdcpplib/-random-.md) and the Stackoverflow posting                 [Why are std::random_shuffle methods being deprecated in C++14?](http://go.microsoft.com/fwlink/?LinkId=397954).  
...
template<class InputIterator, class OutputIterator, class Type>  
 OutputIterator remove_copy(InputIterator _First, InputIteratorLast, OutputIterator _Result, const Type& _Val);  
...
### Remarks  
 ...
 For information on how these functions behave, see [Checked Iterators](../stdcpplib/checked-iterators.md).  
...
### Remarks  
 For more information, and a code sample that uses `shuffle()`, see [\<random>](../stdcpplib/-random-.md).  
...
### Parameters  
 ...
 `comp`  
 User-defined predicate function object that defines the comparison criterion to be satisfied by successive elements in the ordering. This binary predicate takes two arguments and returns `true` if the two arguments are in order and `false` otherwise. This comparator function must impose a strict weak ordering on pairs of elements from the sequence. For more information, see [Algorithms](../stdcpplib/algorithms.md).  

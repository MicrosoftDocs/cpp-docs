...
```  
 ...
 The member function [erase](../stdcpplib/container-class--erase.md) throws an exception only if a copy operation (assignment or copy construction) throws an exception.  
  
 Moreover, no exception is thrown while copying an iterator returned by a member function.  
  
 The member function [swap](../stdcpplib/container-class--swap.md) makes additional promises for *all* container classes defined by STL:  

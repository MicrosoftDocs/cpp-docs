...
## Remarks  
 The member function determines the first element `X` in the controlled sequence that hashes to the same bucket as `key` and has equivalent ordering to `key`. If no such element exists, it returns [hash_multimap::end (STL/CLR)](../cli/hash_multimap--end--stl-clr-.md)`()`; otherwise it returns an iterator that designates `X`. You use it to locate the beginning of a sequence of elements currently in the controlled sequence that match a specified key.  
...
## See Also  
 ...
 [hash_multimap::count (STL/CLR)](../cli/hash_multimap--count--stl-clr-.md)   
 [hash_multimap::equal_range (STL/CLR)](../cli/hash_multimap--equal_range--stl-clr-.md)   
 [hash_multimap::find (STL/CLR)](../cli/hash_multimap--find--stl-clr-.md)   

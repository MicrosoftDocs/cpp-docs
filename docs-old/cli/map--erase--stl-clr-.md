...
## Remarks  
 The first member function removes the element of the controlled sequence pointed to by `where`, and returns an iterator that designates the first element remaining beyond the element removed, or [map::end (STL/CLR)](../cli/map--end--stl-clr-.md)`()` if no such element exists. You use it to remove a single element.  
...
## See Also  
 ...
 [map::clear (STL/CLR)](../cli/map--clear--stl-clr-.md)
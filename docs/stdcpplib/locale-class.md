...
```  
 ...
 to construct a locale object that has no facets. It is also a transparent locale; if the template functions [has_facet](../stdcpplib/-locale--functions.md#has_facet) and [use_facet](../stdcpplib/-locale--functions.md#use_facet) cannot find the requested facet in a transparent locale, they consult first the global locale and then, if that is transparent, the classic locale. Thus, you can write:  
...
private:    facet(const facet&)           // not defined    void operator=(const facet&)  // not defined    };
```  
...
private:    id(const id&)              // not defined    void operator=(const id&)  // not defined    };
```  
...
### Remarks  
 ...
 If a locale name `_Locname` is a null pointer or otherwise invalid, the function throws [runtime_error](../stdcpplib/runtime_error-class.md).  

...
## Remarks  
 ...
 Include the STL standard header \<list> to define the [container](../stdcpplib/stl-containers.md) template class list and several supporting templates.  
...
```  
reference back( ); const _ reference back( ) const;  
...
### Remarks  
 ...
 When compiling with _SECURE_SCL 1, a runtime error will occur if you attempt to access an element in an empty list.  See [Checked Iterators](../stdcpplib/checked-iterators.md) for more information.  
...
```  
const _ iterator begin( ) const; iterator begin( );  
...
```unstlib  
typedef typename Allocator::const _ pointer const _ pointer;  
...
```  
typedef typename Allocator::const _ reference const _ reference;  
```  
...
```  
const _ reverse _ iterator rbegin( ) const;  
...
```  
const _ reverse _ iterator rend( ) const;  
...
```  
const _ iterator end( ) const; iterator end( );  
...
```  
reference front( ); const _ reference front( ) const;  
...
```  
Allocator get _ allocator( ) const;  
...
|-|-|  
|Parameter|Description|  
|`_Right`|The list being copied into the `list`.|  
...
```  
void push _ back( void push_back(    Type&&  _Val );  
...
```  
void push _ front(    const Type&  _Val ); void push_front(    Type&& _Val );  
...
```unstlib  
const _ reverse _ iterator rbegin( ) const; reverse _ iterator rbegin( );  
...
```unstlib  
template<class Predicate> void remove _ if(    Predicate  _Pred )  
...
```unstlib  
const _ reverse _ iterator rend( ) const; reverse _ iterator rend( );  
...
```unstlib  
void resize(    size _ type  _Newsize ); void resize(    size _ type  _Newsize ,    Type  _Val );  
...
### Remarks  
 ...
 If the ranged splice inserts `N` elements, and `&Source != this`, an object of class [iterator](../stdcpplib/forward_list-class.md#forward_list__iterator) is incremented `N` times.  
...
```unstlib  
void swap(    list<Type, Allocator>&  _Right ); friend void swap(    list<Type, Allocator>&  _Left ,    list<Type, Allocator>&  _Right )  

...
## Remarks  
 An object of type `error_condition` stores an error code value and a pointer to an object that represents a [category](../stdcpplib/error_category-class.md) of error codes used for reported user-defined errors.  
...
### Remarks  
 The member operator stores `(value_type)error` as the error code value and a pointer to the [generic_category](../stdcpplib/-system_error--functions.md#generic_category). It returns `*this`.  

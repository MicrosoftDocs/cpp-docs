...
### Remarks  
 `make_pair` converts object of type [reference_wrapper Class](../stdcpplib/reference_wrapper-class.md) to reference types and converts decaying arrays and functions to pointers.  
...
-   Otherwise, the returned type `T` is `decay<T>::type`. If [decay Class](../stdcpplib/decay-class.md) is not supported, the returned type `T` is the same as the input type `T`.  
...
### Example  
  For an example about how to use the helper function `make_pair` to declare and initialize a pair, see [pair Structure](../stdcpplib/pair-structure.md).  
...
##  <a name="swap"></a>  swap  
 Exchanges the elements of two [pair Structure](../stdcpplib/pair-structure.md) objects.  
  
```
template<class T, class U> void swap(pair<T, U>& left, pair<T, U>& right);
```  

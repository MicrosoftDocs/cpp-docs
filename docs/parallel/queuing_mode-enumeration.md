...
|`queuing_mode_automatic`|A queuing mode that specifies that commands be queued up on a command queue that corresponds to the [accelerator_view](../parallel/accelerator_view-class.md) object. Commands are sent to the device when [accelerator_view::flush](../Topic/accelerator_view::flush%20Method.md) is called.|  

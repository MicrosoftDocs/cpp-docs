...
## Example  
 This example uses the [Asynchronous Agents Library](../parallel/asynchronous-agents-library.md) to download files from HTTP servers. The `http_reader` class derives from [concurrency::agent](../parallel/agent-class.md) and uses message passing to asynchronously read which URL names to download.  
  
 The `http_reader` class uses the [concurrency::task_group](../Topic/task_group%20Class.md) class to concurrently read each file. Each task calls the [concurrency::Context::Oversubscribe](../Topic/Context::Oversubscribe%20Method.md) method with the `_BeginOversubscription` parameter set to `true` to enable oversubscription in the current context. Each task then uses the Microsoft Foundation Classes (MFC) [CInternetSession](../mfcref/cinternetsession-class.md) and [CHttpFile](../mfcref/chttpfile-class.md) classes to download the file. Finally, each task calls `Context::Oversubscribe` with the `_BeginOversubscription` parameter set to `false` to disable oversubscription.  
 ...
 [!code[concrt-download-oversubscription#1](../parallel/codesnippet/CPP/how-to--use-oversubscription-to-offset-latency_1.cpp)]  
...
## Robust Programming  
 ...
 [!code[concrt-download-oversubscription#2](../parallel/codesnippet/CPP/how-to--use-oversubscription-to-offset-latency_2.cpp)]  
 ...
 [!code[concrt-download-oversubscription#3](../parallel/codesnippet/CPP/how-to--use-oversubscription-to-offset-latency_3.cpp)]  

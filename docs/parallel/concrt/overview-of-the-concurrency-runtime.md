...
### Parallel Patterns Library  
 ...
 Use the Parallel Patterns Library when you have a local computation that can benefit from parallel execution. For example, you can use the [concurrency::parallel_for](concurrency-namespace-functions.md#parallel_for_function) algorithm to transform an existing `for` loop to act in parallel.  

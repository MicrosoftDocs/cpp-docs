...
1.  Declare a class that is named `semaphore`. Add `public` and `private` sections to this class.  
     ...
     [!code[concrt-cooperative-semaphore#1](../parallel/codesnippet/CPP/how-to--use-the-context-class-to-implement-a-cooperative-semaphore_1.cpp)]  
  
2.  In the `private` section of the `semaphore` class, declare a [std::atomic](../stdcpplib/atomic-structure.md) variable that holds the semaphore count and a [concurrency::concurrent_queue](../parallel/concurrent_queue-class.md) object that holds the contexts that must wait to acquire the semaphore.  
  
     [!code[concrt-cooperative-semaphore#2](../parallel/codesnippet/CPP/how-to--use-the-context-class-to-implement-a-cooperative-semaphore_2.cpp)]  
...
3.  In the `public` section of the `semaphore` class, implement the constructor. The constructor takes a `long long` value that specifies the maximum number of contexts that can concurrently hold the lock.  
     ...
     [!code[concrt-cooperative-semaphore#3](../parallel/codesnippet/CPP/how-to--use-the-context-class-to-implement-a-cooperative-semaphore_3.cpp)]  
...
4.  In the `public` section of the `semaphore` class, implement the `acquire` method. This method decrements the semaphore count as an atomic operation. If the semaphore count becomes negative, add the current context to the end of the wait queue and call the [concurrency::Context::Block](../Topic/Context::Block%20Method.md) method to block the current context.  
     ...
     [!code[concrt-cooperative-semaphore#4](../parallel/codesnippet/CPP/how-to--use-the-context-class-to-implement-a-cooperative-semaphore_4.cpp)]  
...
5.  In the `public` section of the `semaphore` class, implement the `release` method. This method increments the semaphore count as an atomic operation. If the semaphore count is negative before the increment operation, there is at least one context that is waiting for the lock. In this case, unblock the context that is at the front of the wait queue.  
     ...
     [!code[concrt-cooperative-semaphore#5](../parallel/codesnippet/CPP/how-to--use-the-context-class-to-implement-a-cooperative-semaphore_5.cpp)]  
...
## Example  
 ...
 [!code[concrt-cooperative-semaphore#6](../parallel/codesnippet/CPP/how-to--use-the-context-class-to-implement-a-cooperative-semaphore_6.cpp)]  
...
**In loop iteration 8...**  
**In loop iteration 9...**  
**In loop iteration 4...** For more information about the `concurrent_queue` class, see [Parallel Containers and Objects](../parallel/parallel-containers-and-objects.md). For more information about the `parallel_for` algorithm, see [Parallel Algorithms](../parallel/parallel-algorithms.md).  
...
## Robust Programming  
 ...
 [!code[concrt-cooperative-semaphore#7](../parallel/codesnippet/CPP/how-to--use-the-context-class-to-implement-a-cooperative-semaphore_7.cpp)]  
 ...
 [!code[concrt-cooperative-semaphore#8](../parallel/codesnippet/CPP/how-to--use-the-context-class-to-implement-a-cooperative-semaphore_8.cpp)]  
  
## See Also  
 [Contexts](../parallel/contexts.md)   

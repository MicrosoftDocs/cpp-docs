...
- Pass the underlying pointer or a reference to the underlying object. This enables the callee to use the object, but doesn't enable it to share ownership or extend the lifetime. Giving a wrong case, if the callee creates a `shared_ptr` from the raw pointer, the new `shared_ptr` has an independent reference count, which could lead to resource release when exiting the callee, making the original 'shared_ptr' broken. Use this option when the contract between the caller and callee clearly specifies that the caller retains ownership of the `shared_ptr` lifetime.

...
## Operator Keyword for ~  
 The `compl` operator is the text equivalent of `~`. There are two ways to access the `compl` operator in your programs: include the header file `iso646.h`, or compile with [/Za](../buildref/-za---ze--disable-language-extensions-.md).  
...
```  
 ...
 Integral promotion is performed on integral operands, and the resultant type is the type to which the operand is promoted. See [Integral Promotions](../notintoc/integral-promotions.md) for more information on how the promotion is done.  

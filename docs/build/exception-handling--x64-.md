---
title: "Exception Handling (x64)"
ms.custom: na
ms.date: "10/14/2016"
ms.prod: "visual-studio-dev14"
ms.reviewer: na
ms.suite: na
ms.technology: 
  - "devlang-cpp"
ms.tgt_pltfrm: na
ms.topic: "article"
dev_langs: 
  - "C++"
helpviewer_keywords: 
  - "C++ exception handling, x64"
  - "exception handling, x64"
ms.assetid: 41fecd2d-3717-4643-b21c-65dcd2f18c93
caps.latest.revision: 10
ms.author: "corob"
manager: "ghogen"
translation.priority.ht: 
  - "cs-cz"
  - "de-de"
  - "es-es"
  - "fr-fr"
  - "it-it"
  - "ja-jp"
  - "ko-kr"
  - "pl-pl"
  - "pt-br"
  - "ru-ru"
  - "tr-tr"
  - "zh-cn"
  - "zh-tw"
---
# Exception Handling (x64)
This section discusses structured exception handling and C++ exception handling behavior on the [!INCLUDE[vcprx64](../build/includes/vcprx64_md.md)].  
  
-   [Unwind Data for Exception Handling, Debugger Support](../build/unwind-data-for-exception-handling--debugger-support.md)  
  
-   [The Unwind Procedure](../build/unwind-procedure.md)  
  
-   [The Language Specific Handler](../build/language-specific-handler.md)  
  
-   [Unwind Helpers for MASM](../build/unwind-helpers-for-masm.md)  
  
-   [Unwind Data Definitions in C](../build/unwind-data-definitions-in-c.md)  
  
 For more information, see [Exception Handling](../cpp/exception-handling-in-visual-c--.md).  
  
## See Also  
 [x64 Software Conventions](../build/x64-software-conventions.md)
...
|**/Wv** `:version`|Suppress warnings introduced in compiler versions newer than `version`. You can use this option to determine if there are new warnings in code that compiled without warnings when using an older version of the compiler, and to temporarily suppress the new warnings from your build process while you fix them. The optional parameter `version` takes the form `nn`[.`mm`[.`bbbbb`]] where `nn` is the major version number, `mm` is the optional minor version number, and `bbbbb` is the optional build number of the compiler. For example, use `/Wv:17.00.00000` to suppress warnings introduced in Visual C++ 2012 and later. By default, **/Wv** uses the current compiler version number, and no warnings are suppressed. For information about which warnings are suppressed by compiler version, see [Compiler Warnings by compiler version](../..//error-messages/compiler-warnings/compiler-warnings-by-compiler-version.md).|  
...
|**/wd** `n`|Suppresses the compiler warning that is specified by `n`.<br /><br /> For example, `/wd4326` suppresses compiler warning C4326.|  

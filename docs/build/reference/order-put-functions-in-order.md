...
When you compile your source code into an object file, you can tell the compiler to put each function into its own section, called a *COMDAT*, by using the [/Gy (Enable function-level linking)](../../build/gy-enable-function-level-linking.md) compiler option. The **/ORDER** linker option tells the linker to place COMDATs into the executable image in the order you specify.

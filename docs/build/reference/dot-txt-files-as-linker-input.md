...
LINK expects various text files as additional input. The [command-file](link-command-files.md) specifier (**\@**) and the [Base Address](base-base-address.md) (/BASE), [/DEF](def-specify-module-definition-file.md), and [/ORDER](order-put-functions-in-order.md) options all specify text files. These files can have any extension, not just .txt.

...
1. Then, use `apt` commands that follow to install the required build tools on your WSL 2 distro. They ensure that you'll have:
    * A C++ compiler
    * `gdb`
    * `CMake`
    * `rsync`
...
Install these dependencies on distros that use `apt` using the following commands:
...
This walkthrough uses GCC and Ninja on Ubuntu. You'll install a recent version of CMake using Visual Studio’s CMake binary deployment during the walkthrough.

1. Open a CMake project in Visual Studio 2022 version 17.0 Preview 2, or later. Visual Studio defines a CMake project as a folder with a `CMakeLists.txt` file at the project root. In this walkthrough, you'll create a new CMake project by using the Visual Studio **CMake Project** template.

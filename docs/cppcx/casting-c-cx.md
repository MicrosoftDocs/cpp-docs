...
---
# Casting (C++-CX)
Four different cast operators apply to [!INCLUDE[wrt](../cppcx/includes/wrt-md.md)] types: [static_cast Operator](../Topic/static_cast%20Operator.md), [dynamic_cast Operator](../Topic/dynamic_cast%20Operator.md), [safe_cast Operator](../Topic/safe_cast%20\(C++%20Component%20Extensions\).md), and [reinterpret_cast Operator](../Topic/reinterpret_cast%20Operator.md). `safe_cast` and `static_cast` throw an exception when the conversion can't be performed; [static_cast Operator](../Topic/static_cast%20Operator.md) also performs compile-time type checking. `dynamic_cast` returns `nullptr` if it fails to convert the type. Although `reinterpret_cast` returns a non-null value, it might be invalid. For this reason, we recommend that you not use `reinterpret_cast` unless you know that the cast will succeed. In addition, we recommend that you not use C-style casts in your [!INCLUDE[cppwrt](../cppcx/includes/cppwrt-md.md)] ([!INCLUDE[cppwrt_short](../cppcx/includes/cppwrt-short-md.md)]) code because they are identical to `reinterpret_cast`.  
...
## reinterpret_cast  
 We recommend that you not use [reinterpret_cast](../Topic/reinterpret_cast%20Operator.md) because neither a compile-time check nor a run-time check is performed. In the worst case, a `reinterpret_cast` makes it possible for programming errors to go undetected at development time and cause subtle or catastrophic errors in your program’s behavior. Therefore, we recommend that you use `reinterpret_cast` only in those rare cases when you must cast between unrelated types and you know that the cast will succeed. An example of a rare use is to convert a [!INCLUDE[cppwrt](../cppcx/includes/cppwrt-md.md)] type to its underlying ABI type—this means that you are taking control of the reference counting for the object. To do this, we recommend that you use the [ComPtr Class](../Topic/ComPtr%20Class.md) smart pointer. Otherwise, you must specifically call Release on the interface. The following example shows how a ref class can be cast to an `IInspectable*`.  

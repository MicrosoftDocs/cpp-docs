...
Visual Studio includes both 32-bit and 64-bit hosted versions of MASM to target x64 code. Named ml64.exe, this is the assembler that accepts x64 assembler language. The MASM command-line tools are installed when you choose a C++ workload during Visual Studio installation. They are not available as a separate download. To download and install a copy of Visual Studio, see [https://www.visualstudio.com/](https://www.visualstudio.com/). If you do not want to install Visual Studio, but only want the command-line tools, see the **Build Tools for Visual Studio 2017** option on the [Visual Studio Downloads](https://www.visualstudio.com/downloads/) page.

To use MASM to build code for x64 targets on the command line, you must use a developer command prompt, which sets the required path and other environment variables. For information on how to start a developer command prompt, see [Build C/C++ code on the command line](../../build/building-on-the-command-line.md).
...
Inline assembler or use of the ASM keyword is not supported for x64 or ARM targets. To port your x86 code that uses inline assembler to x64 or ARM, you can convert your code to C++, use compiler intrinsics, or create assembler-language files. The Visual C++ compiler supports intrinsics to allow you to use special-function instructions, for example, privileged, bit scan/test, interlocked, and so on, in as close to a cross-platform manner as possible. For information on available intrinsics, see ([Compiler Intrinsics](../../intrinsics/compiler-intrinsics.md)).  

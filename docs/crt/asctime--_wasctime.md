...
---
# asctime, _wasctime
Convert a `tm` time structure to a character string. More secure versions of these functions are available; see [asctime_s, _wasctime_s](../crt/asctime_s--_wasctime_s.md).  
...
## Remarks  
 More secure versions of these functions are available; see [asctime_s, _wasctime_s](../crt/asctime_s--_wasctime_s.md).  
...
|`tm_year`|Year (current year minus 1900)|  
 ...
 The converted character string is also adjusted according to the local time zone settings. For information about configuring the local time, see the [time](../crt/time--_time32--_time64.md), [_ftime](../crt/_ftime--_ftime32--_ftime64.md), and [localtime](../crt/localtime--_localtime32--_localtime64.md) functions and the [_tzset](../crt/_tzset.md) function for information about defining the time zone environment and global variables.  
 ...
 These functions validate their parameters. If `timeptr` is a null pointer, or if it contains out-of-range values, the invalid parameter handler is invoked, as described in [Parameter Validation](../crt/parameter-validation.md). If execution is allowed to continue, the function returns `NULL` and sets `errno` to `EINVAL`.  

...
Find the next name, if any, that matches the `filespec` argument in a previous call to [_findfirst](../crt/9bb46d1a-b946-47de-845a-a0b109a33ead.md), and then alter the `fileinfo` structure contents accordingly.  
...
## Remarks  
 You must call [_findclose](../crt/_findclose.md) after you are finished using either the `_findfirst` or `_findnext` function (or any variants). This frees up resources used by these functions in your application.  

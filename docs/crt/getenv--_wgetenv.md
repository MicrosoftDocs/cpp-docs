...
---
# getenv, _wgetenv
Gets a value from the current environment. More secure versions of these functions are available; see [getenv_s, _wgetenv_s](../crt/getenv_s--_wgetenv_s.md).  
...
## Remarks  
 ...
 If `varname` is `NULL`, this function invokes an invalid parameter handler, as described in [Parameter Validation](../crt/parameter-validation.md). If execution is allowed to continue, this function sets `errno` to `EINVAL` and returns `NULL`.  
...
|`_tgetenv`|`getenv`|`getenv`|`_wgetenv`|  
 ...
 To check or change the value of the `TZ` environment variable, use `getenv`, `_putenv` and `_tzset` as necessary. For more information about `TZ`, see [_tzset](../crt/_tzset.md) and [_daylight, timezone, and _tzname](../crt/_daylight--_dstbias--_timezone--and-_tzname.md).  

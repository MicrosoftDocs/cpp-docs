...
## Remarks  
 ...
 If an error has been assigned a type of –1 by using [_RTC_SetErrorType](../crt/_rtc_seterrortype.md), the error handling function is not called.  
 ...
 **typedef int (__cdecl \*_RTC_error_fnW)(int**  `errorType` **, const wchar_t \*** *filename* **, int**  *linenumber* **, const wchar_t \*** `moduleName` **, const wchar_t \*** *format* **, ...);**  
 ...
 `errorType`  
 The type of error that's specified by [_RTC_SetErrorType](../crt/_rtc_seterrortype.md).  

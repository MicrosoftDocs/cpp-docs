...
## Remarks  
 ...
 If `str` is a null pointer or is `count` is 0, these functions invoke the invalid parameter handler as described in [Parameter Validation](../crt/parameter-validation.md), `errno` is set to `EINVAL`, and the function returns 0.  

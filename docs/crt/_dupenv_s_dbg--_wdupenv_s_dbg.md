...
Get a value from the current environment.  Versions of [_dupenv_s, _wdupenv_s](../crt/_dupenv_s--_wdupenv_s.md) that allocate memory with [_malloc_dbg](../crt/_malloc_dbg.md) to provide additional debugging information.  
...
## Remarks  
 The `_dupenv_s_dbg` and `_wdupenv_s_dbg` functions are identical to `_dupenv_s` and `_wdupenv_s` except that, when `_DEBUG` is defined, these functions use the debug version of [malloc](../crt/malloc.md), [_malloc_dbg](../crt/_malloc_dbg.md), to allocate memory for the value of the environment variable. For information on the debugging features of `_malloc_dbg`, see [_malloc_dbg](../crt/_malloc_dbg.md).  

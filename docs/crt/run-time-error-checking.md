...
|--------------|---------|-------------------------------|  
|[_RTC_GetErrDesc](../crt/_rtc_geterrdesc.md)|Returns a brief description of a run-time error check type.||  
|[_RTC_NumErrors](../crt/_rtc_numerrors.md)|Returns the total number of errors that can be detected by run-time error checks.||  
|[_RTC_SetErrorFunc](../crt/_rtc_seterrorfunc.md)|Designates a function as the handler for reporting run-time error checks.||  
|[_RTC_SetErrorType](../crt/_rtc_seterrortype.md)|Associates an error that is detected by run-time error checks with a type.||  
  
## See Also  
 [Run-Time Routines by Category](../crt/run-time-routines-by-category.md)   
 [/RTC (Run-Time Error Checks)](../buildref/-rtc--run-time-error-checks-.md)   
 [runtime_checks](../c/runtime_checks.md)   

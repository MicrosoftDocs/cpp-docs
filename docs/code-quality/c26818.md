...
This check covers the missing `default` label in switch statements which are switching over a type that is not an enum (i.e., int, char, ...).

## Example

Missing `default` label when switching over an `int`.
...
To fix this issue, insert a `default` label covering all remaining cases.
```cpp
void fn1();
void fn2();
void default_action();
...
```
If no default action needs to be taken, insert an empty `default` label to indicate that the other cases have not been forgotten.

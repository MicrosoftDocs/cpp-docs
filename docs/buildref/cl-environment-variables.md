...
-   LIBPATH, which specifies directories to search for metadata files referenced with [#using](../c/sharpusing-directive--c---.md). See `#using` for more information on LIBPATH.  
...
```  
 ...
 For details on the arguments to the CL and _CL\_ environment variables, see [Compiler Command-Line Syntax](../buildref/compiler-command-line-syntax.md).  

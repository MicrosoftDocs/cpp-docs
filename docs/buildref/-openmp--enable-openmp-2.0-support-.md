...
Causes the compiler to process `#pragma`[omp](../c/omp.md).  
...
## Remarks  
 `#pragma omp` is used to specify [Directives](../parallel/openmp-directives.md) and [Clauses](../parallel/openmp-clauses.md). If **/openmp** is not specified in a compilation, the compiler ignores OpenMP clauses and directives. [OpenMP Function](../parallel/openmp-functions.md) calls are processed by the compiler even if **/openmp** is not specified.  
  
 An application compiled with **/openmp** and using the [Libraries](../parallel/openmp-libraries.md) can only be run on Windows 2000 or later operating systems.  

...
-   The values of `major` and `minor` must be from 0 through 65,535.  
 ...
 The choice of subsystem affects the default starting address for the program. For more information, see [/ENTRY (Entry-Point Symbol)](../buildref/-entry--entry-point-symbol-.md), the linker /ENTRY:*function* option.  
  
 For more information, including the minimum and default values for the major and minor version numbers for each subsystem, see the [/SUBSYSTEM](../buildref/-subsystem--specify-subsystem-.md) linker option.  

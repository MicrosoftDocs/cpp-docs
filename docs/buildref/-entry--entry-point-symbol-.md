...
|**_DllMainCRTStartup**|A DLL; calls `DllMain`, which must be defined with `__stdcall`, if it exists|  
 ...
 If the [/DLL](../buildref/-dll--build-a-dll-.md) or [/SUBSYSTEM](../buildref/-subsystem--specify-subsystem-.md) option is not specified, the linker selects a subsystem and entry point depending on whether **main** or `WinMain` is defined.  

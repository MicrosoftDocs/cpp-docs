...
## Remarks  
 ...
 `file`  
 An image file built with [/clr](../buildref/-clr--common-language-runtime-compilation-.md).  
...
-   3 -- /clr:pure (image is MSIL only, but only able to run on x86 platforms).  
 ...
 You can also programmatically check if an image was built for the common language runtime.  For more information, see [How to: Determine if an Image is Native or CLR](../cli/how-to--determine-if-an-image-is-native-or-clr.md).  

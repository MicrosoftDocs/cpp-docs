...
## Remarks  
 The /DELAY option controls [delayed loading](../buildref/linker-support-for-delay-loaded-dlls.md) of DLLs:  
...
-   The UNLOAD qualifier tells the delay-load helper function to support explicit unloading of the DLL. The Import Address Table (IAT) is reset to its original form, invalidating IAT pointers and causing them to be overwritten.  
     ...
     If you do not select UNLOAD, any call to [FUnloadDelayLoadedDLL](../buildref/explicitly-unloading-a-delay-loaded-dll.md) will fail.  
  
-   The NOBIND qualifier tells the linker not to include a bindable IAT in the final image. The default is to create the bindable IAT for delay-loaded DLLs. The resulting image cannot be statically bound. (Images with bindable IATs may be statically bound prior to execution.) See [/BIND](../buildref/-bind.md).  
 ...
 To specify DLLs to delay load, use the [/DELAYLOAD](../buildref/-delayload--delay-load-import-.md) option.  

...
\*\*\* You can install the **Linux development with C++** workload in the Visual Studio installer to target Linux platforms. For instructions, see [Download, Install and Setup the Linux Workload](linux/download-install-and-setup-the-linux-workload.md). This toolset compiles your executable on the target machine, so you can build for any supported architecture.  

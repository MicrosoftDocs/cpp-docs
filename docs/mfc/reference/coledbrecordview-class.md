...
## Remarks  
 ...
 You can use DDX functions with `COleDbRecordView` to get data directly from the database recordset and display it in a dialog control. You should use the **DDX_&ast;** methods (such as `DDX_Text`), not the **DDX_Field&ast;** functions (such as `DDX_FieldText`) with `COleDbRecordView`. `DDX_FieldText` will not work with `COleDbRecordView` because `DDX_FieldText` takes an additional argument of type **CRecordset&ast;** (for `CRecordView`) or **CDaoRecordset&ast;** (for `CDaoRecordView`).  

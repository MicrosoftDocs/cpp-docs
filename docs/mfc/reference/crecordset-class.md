...
- `CRecordset::forwardOnly` A read-only recordset with only forward scrolling.
   ...
   For `CRecordset`, the default value is `CRecordset::snapshot`. The default-value mechanism allows the Visual Studio wizards to interact with both ODBC `CRecordset` and DAO `CDaoRecordset`, which have different defaults.

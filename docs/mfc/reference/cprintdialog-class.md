 ...
 *dwFlags*  
 One or more flags you can use to customize the settings of the dialog box, combined using the bitwise OR operator. For example, the PD_ALLPAGES flag sets the default print range to all pages of the document. See the [PRINTDLG](https://msdn.microsoft.com/library/windows/desktop/ms646843) structure in the Windows SDK for more information on these flags.  
...
### Remarks  
 ...
 Note that when you call the constructor with *bPrintSetupOnly* set to FALSE, the PD_RETURNDC flag is automatically used. After calling `DoModal`, `GetDefaults`, or `GetPrinterDC`, a printer DC will be returned in `m_pd.hDC`. This DC must be freed with a call to [DeleteDC](https://msdn.microsoft.com/library/windows/desktop/dd183533) by the caller of `CPrintDialog`.  
...
### Remarks  
 ...
 If the constructor for `CPrintDialog` was called with *bPrintSetupOnly* set to FALSE, this function will not only return `hDevNames` and `hDevMode` located in `m_pd.hDevNames` and `m_pd.hDevMode`) to the caller, but will also return a printer DC in `m_pd.hDC`. It is the responsibility of the caller to delete the printer DC and call the Windows [GlobalFree](https://msdn.microsoft.com/library/windows/desktop/aa366579) function on the handles when you are finished with the `CPrintDialog` object.  
...
### Remarks  
 If the *bPrintSetupOnly* parameter of the `CPrintDialog` constructor was FALSE (indicating that the Print dialog box is displayed), then `GetPrinterDC` returns a handle to the printer device context. You must call the Windows [DeleteDC](https://msdn.microsoft.com/library/windows/desktop/dd183533) function to delete the device context when you are done using it.  
...
### Remarks  
 After constructing a `CPrintDialog` object, you can use `m_pd` to set various aspects of the dialog box before calling the [DoModal](#domodal) member function. For more information on the `m_pd` structure, see [PRINTDLG](https://msdn.microsoft.com/library/windows/desktop/ms646843) in the Windows SDK.  

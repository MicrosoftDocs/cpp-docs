 ...
 *nCmdExecOpt*  
 Specifies command-execution options. By default, set to execute the command without prompting the user. See [OLECMDEXECOPT](https://msdn.microsoft.com/library/windows/desktop/ms683930) for a list of values.  
...
### Return Value  
 A pointer to the [IOleDocumentView](https://msdn.microsoft.com/library/windows/desktop/ms678455) interface of the currently active view. If there is no current view, it returns NULL.  
 ...
 *pdwStatus*  
 A pointer to the flags returned as a result of the query. For a list of possible values, see [OLECMDF](https://msdn.microsoft.com/library/windows/desktop/ms695237).  
  
 *pCmdText*  
 Pointer to an [OLECMDTEXT](https://msdn.microsoft.com/library/windows/desktop/ms693314) structure in which to return name and status information for a single command. Can be NULL to indicate that the caller does not need this information.  
  
 *pguidCmdGroup*  
 Unique identifier of the command group; can be NULL to specify the standard group.  
  
### Return Value  
 For a complete listing of return values, see [IOleCommandTarget::QueryStatus](https://msdn.microsoft.com/library/windows/desktop/ms688491) in the Windows SDK.  
  
### Remarks  
 This member function emulates the functionality of the [IOleCommandTarget::QueryStatus](https://msdn.microsoft.com/library/windows/desktop/ms688491) method, as described in the Windows SDK.  

 ...
 *hdr*  
 Identifies the tool that needs text. The only member of this structure you might need is the control's command ID. The control's command ID will be in the *idFrom* member of the **NMHDR** structure, accessed with the syntax `hdr.idFrom`. See [NMHDR](https://msdn.microsoft.com/library/windows/desktop/bb775514) for a discussion of members of the **NMHDR** structure.  

...
|Map menus to handler functions.|Use the **Events** button in the [Properties window](/visual-studio/ide/reference/properties-window) to connect menus and accelerators to handler functions in your code.|The Properties window inserts message-map entries and empty function templates into the source files you specify and manages many manual coding tasks.|  

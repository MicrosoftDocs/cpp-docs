...
This note describes the MFC [CSplitterWnd Class](../mfcref/csplitterwnd-class.md), which provides window splits and manages the resizing of other pane windows.  
...
## Splitter Examples  
 Many of the MFC sample programs use splitter windows directly or indirectly. The MFC General sample [VIEWEX](../top/visual-c---samples.md) illustrates several uses of static splitters, including how to place a splitter in a splitter.  
...
## Terminology Used by Implementation  
 ...
 Pane:  
 An application-specific window that a `CSplitterWnd` manages. A pane is usually an object that is derived from the [CView Class](../mfcref/cview-class.md), but can be any [CWnd](../mfcref/cwnd-class.md) object that has the appropriate child window ID.  

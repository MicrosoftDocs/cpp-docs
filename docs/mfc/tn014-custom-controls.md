...
Subclassing is the Windows term for replacing the <xref:System.Windows.Forms.Control.WndProc%6A> of a window with a customized `WndProc` and calling the old `WndProc` for default functionality.

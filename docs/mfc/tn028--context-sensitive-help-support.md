...
## Help Files  
 The Microsoft Foundation classes assume a single Help file. That Help file must have the same name and path as the application. For example, if the executable is C:\MyApplication\MyHelp.exe the help file must be C:\MyApplication\MyHelp.hlp. You set the path through the `m_pszHelpFilePath` member variable of the [CWinApp Class](../mfcref/cwinapp-class.md).  
...
```  
 ...
 The source code for MAKEHM is available in the MFC Programming Utilities sample [MAKEHM](../top/visual-c---samples.md).  

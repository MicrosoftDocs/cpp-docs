...
3.  In scribble.h, modify the definition for the `CScribbleApp` class so that it is derived from [CWinAppEx Class](../mfcref/cwinappex-class.md).  
...
5.  The main frame for a multiple document interface (MDI) application is no longer derived from the `CMDIFrameWnd` class. Instead, it is derived from the [CMDIFrameWndEx](../mfcref/cmdiframewndex-class.md) class.  
...
10. If you intend to link your application statically, add the following code to the start of the project resource (.rc) file.  
    ...
    ```  
     ...
     The afxribbon.rc file contains resources that are required at run time. The [MFC Application Wizard](../mfcref/mfc-application-wizard.md) includes this file automatically when you create an application.  
...
-   Use the [Resource Editors](../mfc/resource-editors.md) to invent your own bitmaps. Or use the resource editors to assemble bitmaps from the portable network graphics (.png) images that are included with [!INCLUDE[vsprvs](../build/includes/vsprvs_md.md)]. These images are in the `VS2008ImageLibrary` directory.  

...
If a `local_time` specifies a time during this hour, it isn't clear how to convert it. Should the converted time be treated as the "first" time that hour happens, or the second?  If the [`choose`](choose-enum.md) isn't specified to indicate which it should be, expect `result` to be set to `ambiguous`, and `first` and `second` will reflect the two choices for the time the `local_time` could be converted to.

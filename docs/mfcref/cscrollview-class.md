...
---
# CScrollView Class
A [CView](../mfcref/cview-class.md) with scrolling capabilities.  
...
## Remarks  
 You can handle standard scrolling yourself in any class derived from `CView` by overriding the message-mapped [OnHScroll](../mfcref/cwnd-class.md#cwnd__onhscroll) and [OnVScroll](../mfcref/cwnd-class.md#cwnd__onvscroll) member functions. But `CScrollView` adds the following features to its `CView` capabilities:  
...
-   It scrolls automatically in response to messages from the keyboard, a non-scrolling mouse, or the IntelliMouse wheel.  
 ...
 You can handle mouse wheel scrolling yourself by overriding the message-mapped [OnMouseWheel](../mfcref/cwnd-class.md#cwnd__onmousewheel) and [OnRegisteredMouseWheel](../mfcref/cwnd-class.md#cwnd__onregisteredmousewheel) member functions. As they are for `CScrollView`, these member functions support the recommended behaviour for                 [WM_MOUSEWHEEL](http://msdn.microsoft.com/library/windows/desktop/ms645617), the wheel rotation message.  
  
 To take advantage of automatic scrolling, derive your view class from `CScrollView` instead of from `CView`. When the view is first created, if you want to calculate the size of the scrollable view based on the size of the document, call the `SetScrollSizes` member function from your override of either [CView::OnInitialUpdate](../mfcref/cview-class.md#cview__oninitialupdate) or [CView::OnUpdate](../mfcref/cview-class.md#cview__onupdate). (You must write your own code to query the size of the document. For an example, see the [Scribble sample](../top/visual-c---samples.md).)  
...
-   The framework also forwards scroll messages from a `CSplitterWnd` splitter control to the currently active splitter pane (a view). When placed in a [CSplitterWnd](../mfcref/csplitterwnd-class.md) with shared scroll bars, a `CScrollView` object will use the shared ones rather than creating its own.  
...
### Example  
 [!code[NVC_MFCDocView#164](../mfc/codesnippet/CPP/cscrollview-class_1.cpp)]  
...
### Remarks  
 ...
 [!code[NVC_MFCDocView#165](../mfc/codesnippet/CPP/cscrollview-class_2.cpp)]  
...
### Remarks  
 ...
 [!code[NVC_MFCDocView#166](../mfc/codesnippet/CPP/cscrollview-class_3.cpp)]  
  
 Alternatively, you might sometimes need to set a fixed size, as in the following code:  
  
 [!code[NVC_MFCDocView#167](../mfc/codesnippet/CPP/cscrollview-class_4.cpp)]  
...
### Example  
 [!code[NVC_MFCDocView#168](../mfc/codesnippet/CPP/cscrollview-class_5.cpp)]  
  
 [!code[NVC_MFCDocView#169](../mfc/codesnippet/CPP/cscrollview-class_6.cpp)]  
  
## See Also  
 [MFC Sample DIBLOOK](../top/visual-c---samples.md)   

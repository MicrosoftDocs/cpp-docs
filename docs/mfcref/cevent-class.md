...
## Remarks  
 ...
 To access a resource controlled by a `CEvent` object in this manner, first create a variable of either type [CSingleLock](../mfcref/csinglelock-class.md) or type [CMultiLock](../mfcref/cmultilock-class.md) in the access method of your resource. Then call the `Lock` method of the lock object (for example, [CMultiLock::Lock](../mfcref/cmultilock-class.md#cmultilock__lock)). At this point, your thread will either gain access to the resource, wait for the resource to be released and gain access, or wait for the resource to be released, time out, and fail to gain access to the resource. In any case, your resource has been accessed in a thread-safe manner. To release the resource, call `SetEvent` to signal the event object, and then use the `Unlock` method of the lock object (for example, [CMultiLock::Unlock](../mfcref/cmultilock-class.md#cmultilock__unlock)), or let the lock object fall out of scope.  
...
## Example  
 [!code[NVC_MFC_Utilities#45](../mfc/codesnippet/CPP/cevent-class_1.cpp)]  
  
 [!code[NVC_MFC_Utilities#46](../mfc/codesnippet/CPP/cevent-class_2.cpp)]  
...
### Remarks  
 To access or release a `CEvent` object, create a [CMultiLock](../mfcref/cmultilock-class.md) or [CSingleLock](../mfcref/csinglelock-class.md) object and call its [Lock](../mfcref/csinglelock-class.md#csinglelock__lock) and [Unlock](../mfcref/csinglelock-class.md#csinglelock__unlock) member functions.  

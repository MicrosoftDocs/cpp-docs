...
---
# CMemFile Class
The [CFile](../mfcref/cfile-class.md)-derived class that supports memory files.  
...
## Remarks  
 ...
 The most common use of `CMemFile` is to create a `CMemFile` object and use it by calling [CFile](../mfcref/cfile-class.md) member functions. Note that creating a `CMemFile` automatically opens it: you do not call [CFile::Open](../mfcref/cfile-class.md#cfile__open), which is only used for disk files. Because `CMemFile` doesn't use a disk file, the data member `CFile::m_hFile` is not used and has no meaning.  
  
 The `CFile` member functions [Duplicate](../mfcref/cfile-class.md#cfile__duplicate), [LockRange](../mfcref/cfile-class.md#cfile__lockrange), and [UnlockRange](../mfcref/cfile-class.md#cfile__unlockrange) are not implemented for `CMemFile`. If you call these functions on a `CMemFile` object, you will get a [CNotSupportedException](../mfcref/cnotsupportedexception-class.md).  
  
 `CMemFile` uses the run-time library functions [malloc](../crt/malloc.md), [realloc](../crt/realloc.md), and [free](../crt/free.md) to allocate, reallocate, and deallocate memory; and the intrinsic [memcpy](../crt/memcpy--wmemcpy.md) to block copy memory when reading and writing. If you'd like to change this behavior or the behavior when `CMemFile` grows a file, derive your own class from `CMemFile` and override the appropriate functions.  
  
 For more information on `CMemFile`, see the articles [Files in MFC](../mfc/files-in-mfc.md) and [Memory Management (MFC)](../mfc/memory-management.md) and see [File Handling](../crt/file-handling.md) in the                 *Run-Time Library Reference*.  
...
### Remarks  
 ...
 The default implementation uses the run-time library function [malloc](../crt/malloc.md) to allocate memory.  
...
### Remarks  
 Note that the file is opened by the constructor and that you should not call [CFile::Open](../mfcref/cfile-class.md#cfile__open).  
...
### Example  
 [!code[NVC_MFCFiles#36](../atl/codesnippet/CPP/cmemfile-class_1.cpp)]  
...
##  <a name="cmemfile__memcpy"></a>  CMemFile::Memcpy  
 This function is called by the `CMemFile` overrides of [CFile::Read](../mfcref/cfile-class.md#cfile__read) and [CFile::Write](../mfcref/cfile-class.md#cfile__write) to transfer data to and from the memory file.  

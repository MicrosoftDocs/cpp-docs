...
## Remarks  
 ...
 `COleClientItem` can be used with either the [COleDocument](../mfcref/coledocument-class.md), [COleLinkingDoc](../mfcref/colelinkingdoc-class.md), or [COleServerDoc](../mfcref/coleserverdoc-class.md) class. To use `COleClientItem`, derive a class from it and implement the [OnChange](#coleclientitem__onchange) member function, which defines how the container responds to changes made to the item. To support in-place activation, override the [OnGetItemPosition](#coleclientitem__ongetitemposition) member function. This function provides information about the displayed position of the OLE item.  
  
 For more information about using the container interface, see the articles [Containers: Implementing a Container](../mfc/containers--implementing-a-container.md) and [Activation](../mfc/activation--c---.md).  
...
### Remarks  
 If the server application was written using the Microsoft Foundation Class Library, this function causes the [OnDoVerb](../mfcref/coleserveritem-class.md#coleserveritem__ondoverb) member function of the corresponding `COleServerItem` object to be executed.  
 ...
 *clsidOld*  
 A reference to the item's current class ID. The class ID should represent the type of the actual object, as stored, unless it is a link. In that case, it should be the CLSID of the item to which the link refers. The [COleConvertDialog](../mfcref/coleconvertdialog-class.md) automatically provides the correct class ID for the item.  
...
### Remarks  
 This is called automatically by [COleConvertDialog::DoConvert](../mfcref/coleconvertdialog-class.md#coleconvertdialog__doconvert). It is not usually called directly.  
  
##  <a name="coleclientitem__attachdataobject"></a>  COleClientItem::AttachDataObject  
 Call this function to initialize a [COleDataObject](../mfcref/coledataobject-class.md) for accessing the data in the OLE item.  
...
### Parameters  
 `pDataObject`  
 Pointer to the [COleDataObject](../mfcref/coledataobject-class.md) object from which the OLE item is to be created.  
...
### Parameters  
 `pContainerDoc`  
 Pointer to the container document that will contain this item. This can be any [COleDocument](../mfcref/coledocument-class.md) derivative.  
  
### Remarks  
 If you pass a **NULL** pointer, no addition is made to the container document. You must explicitly call [COleDocument::AddItem](../mfcref/coledocument-class.md#coledocument__additem).  
...
### Remarks  
 This is called automatically by [COleConvertDialog](../mfcref/coleconvertdialog-class.md). It is not necessary to call it directly.  
...
### Remarks  
 Data transfer operations, such as pasting from the Clipboard or drag-and-drop operations, provide `COleDataObject` objects containing the information offered by a server application. It is usually used in your override of [CView::OnDrop](../mfcref/cview-class.md#cview__ondrop).  
...
### Remarks  
 The framework calls this function from [COleInsertDialog::CreateItem](../mfcref/coleinsertdialog-class.md#coleinsertdialog__createitem) if the user chooses OK from the Insert Object dialog box when the Create from File button is selected.  
...
### Remarks  
 You typically call this function from the message handler for the Paste Link command on the Edit menu. (The Paste Link command is enabled in the default implementation of [COleDocument](../mfcref/coledocument-class.md) if the Clipboard contains an OLE item that can be linked to.)  
...
### Remarks  
 Call this during a drop operation when the user indicates a link should be created. It can also be used to handle the Edit Paste command. It is called by the framework in `COleClientItem::CreateLinkFromClipboard` and in [COlePasteSpecialDialog::CreateItem](../mfcref/colepastespecialdialog-class.md#colepastespecialdialog__createitem) when the Link option has been selected.  
...
### Remarks  
 The framework calls this function if the user chooses OK from the Insert Object dialog box when the Create from File button is selected and the Link check box is checked. It is called from [COleInsertDialog::CreateItem](../mfcref/coleinsertdialog-class.md#coleinsertdialog__createitem).  
...
### Remarks  
 ...
 Used in [COlePasteSpecialDialog::CreateItem](../mfcref/colepastespecialdialog-class.md#colepastespecialdialog__createitem) when Static is selected.  
...
### Remarks  
 ...
 The delay time is specified by a registry key setting. You can change the delay time by calling [CWinApp::WriteProfileString](../mfcref/cwinapp-class.md#cwinapp__writeprofilestring) or [CWinApp::WriteProfileInt](../mfcref/cwinapp-class.md#cwinapp__writeprofileint). If you do not specify the delay time, a default value of 200 milliseconds is used. Drag delay time is stored as follows:  
...
### Parameters  
 `pDC`  
 Pointer to a [CDC](../mfcref/cdc-class.md) object used for drawing the OLE item.  
  
 `lpBounds`  
 Pointer to a [CRect](../atl/crect-class.md) object or `RECT` structure that defines the bounding rectangle in which to draw the OLE item (in logical units determined by the device context).  
...
### Remarks  
 The function may use the metafile representation of the OLE item created by the [OnDraw](../mfcref/coleserveritem-class.md#coleserveritem__ondraw) member function of `COleServerItem`.  
...
### Parameters  
 `lpSize`  
 Pointer to a **SIZE** structure or a [CSize](../atl/csize-class.md) object that will receive the size information.  
...
### Parameters  
 `pDataSource`  
 Pointer to a [COleDataSource](../mfcref/coledatasource-class.md) object that will receive the data contained in the OLE item.  
...
### Remarks  
 `GetClipboardData` is called as the default implementation of [OnGetClipboardData](#coleclientitem__ongetclipboarddata). Override `OnGetClipboardData` only if you want to offer data formats in addition to those offered by `CopyToClipboard`. Place those formats in the `COleDataSource` object before or after calling `CopyToClipboard`, and then pass the `COleDataSource` object to the [COleDataSource::SetClipboard](../mfcref/coledatasource-class.md#coledatasource__setclipboard) function. For example, if you want the OLE item's position in its container document to accompany it on the Clipboard, you would define your own format for passing that information and place it in the `COleDataSource` before calling `CopyToClipboard`.  
...
### Remarks  
 ...
 This function is called automatically by the [COleLinksDialog](../mfcref/colelinksdialog-class.md) class.  
...
-   `USERCLASSTYPE_APPNAME` Name of the application servicing the class.  
 ...
 `rString`  
 A reference to a [CString](../atl/cstringt-class.md) object to which the string describing the OLE item's type is to be returned.  
...
### Remarks  
 ...
 This is called automatically by the [COleLinksDialog](../mfcref/colelinksdialog-class.md) implementation.  
...
### Remarks  
 It is used to determine when to draw the object with a hatching pattern. An open object should have a hatch pattern drawn on top of the object. You can use a [CRectTracker](../mfcref/crecttracker-class.md) object to accomplish this.  
...
### Remarks  
 ...
 Override this function to apply special rules to the resize/move operation. If the application is written in MFC, this call results because the server called [COleServerDoc::RequestPositionChange](../mfcref/coleserverdoc-class.md#coleserverdoc__requestpositionchange).  
...
### Remarks  
 ...
 If the server was written with the Microsoft Foundation Class Library, the server can cause this function to be called by calling [COleServerDoc::DiscardUndoState](../mfcref/coleserverdoc-class.md#coleserverdoc__discardundostate).  
...
### Return Value  
 A pointer to a [COleDataSource](../mfcref/coledatasource-class.md) object containing the Clipboard data.  
...
### Parameters  
 *rClipRect*  
 Pointer to an object of class [CRect](../atl/crect-class.md) that will hold the clipping-rectangle coordinates of the item.  
...
### Parameters  
 `rPosition`  
 Reference to the [CRect](../atl/crect-class.md) object that will contain the item's position coordinates.  
...
### Remarks  
 ...
 If the container is an MDI application, the default implementation returns a pointer to the [CMDIFrameWnd](../mfcref/cmdiframewnd-class.md) object in `ppMainFrame` and a pointer to the active [CMDIChildWnd](../mfcref/cmdichildwnd-class.md) object in `ppDocFrame`. If the container is an SDI application, the default implementation returns a pointer to the [CFrameWnd](../mfcref/cframewnd-class.md) object in `ppMainFrame` and returns **NULL** in `ppDocFrame`. The default implementation also fills in the members of `lpFrameInfo`.  
...
### Remarks  
 ...
 If the server application is written with the Microsoft Foundation Class Libraries, this function causes the server to call [COleServerDoc::OnReactivateAndUndo](../mfcref/coleserverdoc-class.md#coleserverdoc__onreactivateandundo).  
...
### Parameters  
 `size`  
 A [CSize](../atl/csize-class.md) object that contains the size information.  
...
## See Also  
 [MFC Sample MFCBIND](../top/visual-c---samples.md)   

...
## Remarks  
 A property sheet consists of a `CPropertySheet` object and one or more [CPropertyPage](../mfcref/cpropertypage-class.md) objects. The framework displays a property sheet as a window with a set of tab indices and an area that contains the currently selected page. The user navigates to a specific page by using the appropriate tab.  
  
 `CPropertySheet` provides support for the expanded                 [PROPSHEETHEADER](http://msdn.microsoft.com/library/windows/desktop/bb774546) structure introduced in [!INCLUDE[Win98](../mfcref/includes/win98_md.md)] and Windows NT 2000. The structure contains additional flags and members that support using a "watermark" background bitmap.  
...
### Remarks  
 ...
 `AddPage` adds the [CPropertyPage](../mfcref/cpropertypage-class.md#cpropertypage__cpropertypage) object to the `CPropertySheet` object's list of pages but does not actually create the window for the page. The framework postpones creation of the window for the page until the user selects that page.  
  
 When you add a property page using `AddPage`, the `CPropertySheet` is the parent of the `CPropertyPage`. To gain access to the property sheet from the property page, call [CWnd::GetParent](../mfcref/cwnd-class.md#cwnd__getparent).  
...
### Example  
 [!code[NVC_MFCDocView#129](../mfc/codesnippet/CPP/cpropertysheet-class_1.cpp)]  
...
### Example  
 ...
 [!code[NVC_MFCDocView#130](../mfc/codesnippet/CPP/cpropertysheet-class_2.cpp)]  
...
### Example  
 [!code[NVC_MFCDocView#131](../mfc/codesnippet/CPP/cpropertysheet-class_3.cpp)]  
...
### Parameters  
 ...
 `dwStyle`  
 Window styles for property sheet. For a complete list of available styles, see [Window Styles](../mfcref/window-styles.md).  
  
 `dwExStyle`  
 Extended window styles for property sheet. For a complete list of available styles, see [Extended Window Styles](../mfcref/extended-window-styles.md)  
...
### Remarks  
 ...
 The **Create** member function returns immediately after creating the property sheet. To destroy the property sheet, call [CWnd::DestroyWindow](../mfcref/cwnd-class.md#cwnd__destroywindow).  
...
### Example  
 [!code[NVC_MFCDocView#132](../mfc/codesnippet/CPP/cpropertysheet-class_4.cpp)]  
  
 [!code[NVC_MFCDocView#133](../mfc/codesnippet/CPP/cpropertysheet-class_5.cpp)]  
...
### Remarks  
 The return value corresponds to the ID of the control that closed the property sheet. After this function returns, the windows corresponding to the property sheet and all the pages will have been destroyed. The objects themselves will still exist. Typically, you will retrieve data from the [CPropertyPage](../mfcref/cpropertypage-class.md) objects after `DoModal` returns `IDOK`.  
...
>  This exception must be handled by the operating system if you are compiling with the asynchronous exception handling model. For more information about exception handling models, see [/EH (Exception Handling Model)](../buildref/-eh--exception-handling-model-.md). In this case, do not wrap calls to `CPropertySheet::DoModal` with a C++ try-catch block in which the catch handles all exceptions, for example, `catch (...)`. This block would handle the exception intended for the operating system, and cause unpredictable behavior. However, you can safely use C++ exception handling with specific exception types or structured exception handling where the Access Violation exception is passed through to the operating system.  
 ...
 To avoid generating this first-chance exception, you can manually guarantee that the property sheet has the correct [Window Styles](../mfcref/window-styles.md). You need to set the following styles for a property sheet:  
...
### Remarks  
 ...
 You must call `EnableStackedTabs` when you create a modal or a modeless property sheet. To incorporate this style in a `CPropertySheet`-derived class, write a message handler for `WM_CREATE`. In the overridden version of [CWnd::OnCreate](../mfcref/cwnd-class.md#cwnd__oncreate), call **EnableStackedTabs( FALSE )** before calling the base class implementation.  
  
### Example  
 [!code[NVC_MFCDocView#134](../mfc/codesnippet/CPP/cpropertysheet-class_6.cpp)]  
...
### Example  
 [!code[NVC_MFCDocView#135](../mfc/codesnippet/CPP/cpropertysheet-class_7.cpp)]  
...
### Example  
  See the example for [CPropertyPage::OnWizardFinish](../mfcref/cpropertypage-class.md#cpropertypage__onwizardfinish).  
...
##  <a name="cpropertysheet__gettabcontrol"></a>  CPropertySheet::GetTabControl  
 Retrieves a pointer to a tab control to do something specific to the tab control (that is, to use any of the APIs in [CTabCtrl](../mfcref/ctabctrl-class.md)).  
...
### Example  
 [!code[NVC_MFCDocView#136](../mfc/codesnippet/CPP/cpropertysheet-class_8.cpp)]  
...
### Example  
 [!code[NVC_MFCDocView#143](../mfc/codesnippet/CPP/cpropertysheet-class_9.cpp)]  
...
### Parameters  
 `lpRect`  
 Points to a                                 [RECT](http://msdn.microsoft.com/library/windows/desktop/dd162897) structure or [CRect](../atl/crect-class.md) object that contains the dialog-box coordinates to be converted.  
...
### Remarks  
 ...
 A call to `PressButton` will not send the                         [PSN_APPLY](http://msdn.microsoft.com/library/windows/desktop/bb774552) notification from a property page to the framework. To send this notification, call [CPropertyPage::OnOK](../mfcref/cpropertypage-class.md#cpropertypage__onok).  
  
### Example  
 [!code[NVC_MFCDocView#137](../mfc/codesnippet/CPP/cpropertysheet-class_10.cpp)]  
...
### Remarks  
 The [CPropertyPage](../mfcref/cpropertypage-class.md) object itself is not destroyed until the owner of the `CPropertySheet` window is closed.  
...
### Example  
 [!code[NVC_MFCDocView#138](../mfc/codesnippet/CPP/cpropertysheet-class_11.cpp)]  
...
### Example  
 [!code[NVC_MFCDocView#139](../mfc/codesnippet/CPP/cpropertysheet-class_12.cpp)]  
...
### Remarks  
 Call `SetWizardButtons` only after the dialog is open; you can't call `SetWizardButtons` before you call [DoModal](#cpropertysheet__domodal). Typically, you should call `SetWizardButtons` from [CPropertyPage::OnSetActive](../mfcref/cpropertypage-class.md#cpropertypage__onsetactive).  
...
### Example  
 ...
 [!code[NVC_MFCDocView#140](../mfc/codesnippet/CPP/cpropertysheet-class_13.cpp)]  
  
 [!code[NVC_MFCDocView#141](../mfc/codesnippet/CPP/cpropertysheet-class_14.cpp)]  
...
### Example  
 [!code[NVC_MFCDocView#142](../mfc/codesnippet/CPP/cpropertysheet-class_15.cpp)]  

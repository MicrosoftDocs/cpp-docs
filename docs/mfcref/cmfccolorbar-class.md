...
|`m_BoxSize`|A [CSize](../atl/csize-class.md) object that specifies the size of a cell in a color bar grid.|  
...
|`m_bStdColorDlg`|A Boolean that indicates whether to show the standard system color dialog box or the [CMFCColorDialog](../mfcref/cmfccolordialog-class.md) dialog box. For more information, see [CMFCColorBar::EnableOtherButton](#cmfccolorbar__enableotherbutton).|  
...
|`m_ColorNames`|An [CMap](../mfcref/cmap-class.md) object that associates a set of RGB colors with their names.|  
|`m_colors`|A [CArray](../mfcref/carray-class.md) of                                         [COLORREF](http://msdn.microsoft.com/library/windows/desktop/dd183449) values that contains the colors that are displayed in the color bar control.|  
...
|`m_lstDocColors`|A [CList](../mfcref/clist-class.md) of                                         [COLORREF](http://msdn.microsoft.com/library/windows/desktop/dd183449) values that contains the colors that are currently used in a document.|  
...
|`m_Palette`|A [CPalette](../mfcref/cpalette-class.md) of the colors that are used in the color bar control.|  
|`m_pParentBtn`|A pointer to a [CMFCColorButton](../mfcref/cmfccolorbutton-class.md) object that is the parent of the current button. This value is significant if the color button is in a hierarchy of toolbar controls or is in a color property grid control.|  
|`m_pParentRibbonBtn`|A pointer to a [CMFCRibbonColorButton](../mfcref/cmfcribboncolorbutton-class.md) object that is on the ribbon and is the parent button of the current button. This value is significant if the color button is in a hierarchy of toolbar controls or is in a color property grid control.|  
|`m_pWndPropList`|A pointer to a [CMFCPropertyGridCtrl](../mfcref/cmfcpropertygridctrl-class.md) object.|  
|`m_strAutoColor`|A [CString](../atl/cstringt-class.md) that is the text that is displayed on the **Automatic** button. For more information, see [CMFCColorBar::EnableAutomaticButton](#cmfccolorbar__enableautomaticbutton).|  
|`m_strDocColors`|A [CString](../atl/cstringt-class.md) that is the text that is displayed on the document colors button. For more information, see [CMFCColorBar::SetDocumentColors](#cmfccolorbar__setdocumentcolors).|  
|`m_strOtherColor`|A [CString](../atl/cstringt-class.md) that is the text that is displayed on the *other* button. For more information, see [CMFCColorBar::EnableOtherButton](#cmfccolorbar__enableotherbutton).|  
  
## Remarks  
 Usually, you do not create a `CMFCColorBar` object directly. Instead, the [CMFCColorMenuButton Class](../mfcref/cmfccolormenubutton-class.md) (used in menus and toolbars) or the [CMFCColorButton Class](../mfcref/cmfccolorbutton-class.md) creates the `CMFCColorBar` object.  
...
-   Uses the [CMFCColorPickerCtrl Class](../mfcref/cmfccolorpickerctrl-class.md) control to select a custom color.  
  
-   Supports a "tear-off" state (if it is created by using the [CMFCColorMenuButton Class](../mfcref/cmfccolormenubutton-class.md)).  
...
2.  In your frame window class, override the [CFrameWndEx::OnShowPopupMenu](../mfcref/cframewndex-class.md#cframewndex__onshowpopupmenu) method and replace the regular menu button with a [CMFCColorMenuButton Class](../mfcref/cmfccolormenubutton-class.md) object (by calling [CMFCToolBar::ReplaceButton](../mfcref/cmfctoolbar-class.md#cmfctoolbar__replacebutton)).  
  
3.  Set all the styles and enable or disable the features of the `CMFCColorBar` object during [CMFCColorMenuButton Class](../mfcref/cmfccolormenubutton-class.md) creation. The `CMFCColorMenuButton` object dynamically creates the `CMFCColorBar` object after the framework calls the `CreatePopupMenu` method.  
  
 When the user clicks a color bar control button, the framework uses the `ON_COMMAND` macro to notify the parent of the color bar control. In the macro, the command ID parameter is the value that you assigned to the color bar control button in step 1 (ID_CHAR_COLOR in this example). For more information, see the [CMFCColorMenuButton Class](../mfcref/cmfccolormenubutton-class.md), [CMFCColorButton Class](../mfcref/cmfccolorbutton-class.md), [CMFCColorPickerCtrl Class](../mfcref/cmfccolorpickerctrl-class.md), [CFrameWndEx Class](../mfcref/cframewndex-class.md), and [CMFCToolBar Class](../mfcref/cmfctoolbar-class.md) classes.  
  
## Example  
 The following example demonstrates how to configure a color bar by using various methods in the `CMFCColorBar` class. The methods set the horizontal and vertical margins, enable the other button, create a color bar control window, and sets the currently selected color. This example is part of the [New Controls sample](../top/visual-c---samples.md).  
  
 [!code[NVC_MFC_NewControls#1](../mfcref/codesnippet/CPP/cmfccolorbar-class_1.h)]  
[!code[NVC_MFC_NewControls#2](../mfcref/codesnippet/CPP/cmfccolorbar-class_2.cpp)]  
...
## Inheritance Hierarchy  
 ...
 [CMFCPopupMenuBar](../mfcref/cmfcpopupmenubar-class.md)  
  
 [CMFCColorBar](../mfcref/cmfccolorbar-class.md)  
...
### Parameters  
 ...
 [in] `bAltColorDlg`  
 `TRUE` to display the [CMFCColorDialog](../mfcref/cmfccolordialog-class.md) dialog box; `FALSE` to display the standard [CColorDialog](../mfcref/ccolordialog-class.md) dialog box. The default value is `TRUE`.  
...
### Return Value  
 A [CSize](../atl/csize-class.md) object whose `cx` component contains the number of columns and whose `cy` component contains the number of rows.  

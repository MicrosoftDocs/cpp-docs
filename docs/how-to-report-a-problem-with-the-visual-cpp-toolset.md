...
1. Choose the **Developer Command Prompt** version that matches the version of Visual Studio you're using when it appears in the list of matches.

1. In the **Developer Command Prompt** console, enter the command `cl /Bv /CLR`.
...
1. Locate the line that contains the name of the source code file where the problem occurs; the line below it contains the corresponding cl.exe command and its arguments.
...
Here's example compiler output for this kind of crash in which LTCG is **not** used. If your compiler output looks like this you should provide a [Preprocessed Repro](#preprocessed-repros).
...
1. In the **Developer Command Prompt** console window, enter the command `cl /P argumentsfilename.cpp`.
...
1. In the **Developer Command Prompt** console window, enter the command `cl arguments /TP filename.i`.
...
1. Set the link_repro environment variable to the directory you just created; enter the command `set link_repro=directory`.

1. If you want to perform the build from inside Visual Studio, launch it from the command prompt by entering the command `devenv`. This ensures that the value of the link_repro environment variable is visible to Visual Studio.

1. Build your application, and confirm that the expected problem has occurred.

1. Close Visual Studio now if you launched it in step 3.

1. Clear the link_repro environment variable; enter the command `set link_repro=`

...
'class': syntax is only supported when the 'layout_dependent' parameter of event_receiver is true  
 ...
 You tried to hook an entire interface of events but `layout_dependent` on [event_receiver](../windows/event_receiver.md) attribute is not true; you must hook a single event at a time.  

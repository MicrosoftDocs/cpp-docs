...
'module_name': cannot specify different idl_module attributes in a project  
 ...
 [idl_module](../windows/idl_module.md) attributes with the same name but different `dllname` or `version`parameters were found in two of the files in a compilation. Only one unique `idl_module` attribute can be specified per compilation.  

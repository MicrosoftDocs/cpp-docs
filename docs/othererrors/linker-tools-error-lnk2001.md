...
unresolved external symbol "symbol"  
 ...
 This error message is followed by fatal error [LNK1120](../othererrors/linker-tools-error-lnk1120.md).  
...
-   When upgrading a managed library or web service project from Visual C++ 2003, the **/Zl** compiler option is added to the **Command Line** property page. This will cause LNK2001.  
     ...
     To resolve this error, either add msvcrt.lib and msvcmrt.lib to the linker's Additional Dependencies property. Or, remove **/Zl** from the **Command Line** property page. For more information, see [/Zl (Omit Default Library Name)](../buildref/-zl--omit-default-library-name-.md) and [How to: Open Project Property Pages](../notintoc/how-to--open-project-property-pages.md).  
...
-   If the LNK2001 diagnostic text reports that `__check_commonlanguageruntime_version` is an unresolved external symbol, see [LNK2019](../othererrors/linker-tools-error-lnk2019.md) for information on how to resolve.  
...
-   A [missing function body or variable](../othererrors/missing-function-body-or-variable.md) can cause LNK2001. With just a function prototype or `extern` declaration the compiler can continue without error, but the linker cannot resolve a call to an address or reference to a variable because there is no function code or variable space reserved.  
  
-   Calling a function with parameter types that do not match those in the function declaration can cause LNK2001. [Name decoration](../othererrors/name-decoration.md) incorporates the parameters of a function into the final decorated function name.  
...
-   When using C++, including a function prototype in a class definition and failing to [include the implementation](../othererrors/missing-function-body-or-variable.md) of the function for that class can cause LNK2001.  
...
-   Trying to use a variable declared within a function ([a local variable](../othererrors/automatic--function-scope--variables.md)) outside the scope of that function can cause LNK2001.  
...
-   The project is missing a reference to a library (.LIB) or object (.OBJ) file. See [.lib Files as Linker Input](../buildref/.lib-files-as-linker-input.md) for more information.  
  
-   If you use [/NODEFAULTLIB](../buildref/-nodefaultlib--ignore-libraries-.md) or [/Zl](../buildref/-zl--omit-default-library-name-.md), libraries containing required code will not be linked into the project unless you have explicitly included them. (When compiling with **/clr** or **/clr:pure**, you will see a reference to .cctor; see [Initialization of Mixed Assemblies](../cli/initialization-of-mixed-assemblies.md) for more information.)  
  
-   If you are using Unicode and MFC, you will get an unresolved external on `_WinMain@16` if you don't create an entrypoint to `wWinMainCRTStartup`; use the [/ENTRY](../buildref/-entry--entry-point-symbol-.md). See [Unicode Programming Summary](../text/unicode-programming-summary.md).  
...
-   Linking code requiring the multithreaded libraries (any MFC code or code compiled with [/MT](../buildref/-md---mt---ld--use-run-time-library-.md)) causes LNK2001 on [_beginthread](../crt/_beginthread--_beginthreadex.md), `_beginthreadex`, [_endthread](../crt/_endthread--_endthreadex.md), and `_endthreadex`. See the following Knowledge Base article for more information:  
...
-   There is currently no standard for [C++ naming](../othererrors/name-decoration.md) between compiler vendors or even between different versions of a compiler. Therefore, linking object files compiled with other compilers may not produce the same naming scheme and thus cause error LNK2001.  
  
-   [Mixing inline and non-inline compile options](../othererrors/function-inlining-problems.md) on different modules can cause LNK2001. If a C++ library is created with function inlining turned on (**/Ob1** or **/Ob2**) but the corresponding header file describing the functions has inlining turned off (no `inline` keyword), you will get this error. To prevent this problem, have the inline functions defined with `inline` in the header file you are going to include in other files.  
  
-   If you are using the `#pragma inline_depth` compiler directive, make sure you have a [value of 2 or greater set](../othererrors/function-inlining-problems.md), and make sure you are using the [/Ob1](../buildref/-ob--inline-function-expansion-.md) or [/Ob2](../buildref/-ob--inline-function-expansion-.md) compiler option.  
  
-   Omitting the LINK option /NOENTRY when creating a resource-only DLL will cause LNK2001.  
  
-   Using incorrect /SUBSYSTEM or /ENTRY settings can cause LNK2001. For example, if you write a character-based application (a console application) and specify /SUBSYSTEM:WINDOWS, you will get an unresolved external for `WinMain`. For more information on these options and entry points, see the [/SUBSYSTEM](../buildref/-subsystem--specify-subsystem-.md) and [/ENTRY](../buildref/-entry--entry-point-symbol-.md) linker options.  
  
 **Export Problems**  
  
-   When you are porting an application from 16 to 32 or 64 bits, LNK2001 can occur. The current module-definition (.def) file syntax requires that `__cdecl`, `__stdcall`, and `__fastcall` functions be listed in the EXPORTS section without underscores (undecorated). This differs from the 16-bit syntax, where they must be listed with underscores (decorated). For more information, see the description of the [EXPORTS](../buildref/exports.md) section of module-definition files.  
 ...
 **Interpreting the Output**  
 ...
 For more information on some of the causes listed above, see [Name decoration](../othererrors/name-decoration.md).
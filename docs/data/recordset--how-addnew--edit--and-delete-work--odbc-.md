...
>  This topic applies to objects derived from `CRecordset` in which bulk row fetching has not been implemented. If you are using bulk row fetching, see [Recordset: Fetching Records in Bulk (ODBC)](../data/recordset--fetching-records-in-bulk--odbc-.md).  
 ...
 As a supplement, you might want to read [Record Field Exchange: How RFX Works](../data/record-field-exchange--how-rfx-works.md), which describes the corresponding role of RFX in update operations.  
...
-   If **::SQLSetPos** cannot be used, MFC does the following:  
    ...
    3.  **Update** commits the changes — executes the **UPDATE** statement — and the record is changed on the data source, but not committed if a transaction is in progress (see [Transaction: Performing a Transaction in a Recordset (ODBC)](../data/transaction--performing-a-transaction-in-a-recordset--odbc-.md) for information about how the transaction affects the update). ODBC keeps a copy of the record, which also changes.  
...
-   If **::SQLSetPos** cannot be used, MFC does the following:  
 ...
 For information about the SQL statements used in update operations, see [SQL](../data/sql.md).  
  
## See Also  
 [Recordset (ODBC)](../data/recordset--odbc-.md)   
 [Recordset: More About Updates (ODBC)](../data/recordset--more-about-updates--odbc-.md)   

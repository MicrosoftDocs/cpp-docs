...
The wizard creates a class to contain one row of data; in this case, it is called `CCustomWindowsFile`. The following code for `CCustomWindowsFile` is wizard generated and lists all the files in a directory by using the `WIN32_FIND_DATA` structure. `CCustomWindowsFile` inherits from the `WIN32_FIND_DATA` structure:  

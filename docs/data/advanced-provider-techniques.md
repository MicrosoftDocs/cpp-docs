---
title: "Advanced Provider Techniques"
ms.custom: na
ms.date: "10/14/2016"
ms.prod: "visual-studio-dev14"
ms.reviewer: na
ms.suite: na
ms.technology: 
  - "devlang-cpp"
ms.tgt_pltfrm: na
ms.topic: "article"
dev_langs: 
  - "C++"
helpviewer_keywords: 
  - "OLE DB providers, using"
ms.assetid: 002a9731-7e06-49ac-be50-3cc26aa43759
caps.latest.revision: 8
ms.author: "mblome"
manager: "ghogen"
translation.priority.ht: 
  - "cs-cz"
  - "de-de"
  - "es-es"
  - "fr-fr"
  - "it-it"
  - "ja-jp"
  - "ko-kr"
  - "pl-pl"
  - "pt-br"
  - "ru-ru"
  - "tr-tr"
  - "zh-cn"
  - "zh-tw"
---
# Advanced Provider Techniques
For more information about advanced provider techniques, see the following topics:  
  
-   [Supporting Notifications](../data/supporting-notifications.md)  
  
-   [Supporting Schema Rowsets](../data/supporting-schema-rowsets.md)  
  
-   [Provider Support for Bookmarks](../data/provider-support-for-bookmarks.md)  
  
-   [Passing OLE DB Conformance Tests](../data/passing-ole-db-conformance-tests.md)  
  
-   [OLE DB Resource Pooling and Services](../data/ole-db-resource-pooling-and-services.md)  
  
## See Also  
 [OLE DB Provider Templates Reference](../data/ole-db-provider-templates-reference.md)   
 [OLE DB Consumer Templates](../data/ole-db-consumer-templates--c---.md)
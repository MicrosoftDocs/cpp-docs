...
|`intmax_t`<br />`uintmax_t`|**j** or **I** (uppercase i)|**d**, **i**, **o**, **u**, **x**, or **X**|  
...
The `intmax_t`, `uintmax_t`, `ptrdiff_t`, and `size_t` types are `__int32` or `unsigned __int32` on 32-bit platforms, and `__int64` or `unsigned __int64` on 64-bit platforms. The **I** (uppercase i), **j**, **t**, and **z** size prefixes take the correct argument width for the platform.  
...
> [!NOTE]
>  The **I** (uppercase i), **I32**, **I64**, and **w** argument size modifier prefixes are Microsoft extensions and are not ISO C-compatible. The **h** prefix when it's used with data of type `char` and the **l** prefix when it's used with data of type `double` are Microsoft extensions.  

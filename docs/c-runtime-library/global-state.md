...
An undesirable situation would emerge if global state was shared between applications and the OS. For example, if  your application calls `setlocale()`, it shouldn't effect the locale for every C runtime based component running on the operating system, or vice-versa.
...
The OS specific versions of these functions, for example, the OS specific version of `setlocale()` is `_o_setlocale()`, are in `ucrt.osmode.lib`.
...
<<<<<<< HEAD
## App and OS global state separation
=======
## Global state affected by app/OS separation
>>>>>>> 9cae21430b44bf80c55c0e048ce9c8b5bdd7dd36

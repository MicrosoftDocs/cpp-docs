...
> [!IMPORTANT]
>  Do not pass user input to `_spawn` without explicitly checking its content. `_spawn` will result in a call to [CreateProcess](https://msdn.microsoft.com/library/windows/desktop/ms682425) so keep in mind that unqualified path names could lead to potential security vulnerabilities.  
...
-   Use the Win32 API to create a pipe, then call [AllocConsole](https://msdn.microsoft.com/library/windows/desktop/ms681944), set the handle values in the startup structure, and call [CreateProcess](https://msdn.microsoft.com/library/windows/desktop/ms682425).  

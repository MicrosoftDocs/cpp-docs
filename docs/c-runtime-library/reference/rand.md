...
Generates a pseudorandom number by using a well-known and fully-reproducible algorithm. A more programmatically secure version of this function is available; see [rand_s](rand-s.md). Numbers generated by `rand` are not cryptographically secure. For more cryptographically secure random number generation, use `rand_s` or the functions declared in the C++ Standard Library in [\<random>](../../standard-library/random.md).

...
The `rand_s` function writes a pseudorandom integer in the range 0 to `UINT_MAX` to the input pointer. The `rand_s` function uses the operating system to generate cryptographically secure random numbers. It does not use the seed generated by the [srand](srand.md) function, nor does it affect the random number sequence used by `rand`.

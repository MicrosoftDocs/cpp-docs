...
When **_DEBUG** and **_CRTDBG_MAP_ALLOC** are defined, calls to **_getdcwd_dbg** and **_wgetdcwd_dbg** replace calls to **_getdcwd** and **_wgetdcwd**, respectively, so that you can debug memory allocations. For more information, see[`_getdcwd_dbg`, `_wgetdcwd_dbg`](getdcwd-dbg-wgetdcwd-dbg.md).

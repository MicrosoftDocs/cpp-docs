...
---
# Importing Settings
The [!INCLUDE[vsprvs](../build/includes/vsprvs_md.md)] integrated development environment (IDE) uses the classes that implement the \<xref:Microsoft.VisualStudio.Shell.IProfileManager> interface and are registered to support a VSPackage implementation. This implementation is used to retrieve the state of a VSPackage.  
  
 Because the IDE instantiates the class that implements the \<xref:Microsoft.VisualStudio.Shell.IProfileManager> interface to support the [!INCLUDE[vsprvs](../build/includes/vsprvs_md.md)] settings, the \<xref:Microsoft.VisualStudio.Shell.IProfileManager> interface should be implemented in an independent class.  
...
1.  Declare the class that implements the [!INCLUDE[vsprvs](../build/includes/vsprvs_md.md)] settings.  
...
2.  Ensure that the class that implements the settings retrieves state data from disk.  
    ...
    -   Prior to actually retrieving stored settings, an implementation of the \<xref:Microsoft.VisualStudio.Shell.IProfileManager.LoadSettingsFromXml*> method should use the \<xref:Microsoft.VisualStudio.Shell.Interop.IVsSettingsReader.ReadFileVersion*> method to verify that version of [!INCLUDE[vsprvs](../build/includes/vsprvs_md.md)] that exports the stored settings is supported.  
  
         In the case of the example below, the implementation checks to see if settings were produced by a version of [!INCLUDE[vsprvs](../build/includes/vsprvs_md.md)] with a major version number of `m_supportVer`, and if not, signals an error.  

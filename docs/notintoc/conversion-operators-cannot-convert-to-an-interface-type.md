...
A conversion operator is declared with an interface type as a return type.  
 ...
 At compile time, [!INCLUDE[vbprvb](../cli/includes/vbprvb_md.md)] considers a predefined conversion to exist from any reference type to any interface. Such a conversion might fail at run time, but the compiler cannot predict run-time results, so it allows any such conversion to compile.  

...
|Name|Description|  
|----------|-----------------|  
|[IDispEventImpl::_tihclass](../atl/idispeventimpl-class.md)|The class used to manage the type information. By default, `CComTypeInfoHolder`.|  
...
## Remarks  
 ...
 `IDispEventImpl` works in conjunction with the [event sink map](../Topic/BEGIN_SINK_MAP.md) in your class to route events to the appropriate handler function. To use this class:  
  
 Add a [SINK_ENTRY](../Topic/SINK_ENTRY.md) or [SINK_ENTRY_EX](../Topic/SINK_ENTRY_EX.md) macro to the event sink map for each event on each object that you want to handle. When using `IDispEventImpl` as a base class of a composite control, you can call [AtlAdviseSinkMap](../Topic/AtlAdviseSinkMap.md) to establish and break the connection with the event sources for all entries in the event sink map. In other cases, or for greater control, call [DispEventAdvise](../Topic/IDispEventSimpleImpl::DispEventAdvise.md) to establish the connection between the source object and the base class. Call [DispEventUnadvise](../Topic/IDispEventSimpleImpl::DispEventUnadvise.md) to break the connection.  
 ...
 `IDispEventImpl` provides the same functionality as [IDispEventSimpleImpl](../atl/idispeventsimpleimpl-class.md), except it gets type information about the interface from a type library rather than having it supplied as a pointer to an [_ATL_FUNC_INFO](../atl/_atl_func_info-structure.md) structure. Use `IDispEventSimpleImpl` when you do not have a type library describing the event interface or want to avoid the overhead associated with using the type library.  
...
## Inheritance Hierarchy  
 ...
 [IDispEventSimpleImpl](../atl/idispeventsimpleimpl-class.md)  
...
### Remarks  
 See [IDispatch::GetIDsOfNames](assetId:///6f6cf233-3481-436e-8d6a-51f93bf91619) in the [!INCLUDE[winSDK](../atl/includes/winsdk_md.md)].  
...
### Remarks  
 See [IDispatch::GetTypeInfoCount](assetId:///da876d53-cb8a-465c-a43e-c0eb272e2a12) in the [!INCLUDE[winSDK](../atl/includes/winsdk_md.md)].  

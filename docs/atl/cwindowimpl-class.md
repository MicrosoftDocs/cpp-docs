 ...
 *TBase*  
 The base class of your class. By default, the base class is [CWindow](../atl/cwindow-class.md).  
...
|||  
|-|-|  
|[GetWndClassInfo](../Topic/CWindowImpl::GetWndClassInfo.md)|Returns a static instance of [CWndClassInfo](../atl/cwndclassinfo-class.md), which manages the window class information.|  
...
## Remarks  
 ...
 `CWindowImpl::Create` creates a window based on the window class information that's managed by [CWndClassInfo](../atl/cwndclassinfo-class.md). `CWindowImpl` contains the [DECLARE_WND_CLASS](../Topic/DECLARE_WND_CLASS.md) macro, which means `CWndClassInfo` registers a new window class. If you want to superclass an existing window class, derive your class from `CWindowImpl` and include the [DECLARE_WND_SUPERCLASS](../Topic/DECLARE_WND_SUPERCLASS.md) macro. In this case, `CWndClassInfo` registers a window class that's based on an existing class but uses `CWindowImpl::WindowProc`. For example:  
  
 [!code[NVC_ATL_Windowing#43](../atl/codesnippet/CPP/cwindowimpl-class_1.h)]  
...
>  For any given `CWindowImpl` object, call either **Create** or `SubclassWindow`. Don't invoke both methods on the same object.  
 ...
 In addition to `CWindowImpl`, ATL provides [CContainedWindow](../atl/ccontainedwindowt-class.md) to create a window that's contained in another object.  
  
 The base class destructor (~ **CWindowImplRoot**) ensures that the window is gone before the object is destroyed.  
  
 `CWindowImpl` derives from **CWindowImplBaseT**, which derives from **CWindowImplRoot**, which derives from **TBase** and [CMessageMap](../atl/cmessagemap-class.md).  
...
|ATL Project Wizard|[Creating an ATL Project](../atl/creating-an-atl-project.md)|  
  
## Inheritance Hierarchy  
 [CMessageMap](../atl/cmessagemap-class.md)  
...
### Return Value  
 A static instance of [CWndClassInfo](../atl/cwndclassinfo-class.md).  
...
### Remarks  
 ...
 To superclass an existing window class, derive your class from `CWindowImpl` and include the [DECLARE_WND_SUPERCLASS](../Topic/DECLARE_WND_SUPERCLASS.md) macro to override `GetWndClassInfo`. For more information, see the [CWindowImpl](../atl/cwindowimpl-class.md) overview.  

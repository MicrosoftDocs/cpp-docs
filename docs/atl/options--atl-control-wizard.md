...
-   **Composite control**: An ActiveX control that can contain (similar to a dialog box) other ActiveX controls or Windows controls. A composite control includes the following:  
    ...
    -   An HTML test page containing the composite control.  
     ...
     By default, this control sets [CComControlBase::m_bWindowOnly](../Topic/CComControlBase::m_bWindowOnly.md) to true, to indicate that this is a windowed control. It implements a sink map. For more information, see [Support for DHTML Control](../atl/atl-support-for-dhtml-controls.md).  
...
-   **Connection points**: Enables connection points for your object by making your object's class derive from [IConnectionPointContainerImpl](../atl/iconnectionpointcontainerimpl-class.md) and allowing it to expose a source interface.  

...
### Parameters  
 `pSid`  
 Pointer to a [CSid](../atl/csid-class.md) (security identifier) that receives a copy of the group stored in the CDacl.  
...
```
    operatorconst  SECURITY_DESCRIPTOR *() const throw();
```  
...
### Parameters  
 `Sid`  
 The [CSid](../atl/csid-class.md) object for the security descriptor's new primary owner. This parameter must not be NULL.  

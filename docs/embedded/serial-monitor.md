...
To activate the Serial Monitor window, select **Tools** from the menu bar and then choose **Serial Monitor 2**.

:::image type="content" source="media/serial-monitor.png" alt-text="Screenshot of the Visual Studio Serial Monitor window.":::

## Capabilities

- **Send data to a serial port**: Enter text into the **Command** text field at the bottom of the view. Select **Send** to send the data.
...
| **Encoding** | Can select type of encoding for messages sent to serial port | Use the **Encoding** dropdown in **Serial** mode | Text (utf8), Hex, Binary |
...
| **File Logging** | Allows the ability to log output to a file | Use the **file logging** toggle button, as well as the **Record file path** button to choose the desired directory | On/Off |

...
Because the Universal CRT is a fundamental dependency of the C++ libraries, the Visual C++ redistributable (VCRedist) installs the initial version of the Universal CRT (version 10.0.10240) on machines that do not have a version already installed, sufficient to satisfy the C++ library dependencies. If your application depends on a more recent version of the Universal CRT, you must use Windows Update to bring your machine fully up-to-date, or install that more recent version explicitly. It is best to have the Universal C Runtime already installed via Windows Update or via MSU prior to installing the VCRedist, to avoid potential multiple required reboots.

Not all operating systems are eligible for the most recent Universal C Runtime via Windows Update. On Windows 10, the centrally deployed version will match the version of the operating system - to update the Universal C Runtime further, one must update the operating system. For Windows Vista through Windows 8.1, the latest available Universal C Runtime is currently based off of the version included with the Windows 10 Anniversary Update, with additional fixes (version 10.0.14393).

...
---
# Interface Attributes
The following attributes apply to the [interface (or __interface)](../cpp/__interface.md) C++ keyword.  
...
|[object](../windows/object--c---.md)|Identifies a custom interface.|  
...
|[uuid](../windows/uuid--c---attributes-.md)|Provides the unique ID for the library|  
  
 You must observe these rules for defining an interface:  
  
-   Default calling convention is [__stdcall](../cpp/__stdcall.md).  
  
-   A GUID is supplied for you if you do not supply one.  
  
-   No overloaded methods are allowed.  
  
 When not specifying the [uuid](../windows/uuid--c---attributes-.md) attribute and using the same interface name in different attribute projects, the same GUID is generated.  

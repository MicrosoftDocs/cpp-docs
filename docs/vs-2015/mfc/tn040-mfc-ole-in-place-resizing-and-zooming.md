...
The latest version of this topic can be found at [TN040: MFC-OLE In-Place Resizing and Zooming](https://docs.microsoft.com/cpp/mfc/tn040-mfc-ole-in-place-resizing-and-zooming).  
...
## MFC Support for Zooming  
 ...
 For an example of zooming correctly see the MFC OLE sample [HIERSVR](../top/visual-cpp-samples.md). Zooming in HIERSVR is complicated by the fact that it displays text, and text, in general, does not scale in a linear fashion (hints, typographic conventions, design widths, and heights all complicate the matter). Still, HIERSVR is a reasonable reference for implementing zooming correctly, and so is the MFC Tutorial [SCRIBBLE](../top/visual-cpp-samples.md) (step 7).  

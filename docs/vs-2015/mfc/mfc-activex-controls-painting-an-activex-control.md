...
The latest version of this topic can be found at [MFC ActiveX Controls: Painting an ActiveX Control](https://docs.microsoft.com/cpp/mfc/mfc-activex-controls-painting-an-activex-control).  
...
>  It is important that your modifications to `OnDraw` do not depend on the rectangle's upper left point being equal to (0, 0) and that you draw only inside the rectangle passed to `OnDraw`. Unexpected results can occur if you draw beyond the rectangle's area.  
 ...
 [!code-cpp[NVC_MFC_AxUI#1](../snippets/cpp/VS_Snippets_Cpp/NVC_MFC_AxUI/Cpp/MyAxUICtrl.cpp#1)]  

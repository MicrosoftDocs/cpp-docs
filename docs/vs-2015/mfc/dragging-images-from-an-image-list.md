...
The latest version of this topic can be found at [Dragging Images from an Image List](https://docs.microsoft.com/cpp/mfc/dragging-images-from-an-image-list).  
  
  
CImageList](../Topic/CImageList%20Class.md) includes functions for dragging an image on the screen. The dragging functions move an image smoothly, in color, and without any flashing of the cursor. Both masked and unmasked images can be dragged.  

...
The latest version of this topic can be found at [CWinFormsDialog Class](https://docs.microsoft.com/cpp/mfc/reference/cwinformsdialog-class).  
...
## Remarks  
 `CWinFormsDialog` is a wrapper for an MFC dialog class ( [CDialog](../../mfc/reference/cdialog-class.md)) that hosts a Windows Forms ( <xref:System.Windows.Forms.UserControlqualifyHint%3DFalse%26autoUpgrade%3DTrue>) user control. This allows the display of .NET Framework controls on a modal or modeless MFC dialog box.  
...
### Return Value  
 Returns a reference to the <xref:System.Windows.Forms.UserControlqualifyHint%3DFalse%26autoUpgrade%3DTrue> control in the MFC dialog box.  
...
### Remarks  
 This operator casts a type as a reference to a <xref:System.Windows.Forms.UserControlqualifyHint%3DFalse%26autoUpgrade%3DTrue> control. It is used to pass a `CWinFormsDialog<``TManagedControl``>` dialog box to functions that accept a pointer to a Windows Forms user control object.  
  
## See Also  
 <xref:System.Windows.Forms.UserControlqualifyHint%3DTrue%26autoUpgrade%3DTrue>   

...
The latest version of this topic can be found at [Accessing All Members of a Collection](https://docs.microsoft.com/cpp/mfc/accessing-all-members-of-a-collection).  
...
1.  Use sequential index numbers with the `GetAt` member function:  
     ...
     [!code-cpp[NVC_MFCCollections#12](../snippets/cpp/VS_Snippets_Cpp/NVC_MFCCollections/Cpp/MFCCollections.cpp#12)]  
     ...
     [!code-cpp[NVC_MFCCollections#13](../snippets/cpp/VS_Snippets_Cpp/NVC_MFCCollections/Cpp/MFCCollections.cpp#13)]  
...
1.  Use the member functions `GetHeadPosition` and `GetNext` to work your way through the list:  
     ...
     [!code-cpp[NVC_MFCCollections#14](../snippets/cpp/VS_Snippets_Cpp/NVC_MFCCollections/Cpp/MFCCollections.cpp#14)]  
...
1.  Use `GetStartPosition` to get to the beginning of the map and `GetNextAssoc` to repeatedly get the next key and value from the map, as shown by the following example:  
     ...
     [!code-cpp[NVC_MFCCollections#15](../snippets/cpp/VS_Snippets_Cpp/NVC_MFCCollections/Cpp/MFCCollections.cpp#15)]  
    ...
    >  For nontemplate maps, the compiler requires a reference to a `CObject` pointer in the last parameter to `GetNextAssoc`. On input, you must cast your pointers to that type, as shown in the next example.  
     ...
     [!code-cpp[NVC_MFCCollections#16](../snippets/cpp/VS_Snippets_Cpp/NVC_MFCCollections/Cpp/MFCCollections.cpp#16)]  

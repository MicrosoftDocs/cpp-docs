...
The latest version of this topic can be found at [MFC ActiveX Controls: Using Data Binding in an ActiveX Control](https://docs.microsoft.com/cpp/mfc/mfc-activex-controls-using-data-binding-in-an-activex-control).  
...
5.  Select the following check boxes from the **IDL Attributes** tab: **displaybind** and **defaultbind** to add the attributes to the property definition in the project's .IDL file. These attributes make the control visible to the user and make the stock property the default bindable property.  
 ...
 [!code-cpp[NVC_MFC_AxData#1](../snippets/cpp/VS_Snippets_Cpp/NVC_MFC_AxData/Cpp/MyAxDataCtrl.cpp#1)]  
...
11. Modify the body of the `SetMyProp` function so that it contains the following code:  
     ...
     [!code-cpp[NVC_MFC_AxData#2](../snippets/cpp/VS_Snippets_Cpp/NVC_MFC_AxData/Cpp/MyAxDataCtrl.cpp#2)]  
...
14. Modify the `OnDraw` function so that it contains the following code:  
     ...
     [!code-cpp[NVC_MFC_AxData#3](../snippets/cpp/VS_Snippets_Cpp/NVC_MFC_AxData/Cpp/MyAxDataCtrl.cpp#3)]  
...
15. To the public section of the header file the header file for your control class, add the following definitions (constructors) for member variables:  
     ...
     [!code-cpp[NVC_MFC_AxData#4](../snippets/cpp/VS_Snippets_Cpp/NVC_MFC_AxData/Cpp/MyAxDataCtrl.h#4)]  
  
16. Make the following line the last line in the `DoPropExchange` function:  
  
     [!code-cpp[NVC_MFC_AxData#5](../snippets/cpp/VS_Snippets_Cpp/NVC_MFC_AxData/Cpp/MyAxDataCtrl.cpp#5)]  
  
17. Modify the `OnResetState` function so that it contains the following code:  
  
     [!code-cpp[NVC_MFC_AxData#6](../snippets/cpp/VS_Snippets_Cpp/NVC_MFC_AxData/Cpp/MyAxDataCtrl.cpp#6)]  
  
18. Modify the `GetMyProp` function so that it contains the following code:  
  
     [!code-cpp[NVC_MFC_AxData#7](../snippets/cpp/VS_Snippets_Cpp/NVC_MFC_AxData/Cpp/MyAxDataCtrl.cpp#7)]  

...
## Registering Support for a Non-Standard Format  
 ...
 If a <xref:System.Drawing.Design.ToolboxItem> provider VSPackage does not register any special Clipboard formats, it must support the [!INCLUDE[vsprvs](../includes/vsprvs-md.md)] standard formats. For more information on the standard **Toolbox** Clipboard formats, see [Extending the Toolbox](../misc/extending-the-toolbox.md).  
...
## Registering Support for Dynamic Toolbox-Item Configuration  
 ...
 The registry data under HKEY_LOCAL_MACHINE\SOFTWARE\Microsoft\VisualStudio\\*\<Version>*\CLSID contains the information the [!INCLUDE[vsprvs](../includes/vsprvs-md.md)] environment requires to invoke the implementation of <xref:Microsoft.VisualStudio.Shell.IConfigureToolboxItem>.  

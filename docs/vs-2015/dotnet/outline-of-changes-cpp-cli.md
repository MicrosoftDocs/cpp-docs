...
The latest version of this topic can be found at [Outline of Changes (C++/CLI)](https://docs.microsoft.com/cpp/dotnet/outline-of-changes-cpp-cli).  
  
  
This outline shows you examples of some of the changes in the language from Managed Extensions for C++ to [!INCLUDE[cpp_current_long](../includes/cpp-current-long-md.md)]. Follow the link that accompanies each item for more information.  
...
-   The overload resolution between the built-in arithmetic types and the Object class hierarchy has reversed between Managed Extensions and [!INCLUDE[cpp_current_long](../includes/cpp-current-long-md.md)]. As a side-effect, CLR enums are no longer implicitly converted to arithmetic types.  

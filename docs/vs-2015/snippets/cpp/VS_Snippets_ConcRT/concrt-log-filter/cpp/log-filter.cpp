...
{
public:
    MyAgent(ISource<int>& source) : _source(source) {}
    ...
    void run()
    {
        ...
        //after network is hooked up, connect the start point to the input buf
        _source.link_target(&_input);
    }
private:
    ISource<int>& _source;
    unbounded_buffer<int> _input;
};
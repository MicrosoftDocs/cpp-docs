...
The latest version of this topic can be found at [Exporting and Importing Using AFX_EXT_CLASS](https://docs.microsoft.com/cpp/build/exporting-and-importing-using-afx-ext-class).  
  
  
Extension DLLs](../Topic/Extension%20DLLs:%20Overview.md) use the macro **AFX_EXT_CLASS** to export classes; the executables that link to the extension DLL use the macro to import classes. With the **AFX_EXT_CLASS** macro, the same header files that are used to build the extension DLL can be used with the executables that link to the DLL.  

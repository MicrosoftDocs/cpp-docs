...
The latest version of this topic can be found at [Build System Changes](https://docs.microsoft.com/cpp/build/build-system-changes).  
...
## .vcproj is now .vcxproj  
 Project files no longer use the .vcproj file name extension. Visual Studio automatically converts project files that were created by an earlier release of Visual C++ to the format that is used by the current system. For more information about how to manually upgrade a project, see [/Upgrade (devenv.exe)](http://msdn.microsoft.com/library/3468045c-5cc9-4157-9a9d-622452145d27).  
...
> [!IMPORTANT]
>  In the current release, the [!INCLUDE[TLA2#tla_ide](../includes/tla2sharptla-ide-md.md)] does not support the creation of new rules. For that reason, the easiest way to use a rule file from a project that was created by using an earlier release of Visual C++ is to migrate the project to the current release.  
...
## VC++ Directories Settings  
 ...
 You cannot change the VC++ directories settings if you run [devenv /resetsettings](http://msdn.microsoft.com/library/1d41021c-6f58-4bd5-b122-d1c995812192) at the command line. You also cannot change the settings if you open the **Tools** menu, click **Import and Export Settings**, and then select the **Reset all settings** option.  

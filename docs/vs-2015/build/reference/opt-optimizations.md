...
The latest version of this topic can be found at [-OPT (Optimizations)](https://docs.microsoft.com/cpp/build/reference/opt-optimizations).  
 ...
 **ICF[=**  `iterations` **] &#124; NOICF**  
 ...
 The linker behaves differently when **/OPT:REF** is specified—and **ICF** is in effect by default—than it does when **/OPT:REF,ICF** is specified explicitly. The form of **ICF** that's enabled with **/OPT:REF** alone does not fold read-only data—this includes .rdata, .pdata, and .xdata. Therefore, fewer functions are folded when images are produced for [!INCLUDE[vcprx64](../../includes/vcprx64-md.md)] because functions in these modules are more dependent on read-only data—for example, .pdata and .xdata. To get full **ICF** folding behavior, explicitly specify **/OPT:ICF**.  

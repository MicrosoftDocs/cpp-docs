...
The latest version of this topic can be found at [-Zc:inline (Remove unreferenced COMDAT)](https://docs.microsoft.com/cpp/build/reference/zc-inline-remove-unreferenced-comdat).  
...
## Remarks  
 ...
 If **/Zc:inline** is specified, the compiler enforces the C++11 requirement that all functions declared `inline` must have a definition available in the same translation unit if they are used. When the option is not specified, [!INCLUDE[vcprvc](../../includes/vcprvc-md.md)] allows non-conformant code that invokes functions declared `inline` even if no definition is visible. For more information, see the C++11 standard, in section 3.2 and section 7.1.2. This compiler option was introduced in Visual Studio 2013 Update 2.  

...
The latest version of this topic can be found at [Troubleshooting C/C++ Isolated Applications and Side-by-side Assemblies](https://docs.microsoft.com/cpp/build/troubleshooting-c-cpp-isolated-applications-and-side-by-side-assemblies).  
...
## Example  
 Assume we have an application, appl.exe, that's built by using [!INCLUDE[vcprvc](../includes/vcprvc-md.md)]. The application manifest either is embedded in appl.exe as the binary resource RT_MANIFEST, which has an ID equal to 1, or is stored as the separate file appl.exe.manifest. The content of this manifest resembles this:  

...
The latest version of this topic can be found at [partial  (C++ Component Extensions)](https://docs.microsoft.com/cpp/windows/partial-cpp-component-extensions).  
  
The `partial` keyword enables different parts of the same ref class to be authored independently and in different files.  
  
## All Runtimes  
 (This language feature applies only to the [!INCLUDE[wrt](../includes/wrt-md.md)].)  
...
### Parameters  
 *class-key*  
 A keyword that declares a class or struct that is supported by the [!INCLUDE[wrt](../includes/wrt-md.md)]. Either `ref class`, `value class`, `ref struct`, or `value struct`.  

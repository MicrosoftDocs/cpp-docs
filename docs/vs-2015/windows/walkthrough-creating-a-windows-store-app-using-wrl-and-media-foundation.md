...
The latest version of this topic can be found at [Walkthrough: Creating a Windows Store app using WRL and Media Foundation](https://docs.microsoft.com/cpp/windows/walkthrough-creating-a-windows-store-app-using-wrl-and-media-foundation).  
  
  
Learn how to use the [!INCLUDE[cppwrl](../includes/cppwrl-md.md)] ([!INCLUDE[cppwrl_short](../includes/cppwrl-short-md.md)]) to create a [!INCLUDE[win8_appname_long](../includes/win8-appname-long-md.md)] app that uses [Microsoft Media Foundation](http://msdn.microsoft.com/library/windows/apps/ms694197).  
...
>  Instead of C#, you can also use JavaScript, Visual Basic, or C++ to consume the custom transform component.  
 ...
 In most cases, you can use [!INCLUDE[cppwrt](../includes/cppwrt-md.md)] ([!INCLUDE[cppwrt_short](../includes/cppwrt-short-md.md)]) to create [!INCLUDE[wrt](../includes/wrt-md.md)] components. (For more information, see [Visual C++ Language Reference](../Topic/Visual%20C++%20Language%20Reference%20\(C++-CX\).md).) However, sometimes you have to use the [!INCLUDE[cppwrl_short](../includes/cppwrl-short-md.md)]. For example, when you create a media extension for Microsoft Media Foundation, you must create a component that implements both COM and [!INCLUDE[wrt](../includes/wrt-md.md)] interfaces. Because [!INCLUDE[cppwrt_short](../includes/cppwrt-short-md.md)] can only create [!INCLUDE[wrt](../includes/wrt-md.md)] objects, to create a media extension you must use the [!INCLUDE[cppwrl_short](../includes/cppwrl-short-md.md)] because it enables the implementation of both COM and [!INCLUDE[wrt](../includes/wrt-md.md)] interfaces.  
...
-   The `namespace` and `runtimeclass` attributes, and the `NTDDI_WIN8`[version](http://msdn.microsoft.com/en-us/66ac5cf3-2230-44fd-aaf6-8013e4a4ae81) attribute value are important parts of the MIDL definition for a Media Foundation component that uses [!INCLUDE[cppwrl_short](../includes/cppwrl-short-md.md)].  
  
-   [Microsoft::WRL::RuntimeClass](../windows/runtimeclass-class.md) is the base class for the custom Media Foundation component. The [Microsoft::WRL::RuntimeClassType::WinRtClassicComMix](../windows/runtimeclasstype-enumeration.md) enum value, which is provided as a template argument, marks the class for use both as a [!INCLUDE[wrt](../includes/wrt-md.md)] class and as a classic COM runtime class.  
...
-   Use project references to make [!INCLUDE[cppwrl_short](../includes/cppwrl-short-md.md)] components accessible to [!INCLUDE[win8_appname_long](../includes/win8-appname-long-md.md)] apps.  
  
### To use the [!INCLUDE[cppwrl_short](../includes/cppwrl-short-md.md)] to create the Media Foundation grayscale transform component  
...
4.  Add this code to GrayscaleTransform.idl.  
     ...
     [!code-cpp[wrl-media-capture#1](../snippets/cpp/VS_Snippets_Misc/wrl-media-capture/cpp/grayscaletransform/grayscaletransform.idl#1)]  
  
5.  Use the following code to replace the contents of pch.h.  
  
     [!code-cpp[wrl-media-capture#2](../snippets/cpp/VS_Snippets_Misc/wrl-media-capture/cpp/grayscaletransform/pch.h#2)]  
  
6.  Add a new header file to the project, name it `BufferLock.h`, and then add this code:  
  
     [!code-cpp[wrl-media-capture#3](../snippets/cpp/VS_Snippets_Misc/wrl-media-capture/cpp/grayscaletransform/bufferlock.h#3)]  
...
8.  Use the following code to replace the contents of GrayscaleTransform.cpp.  
     ...
     [!code-cpp[wrl-media-capture#4](../snippets/cpp/VS_Snippets_Misc/wrl-media-capture/cpp/grayscaletransform/grayscaletransform.cpp#4)]  
...
10. Use the following code to replace the contents of dllmain.cpp.  
     ...
     [!code-cpp[wrl-media-capture#6](../snippets/cpp/VS_Snippets_Misc/wrl-media-capture/cpp/grayscaletransform/dllmain.cpp#6)]  
...
### To use the [!INCLUDE[cppwrl_short](../includes/cppwrl-short-md.md)] the custom Media Foundation component from a C# app  
...
4.  In MainPage.xaml, add this code to the root [Grid](http://msdn.microsoft.com/library/windows/apps/xaml/windows.ui.xaml.controls.grid.aspx) element:  
     ...
     [!code-xml[wrl-media-capture#7](../snippets/csharp/VS_Snippets_Misc/wrl-media-capture/cs/mediacapture/mainpage.xaml#7)]  
  
5.  Use the following code to replace the contents of MainPage.xaml.cs.  
  
     [!code-csharp[wrl-media-capture#8](../snippets/csharp/VS_Snippets_Misc/wrl-media-capture/cs/mediacapture/mainpage.xaml.cs#8)]  
  
 The following illustration shows the MediaCapture app.  
  
 ![MediaCapture app capturing a photo](../windows/media/wrl-media-capture.png "WRL_Media_Capture")  

---
title: "Module::MethodReleaseNotifier::method_ Data Member | Microsoft Docs"
ms.custom: ""
ms.date: "2018-06-30"
ms.prod: "visual-studio-dev14"
ms.reviewer: ""
ms.suite: ""
ms.technology: 
  - "devlang-cpp"
ms.tgt_pltfrm: ""
ms.topic: "reference"
f1_keywords: 
  - "module/Microsoft::WRL::Module::MethodReleaseNotifier::method_"
dev_langs: 
  - "C++"
helpviewer_keywords: 
  - "method_ data member"
ms.assetid: 57f35cee-4eef-4a3a-9c70-ed4356b37d87
caps.latest.revision: 5
author: "mikeblome"
ms.author: "mblome"
manager: "ghogen"
---
# Module::MethodReleaseNotifier::method_ Data Member
[!INCLUDE[blank_token](../includes/blank-token.md)]

The latest version of this topic can be found at [Module::MethodReleaseNotifier::method_ Data Member](https://docs.microsoft.com/cpp/windows/module-methodreleasenotifier-method-data-member).  
  
  
Holds a pointer to the event handler for the current Module::MethodReleaseNotifier object.  
  
## Syntax  
  
```  
void (T::* method_)();  
```  
  
## Requirements  
 **Header:** module.h  
  
 **Namespace:** Microsoft::WRL  
  
## See Also  
 [Module::MethodReleaseNotifier Class](../windows/module-methodreleasenotifier-class.md)


...
The latest version of this topic can be found at [Adding a Property to the Control (ATL Tutorial, Part 3)](https://docs.microsoft.com/cpp/atl/adding-a-property-to-the-control-atl-tutorial-part-3).  
...
1.  From Solution Explorer, open PolyCtl.h and add the following line after the definition of `m_clrFillColor`:  
     ...
     [!code-cpp[NVC_ATL_Windowing#44](../snippets/cpp/VS_Snippets_Cpp/NVC_ATL_Windowing/Cpp/PolyCtl.h#44)]  
  
2.  Set the default value of `m_nSides`. Make the default shape a triangle by adding a line to the constructor in PolyCtl.h:  
  
     [!code-cpp[NVC_ATL_Windowing#45](../snippets/cpp/VS_Snippets_Cpp/NVC_ATL_Windowing/Cpp/PolyCtl.h#45)]  
...
3.  Implement the `Get` and `Put` methods. The `get_Sides` and `put_Sides` function declarations have been added to PolyCtl.h. Replace the code in PolyCtl.cpp for `get_Sides` and `put_Sides` with the following code:  
     ...
     [!code-cpp[NVC_ATL_Windowing#46](../snippets/cpp/VS_Snippets_Cpp/NVC_ATL_Windowing/Cpp/PolyCtl.cpp#46)]  

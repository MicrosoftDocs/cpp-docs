...
The latest version of this topic can be found at [Adding an Event (ATL Tutorial, Part 5)](https://docs.microsoft.com/cpp/atl/adding-an-event-atl-tutorial-part-5).  
...
12. Repeat the steps above to define a `ClickOut` method with the same `LONG` parameters `x` and `y`, the same **Parameter attributes** and the same `void` return type.  
 ...
 [!code-cpp[NVC_ATL_Windowing#56](../snippets/cpp/VS_Snippets_Cpp/NVC_ATL_Windowing/Cpp/NVC_ATL_Windowing.idl#56)]  
...
1.  Change the code which comprises the `OnLButtonDown` method in PolyCtl.cpp (deleting any code placed by the wizard) so that it looks like this:  
     ...
     [!code-cpp[NVC_ATL_Windowing#57](../snippets/cpp/VS_Snippets_Cpp/NVC_ATL_Windowing/Cpp/PolyCtl.cpp#57)]  

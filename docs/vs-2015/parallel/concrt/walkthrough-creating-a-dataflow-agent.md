...
##  <a name="control-flow"></a> Creating a Basic Control-Flow Agent  
 ...
 [!code-cpp[concrt-dataflow-agent#1](../../snippets/cpp/VS_Snippets_ConcRT/concrt-dataflow-agent/cpp/dataflow-agent.cpp#1)]  
...
2.  Remove the body of the loop that calls `receive` from the `run` method.  
 ...
 [!code-cpp[concrt-dataflow-agent#2](../../snippets/cpp/VS_Snippets_ConcRT/concrt-dataflow-agent/cpp/dataflow-agent.cpp#2)]  
...
3.  In the `run` method, after the initialization of the variables `negative_count` and `positive_count`, add a `countdown_event` object that tracks the count of active operations.  
 ...
 [!code-cpp[concrt-dataflow-agent#6](../../snippets/cpp/VS_Snippets_ConcRT/concrt-dataflow-agent/cpp/dataflow-agent.cpp#6)]  
...
4.  Create the message buffer objects that will participate in the dataflow network.  
 ...
 [!code-cpp[concrt-dataflow-agent#3](../../snippets/cpp/VS_Snippets_ConcRT/concrt-dataflow-agent/cpp/dataflow-agent.cpp#3)]  
  
5.  Connect the message buffers to form a network.  
  
 [!code-cpp[concrt-dataflow-agent#4](../../snippets/cpp/VS_Snippets_ConcRT/concrt-dataflow-agent/cpp/dataflow-agent.cpp#4)]  
...
6.  Wait for the `event` and `countdown event` objects to be set. These events signal that that the agent has received the sentinel value and that all operations have finished.  
 ...
 [!code-cpp[concrt-dataflow-agent#5](../../snippets/cpp/VS_Snippets_ConcRT/concrt-dataflow-agent/cpp/dataflow-agent.cpp#5)]  
  
 The following diagram shows the complete dataflow network for the `dataflow_agent` class:  
  
 ![The dataflow network](../../parallel/concrt/media/concrt-dataflow.png "ConcRT_Dataflow")  
...
|`connector`|A [concurrency::unbounded_buffer](~/parallel/concrt/reference/unbounded-buffer-class.md) object that connects the source message buffer to the internal network.|  
 ...
 [!code-cpp[concrt-dataflow-agent#7](../../snippets/cpp/VS_Snippets_ConcRT/concrt-dataflow-agent/cpp/dataflow-agent.cpp#7)]  
...
##  <a name="logging"></a> Creating a Message-Logging Agent  
 ...
 [!code-cpp[concrt-log-filter#1](../../snippets/cpp/VS_Snippets_ConcRT/concrt-log-filter/cpp/log-filter.cpp#1)]  

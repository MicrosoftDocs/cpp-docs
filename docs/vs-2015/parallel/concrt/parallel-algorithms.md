...
### Example  
 ...
 [!code-cpp[concrt-parallel-for-structure#1](../../snippets/cpp/VS_Snippets_ConcRT/concrt-parallel-for-structure/cpp/parallel-for-structure.cpp#1)]  
...
### Example  
 ...
 [!code-cpp[concrt-parallel-for-each-structure#1](../../snippets/cpp/VS_Snippets_ConcRT/concrt-parallel-for-each-structure/cpp/parallel-for-each-structure.cpp#1)]  
...
### Example  
 ...
 [!code-cpp[concrt-parallel-invoke-structure#1](../../snippets/cpp/VS_Snippets_ConcRT/concrt-parallel-invoke-structure/cpp/parallel-invoke-structure.cpp#1)]  
...
-   Perform 3-D ray tracing, where each iteration refers to one pixel that must be rendered.  
 ...
 [!code-cpp[concrt-basic-parallel-transform#1](../../snippets/cpp/VS_Snippets_ConcRT/concrt-basic-parallel-transform/cpp/basic-parallel-transform.cpp#1)]  
...
>  This example demonstrates the basic use of `parallel_transform`. Because the work function does not perform a significant amount of work, a significant increase in performance is not expected in this example.  
 ...
 [!code-cpp[concrt-parallel-transform-vectors#2](../../snippets/cpp/VS_Snippets_ConcRT/concrt-parallel-transform-vectors/cpp/parallel-transform-vectors.cpp#2)]  
...
-   Combine a list of elements, such as strings, into one element.  
 ...
 [!code-cpp[concrt-basic-parallel-reduce#1](../../snippets/cpp/VS_Snippets_ConcRT/concrt-basic-parallel-reduce/cpp/basic-parallel-reduce.cpp#1)]  
...
###  <a name="map_reduce_example"></a> Example: Performing Map and Reduce in Parallel  
 ...
 [!code-cpp[concrt-parallel-map-reduce-sum-of-primes#1](../../snippets/cpp/VS_Snippets_ConcRT/concrt-parallel-map-reduce-sum-of-primes/cpp/parallel-map-reduce-sum-of-primes.cpp#1)]  
...
>  The `parallel_for_each` and `parallel_transform` algorithms support only containers that use random access iterators (such as std::[vector](../../standard-library/vector-class.md)) for the static, simple, and affinity partitioners. The use of containers that use bidirectional and forward iterators produces a compile-time error. The default partitioner, `auto_partitioner`, supports all three of these iterator types.  
 ...
 [!code-cpp[concrt-static-partitioner#1](../../snippets/cpp/VS_Snippets_ConcRT/concrt-static-partitioner/cpp/static-partitioner.cpp#1)]  
 ...
 [!code-cpp[concrt-affinity-partitioner#1](../../snippets/cpp/VS_Snippets_ConcRT/concrt-affinity-partitioner/cpp/affinity-partitioner.cpp#1)]  
...
|`parallel_radixsort`|Integer key-based sort that requires O(N) space.|Hash-based|Stable|Requires additional O(N) space|O(N/P)|Random|  
 ...
 ![Comparison of the sorting algorithms](../../parallel/concrt/media/concrt-parallel-sorting.png "concrt_parallel_sorting")  
...
>  These parallel sorting algorithms support move semantics. You can define a move assignment operator to enable swap operations to occur more efficiently. For more information about move semantics and the move assignment operator, see [Rvalue Reference Declarator: &&](../../cpp/rvalue-reference-declarator-amp-amp.md), and [Move Constructors and Move Assignment Operators (C++)](../../cpp/move-constructors-and-move-assignment-operators-cpp.md). If you do not provide a move assignment operator or swap function, the sorting algorithms use the copy constructor.  
 ...
 [!code-cpp[concrt-parallel-sort-points#1](../../snippets/cpp/VS_Snippets_ConcRT/concrt-parallel-sort-points/cpp/parallel-sort-points.cpp#1)]  
 ...
 [!code-cpp[concrt-parallel-sort-points#2](../../snippets/cpp/VS_Snippets_ConcRT/concrt-parallel-sort-points/cpp/parallel-sort-points.cpp#2)]  
  
 [!code-cpp[concrt-parallel-sort-points#3](../../snippets/cpp/VS_Snippets_ConcRT/concrt-parallel-sort-points/cpp/parallel-sort-points.cpp#3)]  
...
>  Implementing a good hash function requires that you know the dataset range and how each element in the dataset is transformed to a corresponding unsigned value. Because the hash operation works on unsigned values, consider a different sorting strategy if unsigned hash values cannot be produced.  
 ...
 [!code-cpp[concrt-choosing-parallel-sort#1](../../snippets/cpp/VS_Snippets_ConcRT/concrt-choosing-parallel-sort/cpp/choosing-parallel-sort.cpp#1)]  

...
## Programming Model  
 ...
 The following illustration shows how two agents use message blocks and message-passing functions to communicate. In this illustration, `agent1` sends a message to `agent2` by using the [concurrency::send](http://msdn.microsoft.com/library/8713fb36-066b-47de-af12-589fa74805d6) function and a [concurrency::unbounded_buffer](~/parallel/concrt/reference/unbounded-buffer-class.md) object. `agent2` uses the [concurrency::receive](http://msdn.microsoft.com/library/f36bbca1-97ac-4343-bfac-ea71ca2139e9) function to read the message. `agent2` uses the same method to send a message to `agent1`. Dashed arrows represent the flow of data between agents. Solid arrows connect the agents to the message blocks that they write to or read from.  
  
 ![The components of the Agents Library](../../parallel/concrt/media/agent-librarycomp.png "Agent_LibraryComp")  
...
## Example  
 ...
 [!code-cpp[concrt-basic-agents#1](../../snippets/cpp/VS_Snippets_ConcRT/concrt-basic-agents/cpp/basic-agents.cpp#1)]  

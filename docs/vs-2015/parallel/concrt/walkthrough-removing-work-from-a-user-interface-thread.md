...
-   [Cancellation](../../parallel/concrt/exception-handling-in-the-concurrency-runtime.md#cancellation_in_the_ppl)  
 ...
 We also recommend that you understand the basics of MFC application development and [!INCLUDE[ndptecgdiplus](../../includes/ndptecgdiplus-md.md)] before you start this walkthrough. For more information about MFC, see [MFC Desktop Applications](../../mfc/mfc-desktop-applications.md). For more information about [!INCLUDE[ndptecgdiplus](../../includes/ndptecgdiplus-md.md)], see [GDI+]--brokenlink--(_gdiplus_GDI_start_cpp).  
...
##  <a name="serial"></a> Implementing the Serial Version of the Mandelbrot Application  
 This section describes how to draw the Mandelbrot fractal. This version draws the Mandelbrot fractal to a [!INCLUDE[ndptecgdiplus](../../includes/ndptecgdiplus-md.md)] [Bitmap](https://msdn.microsoft.com/library/ms534420.aspx) object and then copies the contents of that bitmap to the client window.  
  
#### To implement the serial version of the Mandelbrot application  
  
1.  In stdafx.h, add the following `#include` directive:  
  
     [!code-cpp[concrt-mandelbrot#1](../../snippets/cpp/VS_Snippets_ConcRT/concrt-mandelbrot/cpp/mandelbrot/stdafx.h#1)]  
...
2.  In ChildView.h, after the `pragma` directive, define the `BitmapPtr` type. The `BitmapPtr` type enables a pointer to a `Bitmap` object to be shared by multiple components. The `Bitmap` object is deleted when it is no longer referenced by any component.  
     ...
     [!code-cpp[concrt-mandelbrot#2](../../snippets/cpp/VS_Snippets_ConcRT/concrt-mandelbrot/cpp/mandelbrot/childview.h#2)]  
  
3.  In ChildView.h, add the following code to the `protected` section of the `CChildView` class:  
  
     [!code-cpp[concrt-mandelbrot#3](../../snippets/cpp/VS_Snippets_ConcRT/concrt-mandelbrot/cpp/mandelbrot/childview.h#3)]  
  
4.  In ChildView.cpp, comment out or remove the following lines.  
  
     [!code-cpp[concrt-mandelbrot#4](../../snippets/cpp/VS_Snippets_ConcRT/concrt-mandelbrot/cpp/mandelbrot/childview.cpp#4)]  
  
     In Debug builds, this step prevents the application from using the `DEBUG_NEW` allocator, which is incompatible with [!INCLUDE[ndptecgdiplus](../../includes/ndptecgdiplus-md.md)].  
  
5.  In ChildView.cpp, add a `using` directive to the `Gdiplus` namespace.  
  
     [!code-cpp[concrt-mandelbrot#5](../../snippets/cpp/VS_Snippets_ConcRT/concrt-mandelbrot/cpp/mandelbrot/childview.cpp#5)]  
  
6.  Add the following code to the constructor and destructor of the `CChildView` class to initialize and shut down [!INCLUDE[ndptecgdiplus](../../includes/ndptecgdiplus-md.md)].  
  
     [!code-cpp[concrt-mandelbrot#6](../../snippets/cpp/VS_Snippets_ConcRT/concrt-mandelbrot/cpp/mandelbrot/childview.cpp#6)]  
  
7.  Implement the `CChildView::DrawMandelbrot` method. This method draws the Mandelbrot fractal to the specified `Bitmap` object.  
  
     [!code-cpp[concrt-mandelbrot#7](../../snippets/cpp/VS_Snippets_ConcRT/concrt-mandelbrot/cpp/mandelbrot/childview.cpp#7)]  
...
8.  Implement the `CChildView::OnPaint` method. This method calls `CChildView::DrawMandelbrot` and then copies the contents of the `Bitmap` object to the window.  
     ...
     [!code-cpp[concrt-mandelbrot#8](../../snippets/cpp/VS_Snippets_ConcRT/concrt-mandelbrot/cpp/mandelbrot/childview.cpp#8)]  
...
1.  In stdafx.h, add the following `#include` directives:  
     ...
     [!code-cpp[concrt-mandelbrot#101](../../snippets/cpp/VS_Snippets_ConcRT/concrt-mandelbrot/cpp/parallelmandelbrot/stdafx.h#101)]  
...
2.  In ChildView.h, add `task_group` and `unbounded_buffer` member variables to the `protected` section of the `CChildView` class. The `task_group` object holds the tasks that perform drawing; the `unbounded_buffer` object holds the completed Mandelbrot image.  
     ...
     [!code-cpp[concrt-mandelbrot#102](../../snippets/cpp/VS_Snippets_ConcRT/concrt-mandelbrot/cpp/parallelmandelbrot/childview.h#102)]  
...
5.  Update the `CChildView::OnPaint` method to receive the updated `Bitmap` object and draw the image to the client window.  
     ...
     [!code-cpp[concrt-mandelbrot#105](../../snippets/cpp/VS_Snippets_ConcRT/concrt-mandelbrot/cpp/parallelmandelbrot/childview.cpp#105)]  
...
1.  In ChildView.h, in the `protected` section of the `CChildView` class, add declarations for the `OnSize`, `OnSizing`, and `OnDestroy` message map functions.  
     ...
     [!code-cpp[concrt-mandelbrot#201](../../snippets/cpp/VS_Snippets_ConcRT/concrt-mandelbrot/cpp/parallelmandelbrot/childview.h#201)]  
  
2.  In ChildView.cpp, modify the message map to contain handlers for the `WM_SIZE`, `WM_SIZING`, and `WM_DESTROY` messages.  
  
     [!code-cpp[concrt-mandelbrot#202](../../snippets/cpp/VS_Snippets_ConcRT/concrt-mandelbrot/cpp/parallelmandelbrot/childview.cpp#202)]  
  
3.  Implement the `CChildView::OnSizing` method. This method cancels any existing drawing tasks.  
  
     [!code-cpp[concrt-mandelbrot#203](../../snippets/cpp/VS_Snippets_ConcRT/concrt-mandelbrot/cpp/parallelmandelbrot/childview.cpp#203)]  
...
4.  Implement the `CChildView::OnSize` method. This method cancels any existing drawing tasks and creates a new drawing task for the updated client window size.  
     ...
     [!code-cpp[concrt-mandelbrot#204](../../snippets/cpp/VS_Snippets_ConcRT/concrt-mandelbrot/cpp/parallelmandelbrot/childview.cpp#204)]  
  
5.  Implement the `CChildView::OnDestroy` method. This method cancels any existing drawing tasks.  
  
     [!code-cpp[concrt-mandelbrot#205](../../snippets/cpp/VS_Snippets_ConcRT/concrt-mandelbrot/cpp/parallelmandelbrot/childview.cpp#205)]  
  
6.  In ChildView.cpp, define the `scope_guard` class, which implements the RAII pattern.  
  
     [!code-cpp[concrt-mandelbrot#206](../../snippets/cpp/VS_Snippets_ConcRT/concrt-mandelbrot/cpp/parallelmandelbrot/childview.cpp#206)]  
  
7.  Add the following code to the `CChildView::DrawMandelbrot` method after the call to `Bitmap::LockBits`:  
  
     [!code-cpp[concrt-mandelbrot#207](../../snippets/cpp/VS_Snippets_ConcRT/concrt-mandelbrot/cpp/parallelmandelbrot/childview.cpp#207)]  
...
8.  Modify the end of the `CChildView::DrawMandelbrot` method to dismiss the `scope_guard` object after the bitmap bits are unlocked, but before any messages are sent to the UI thread. This ensures that the UI thread is not updated before the bitmap bits are unlocked.  
     ...
     [!code-cpp[concrt-mandelbrot#208](../../snippets/cpp/VS_Snippets_ConcRT/concrt-mandelbrot/cpp/parallelmandelbrot/childview.cpp#208)]  

...
A statement attempts to construct a type from a generic type, or call a generic method, without using an [Of](/dotnet/articles/visual-basic/language-reference/statements/of-clause) clause.  
  
 The [!INCLUDE[vbprvb](../dotnet/includes/vbprvb_md.md)] syntax for generic types calls for the type parameters and type arguments to be introduced by the `Of` keyword. In addition, the type parameter list or type argument list must be enclosed within parentheses.  

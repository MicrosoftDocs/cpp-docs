...
A `Let` or `Set` assignment statement does not include an equal sign (`=`).  
 ...
 The [Set Statement](/dotnet/articles/visual-basic/language-reference/statements/set-statement) is used in property definitions and has no relation to the `Set` assignment statement.  

...
## Remarks  
 The member function returns a pair of iterators `cliext::pair<iterator, iterator>(` [hash_multimap::lower_bound (STL/CLR)](../cli/hash_multimap--lower_bound--stl-clr-.md)`(``key``),` [hash_multimap::upper_bound (STL/CLR)](../cli/hash_multimap--upper_bound--stl-clr-.md)`(``key``))`. You use it to determine the range of elements currently in the controlled sequence that match a specified key.  

...
A *symbol* is the name for a function or global variable used internally by a compiled object file or library. For a global variable, a symbol is defined in the object file where storage is allocated for it. A function is defined where the compiled code for the function body is placed. An *external symbol* is a symbol that's *referenced* in one object file, but defined in a different library or object file. An *exported symbol* is one that's made publicly available by the object file or library that defines it.

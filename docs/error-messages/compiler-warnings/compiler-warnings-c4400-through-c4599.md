...
|Compiler warning (level 1) C4492|'*function1*': matches base ref class method '*function2*', but is not marked 'override'|  
...
|Compiler warning (level 1) C4586|'*type*': A public type cannot be declared in a top-level namespace called 'Windows'|  

...
1. If a `Dispose` method is present, rewrite that into the class destructor.

1. If a destructor is present but there is no `Dispose` method, retain the destructor while performing the first item.
